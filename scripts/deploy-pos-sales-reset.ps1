# Deploy API + reset all POS sale orders + recalc stock on production
param(
...
Invoke-PuttyScp -Pscp $pscp -Password $Password -LocalPath "$RepoRoot\scripts\reset_pos_sale_orders_recalc_stock.sql" -RemotePath "${User}@${Server}:/root/reset_pos_sale_orders_recalc_stock.sql"
...
echo '--- reset_pos_sale_orders_recalc_stock ---' && \
docker cp /root/reset_pos_sale_orders_recalc_stock.sql zkteco_postgres:/tmp/reset_pos_sale_orders_recalc_stock.sql && \
docker exec zkteco_postgres psql -U postgres -d ZKTecoADMS -f /tmp/reset_pos_sale_orders_recalc_stock.sql && \
echo '--- deploy API ---' && \
/root/deploy_api.sh && \
sleep 8 && \
...
Write-Host "==> Remote SQL reset + API deploy..."
