...
class PosPrinterHardware {
  ...
  static Future<({List<Map<String, String>> devices, String? hint})>
    ...
    // Android 12+: Nearby devices. iOS: bluetooth. Android cũ: Location.
    try {
      if (Platform.isIOS) {
    ...
    if (!enabled) {
      ...
      return (
        ...
        hint: Platform.isIOS
            ? 'Bluetooth đang tắt. Bật Bluetooth, ghép máy in trong Cài đặt iOS, rồi bấm làm mới.'
    ...
    // Native bonded list chỉ Android; iOS dùng plugin pairedBluetooths.
    if (isSupported) {
