...
class TaskAssignmentTab extends StatefulWidget {
  ...
  final void Function(String employeeId)? onFilterByAssignee;
...
class _TaskAssignmentTabState extends State<TaskAssignmentTab> {
  ...
  Widget _buildMetricGrid(TaskAssignmentDashboard d) {
    final cards = <Widget>[
      _metricCard(
        'Chờ tôi xác nhận',
      ...
      if (widget.isManager)
        _metricCard(
          ...
          onTap: () => widget.onFilterList?.call(assignedByMe: true),
        ),
      if (widget.isManager)
        _metricCard(
          'Quá hạn (đã giao)',
