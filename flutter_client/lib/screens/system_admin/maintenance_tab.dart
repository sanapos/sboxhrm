    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Row(children: [
            Expanded(
                child: Text('Cửa sổ bảo trì (${windows.length})',
            ...
            ElevatedButton.icon(
              ...
              label: const Text('Lên lịch bảo trì'),
            ),
          ]),
        ...
        child: Column(
          ...
          children: [
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (_) => [
                  PopupMenuItem(
                      ...
                      child: Text(isActive ? '⏸️ Tắt' : '▶️ Bật')),
                  const PopupMenuItem(
                      value: 'delete',
                      child: Text('🗑️ Xoá',
            ...
            Row(children: [
              ...
              Text(
                  '${start != null ? _df.format(start) : "?"} → ${end != null ? _df.format(end) : "?"}',
                  style:
                      TextStyle(fontSize: 12, color: Colors.grey[700])),
              const Spacer(),
              if ((w['notifyBeforeMinutes'] as List?)?.isNotEmpty == true)
                Text(
                    'Nhắc trước: ${(w['notifyBeforeMinutes'] as List).join(", ")} phút',
      ...
      case 'delete':
        final ok = await showDialog<bool>(
                ...
                builder: (_) => AlertDialog(
                      title: const Text('Xoá cửa sổ bảo trì?'),
            ...
            child: SingleChildScrollView(
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                ...
                ListTile(
                    ...
                    title: Text('Bắt đầu: ${_df.format(start)}'),
                    trailing: TextButton(
                ...
                ListTile(
                    contentPadding: EdgeInsets.zero,
                    title: Text('Kết thúc: ${_df.format(end)}'),
                    trailing: TextButton(
                      child: const Text('Chọn'),
                      onPressed: () async {
                ...
                SwitchListTile(
                    dense: true,
                    title: const Text('Chặn truy cập user thường (trả 503)'),
                    value: blockAccess,
                    onChanged: (v) => setS(() => blockAccess = v)),
                SwitchListTile(
                    dense: true,
                    title: const Text('Bật ngay (active)'),
                    value: isActive,
              ...
              onPressed: () async {
                if (title.text.trim().isEmpty || message.text.trim().isEmpty) {
                  ScaffoldMessenger.of(ctx).showSnackBar(
                      const SnackBar(content: Text('Nhập tiêu đề & mô tả')));
                  return;
                }
