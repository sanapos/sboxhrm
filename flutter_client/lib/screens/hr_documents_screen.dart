...
class _HrDocumentsScreenState extends State<HrDocumentsScreen>
  ...
  Widget _buildDocumentsList(List<Map<String, dynamic>> docs) {
    ...
    final paginatedDocs =
        docs.sublist(startIndex.clamp(0, totalCount), endIndex);
  ...
  Widget _buildExpiringList() {
    ...
    final endIndex = (page * _pageSize).clamp(0, totalCount);
    final paginatedDocs =
        _expiringDocs.sublist(startIndex.clamp(0, totalCount), endIndex);
  ...
  Widget _buildDocDeckItem(Map<String, dynamic> doc,
    ...
    AppResponsiveDialog.show(
      ...
      actions: AppDialogActions(
        ...
        onConfirm: () async {
          ...
          try {
            if (isEdit) {
              await _apiService.updateHrDocument(
                  doc['id']?.toString() ?? '', data);
            } else {
