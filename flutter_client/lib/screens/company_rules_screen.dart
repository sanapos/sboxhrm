...
class CompanyRulesScreen extends StatefulWidget {
  ...
  @override
  State<CompanyRulesScreen> createState() => _CompanyRulesScreenState();
}

class _CompanyRulesScreenState extends State<CompanyRulesScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      debugPrint('Load rules data error: $e');
    }
  ...
  Future<void> _loadArticles() async {
    try {
      final result = await _api.getCommunications(type: 7, pageSize: 200);
      ...
      debugPrint('Load regulations error: $e');
    }
  }

  Future<void> _loadCategories() async {
    try {
      final cats = await _api.getContentCategories(contentType: 7);
      if (mounted) {
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Row(children: [
                    Container(
                      ...
                      child: const Icon(Icons.gavel, color: Color(0xFF1E3A5F), size: 22),
                    ),
  ...
  void _openDetail(Map<String, dynamic> article) {
    Navigator.push(context, MaterialPageRoute(
      builder: (_) => _ArticleViewPage(article: article, themeColor: const Color(0xFF1E3A5F)),
    ));
  }
...
// Article Detail View
// ═══════════════════════════════════════════════════════════════
class _ArticleViewPage extends StatelessWidget {
  final Map<String, dynamic> article;
  final Color themeColor;
  const _ArticleViewPage({required this.article, required this.themeColor});
