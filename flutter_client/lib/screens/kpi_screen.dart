...
class _KpiScreenState extends State<KpiScreen> with TickerProviderStateMixin {
  ...
  Map<String, dynamic>? get _currentPeriod {
    if (_selPeriodId == null || _periods.isEmpty) return null;
    for (final p in _periods) {
      if (p['id']?.toString() == _selPeriodId) return p;
    }
    return _periods.first;
  }

  int get _currentPeriodStatus =>
      _currentPeriod != null ? _periodStatusInt(_currentPeriod!['status']) : -1;

  bool get _isPeriodOpen => _currentPeriodStatus == 0;

  bool get _isPeriodLocked => _currentPeriodStatus == 1;

  bool get _isPeriodCalculated => _currentPeriodStatus == 2;

  bool get _hasPersistedSalaries => _filteredSalaries.isNotEmpty;

  bool _requireOpenPeriod({String? message}) {
    if (_isPeriodOpen) return true;
    NotificationOverlayManager().showWarning(
      title: 'Chu kỳ đã khóa',
      message: message ??
          'Chỉ được chỉnh sửa chỉ tiêu/doanh số khi chu kỳ đang ở trạng thái Mở.',
    );
    return false;
  }

  bool _requireLockedPeriod({String? message}) {
    if (_isPeriodLocked) return true;
    NotificationOverlayManager().showWarning(
      title: 'Chưa thể tính lương',
      message: message ??
          'Vui lòng khóa chu kỳ trước khi tính lương KPI.',
    );
    return false;
  }

  double _previewSalaryTotal(Map<String, dynamic> t) {
    ...
    final tierBonus = _calcTierBonuses(t).fold<double>(
        0, (s, b) => s + ((b['bonus'] as num?)?.toDouble() ?? 0));
    final penBonus = _calcPenaltyBonus(t);
    return math.max(0.0, salaryHT + penBonus + tierBonus);
  }

  Widget _salaryModeBadge(bool isPreview) {
    if (!isPreview) {
      return Container(
  ...
  Widget _buildStatusBadge(String label, int status, {bool compact = false}) {
    final color = _periodStatusColor(status);
    return Container(
  ...
  List<Map<String, dynamic>> get _filteredTargets {
    ...
    if (_filterEmployeeId != null) {
      list = list
          .where((t) => t['employeeId']?.toString() == _filterEmployeeId)
          .toList();
    }
    return list;
  }
  ...
  List<Map<String, dynamic>> get _filteredSalaries {
    ...
    return _salaries
        .where((s) => filteredEmpIds.contains(s['employeeId']?.toString()))
        .toList();
  }
  ...
  Widget _buildExportButtons({
    ...
    VoidCallback? onPng,
    String excelLabel = 'Xuất Excel',
  }) {
    final perm = Provider.of<PermissionProvider>(context, listen: false);
    final canExport = perm.canExport('KPI');
    return Wrap(
      spacing: 8,
      runSpacing: 8,
      children: [
  ...
  Future<void> _exportTargetsExcel() async {
    ...
    try {
      ...
      // Headers giờng Google Sheet: Mã NV, Tên NV, Tổng KPI, Chỉ tiêu
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        sheet
            ...
            .value = excel_lib.TextCellValue(empCode);
        sheet
            .cell(excel_lib.CellIndex.indexByColumnRow(
                columnIndex: 1, rowIndex: row))
            .value = excel_lib.TextCellValue(empName);
        sheet
            .cell(excel_lib.CellIndex.indexByColumnRow(
                columnIndex: 2, rowIndex: row))
            .value = excel_lib.DoubleCellValue(actual);
        sheet
            .cell(excel_lib.CellIndex.indexByColumnRow(
      ...
      for (int i = 0; i < targets.length; i++) {
        ...
        sheet
                .cell(excel_lib.CellIndex.indexByColumnRow(
                    columnIndex: 0, rowIndex: row))
                .value =
            excel_lib.TextCellValue(t['employeeCode']?.toString() ?? '');
        sheet
                .cell(excel_lib.CellIndex.indexByColumnRow(
                    columnIndex: 1, rowIndex: row))
                .value =
        ...
        sheet
            .cell(excel_lib.CellIndex.indexByColumnRow(
                columnIndex: 2, rowIndex: row))
            .value = excel_lib.DoubleCellValue(act);
        sheet
            .cell(excel_lib.CellIndex.indexByColumnRow(
                columnIndex: 3, rowIndex: row))
            .value = excel_lib.DoubleCellValue(tgt);
        sheet
            .cell(excel_lib.CellIndex.indexByColumnRow(
                columnIndex: 4, rowIndex: row))
            .value = excel_lib.DoubleCellValue(pct);
        sheet
            .cell(excel_lib.CellIndex.indexByColumnRow(
                columnIndex: 5, rowIndex: row))
            .value = excel_lib.DoubleCellValue(penaltyBonus);
        sheet
            .cell(excel_lib.CellIndex.indexByColumnRow(
                columnIndex: 6, rowIndex: row))
            .value = excel_lib.DoubleCellValue(salaryHT);
        sheet
            .cell(excel_lib.CellIndex.indexByColumnRow(
                ...
                children: [
                  LayoutBuilder(builder: (context, constraints) {
                    final avgPct = filteredTargets.isNotEmpty
                        ? filteredTargets
                                .map((t) => ((t['completionRate'] ?? 0) as num)
                                    .toDouble())
                                .reduce((a, b) => a + b) /
                            filteredTargets.length
                        : 0.0;
                    final totalBonus = _hasPersistedSalaries
                    ...
                    final cards = [
                      ...
                      _statCard('Tiến độ TB', '${avgPct.toStringAsFixed(1)}%',
                          Icons.analytics_outlined),
                      _statCard('Tổng lương', _cur.format(totalBonus),
                  ...
                  LayoutBuilder(builder: (context, constraints) {
                    if (constraints.maxWidth >= 900) {
                      return Row(
                        ...
                        children: [
                          Expanded(
                              flex: 3, child: _buildProgressOverview(theme)),
  ...
  Widget _buildTopPerformers(ThemeData theme) {
    final sorted = List<Map<String, dynamic>>.from(
        _filteredTargets.where((t) => t['actualValue'] != null));
    sorted.sort((a, b) => ((b['completionRate'] ?? 0) as num)
        .compareTo((a['completionRate'] ?? 0) as num));
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: i < 3
                        ? LinearGradient(colors: [
                            rankColors[i].withValues(alpha: 0.08),
                            Colors.white
                          ])
                        : null,
                  ...
                  child: Column(children: [
                    Row(children: [
                      ...
                      if (!isMobile) ...[
                        _actionIconBtn(Icons.phone, 'Gọi điện', _green,
                            () => _showActionConfirm(t, 'call')),
                        _actionIconBtn(Icons.description, 'Yêu cầu giải trình',
                            _amber, () => _showActionConfirm(t, 'explain')),
                        _actionIconBtn(Icons.event, 'Lên lịch họp', _blue,
                            () => _showActionConfirm(t, 'meeting')),
                        _actionIconBtn(Icons.assignment, 'Báo cáo', _accent,
                            () => _showDailyReportDialog(t)),
                      ],
                    ]),
                    if (isMobile)
                      Padding(
  ...
  void _showActionConfirm(Map<String, dynamic> target, String actionType) {
    ...
    switch (actionType) {
      case 'call':
        ...
        content =
            'Bạn muốn gọi điện cho nhân viên $name để trao đổi về tiến độ KPI?';
        icon = Icons.phone;
        color = _green;
        break;
      case 'explain':
        ...
        content =
            'Gửi yêu cầu giải trình cho $name về tiến độ chỉ tiêu hiện tại?';
          ...
          FilledButton.icon(
            onPressed: () {
              ...
              NotificationOverlayManager().showInfo(
                  title: 'Thông báo',
                  message: actionType == 'call'
                      ? 'Đã tạo nhắc nhở gọi điện $name'
                      : actionType == 'explain'
                          ? 'Đã gửi yêu cầu giải trình cho $name'
  ...
  // ------------------------------------------------
  //  TAB: CÀI ĐẶT (Chu kỳ + Google Sheet)
  // ------------------------------------------------
            ...
            child: ExpansionTile(
              ...
              children: [
                if (_selPeriodId == null)
                  _emptyState('Chọn chu kỳ ở header', Icons.calendar_today)
                else
                  _buildGSheetConfigBody(theme),
              ],
            ),
          ),
        ],
  ...
  Widget _buildPeriodsSection(ThemeData theme) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Align(
        ...
        if (_periods.isEmpty)
          ...
          ..._periods.map((p) => Responsive.isMobile(context)
              ? Padding(
  ...
  Widget _buildGSheetConfigBody(ThemeData theme) {
    ...
    return Column(
      ...
      children: [
        ...
        if (!_credentialsConfigured) const SizedBox(height: 16),
  ...
  // ------------------------------------------------
  //  TAB 2: CHU KỲ (legacy section helpers)
  // ------------------------------------------------
  ...
  Widget _buildPeriodDeckItem(Map<String, dynamic> p) {
    ...
    final start = DateTime.tryParse((p['periodStart'] ?? '').toString());
    final end = DateTime.tryParse((p['periodEnd'] ?? '').toString());
          ...
          Row(children: [
            if (status == 0 &&
                Provider.of<PermissionProvider>(context, listen: false)
                    .canApprove('KPI'))
              _chip('Khóa', Icons.lock_rounded, _amber,
                  () => _updatePeriodStatus(p['id'], 1)),
            if (status == 1 &&
                Provider.of<PermissionProvider>(context, listen: false)
                    .canApprove('KPI'))
            ...
            if (status == 1)
              _chip('Tính lương', Icons.calculate_rounded, _blue,
                  () => _calculateSalary(p['id']?.toString())),
            if (status == 2 &&
                Provider.of<PermissionProvider>(context, listen: false)
                    .canApprove('KPI'))
              _chip('Duyệt', Icons.verified_rounded, _green,
                  () => _updatePeriodStatus(p['id'], 3)),
            const Spacer(),
            if (!isCurrent)
              TextButton.icon(
                onPressed: () {
            ...
            if (Provider.of<PermissionProvider>(context, listen: false)
              ...
              IconButton(
                ...
                onPressed: () =>
                    _deleteItem(() => _api.deleteKpiPeriod(p['id'].toString())),
              ),
          ]),
        ]),
      ),
        ...
        Future<void> onSave() async {
          final data = {
            'name': nameCtrl.text.isNotEmpty
                ? nameCtrl.text
                : 'Tháng ${monthCtrl.text}/${yearCtrl.text}',
  ...
  Future<void> _calculateSalary(String? periodId) async {
    ...
    if (!_requireLockedPeriod()) return;
    ...
    if (mounted) {
      ...
      if (res['isSuccess'] == true) {
        ...
        NotificationOverlayManager().showError(
            title: 'Lỗi', message: res['message'] ?? 'Lỗi tính lương');
      }
    }
  }
  ...
  Widget _buildTargetsTab(ThemeData theme) {
    ...
    return Column(children: [
      ...headerSections,
      Expanded(child: _buildTargetsListBody(filtered)),
    ]);
  }
  ...
  Widget _buildTargetMiniStats(List<Map<String, dynamic>> filtered,
    ...
    return LayoutBuilder(builder: (context, constraints) {
      ...
      final items = [
        ...
        _miniStat(
            'Đạt ≥100%',
            '${filtered.where((t) => (t['completionRate'] ?? 0) >= 100).length}'),
        _miniStat(
            '70-99%',
            '${filtered.where((t) {
              final p = (t['completionRate'] ?? 0) as num;
              return p >= 70 && p < 100;
            }).length}'),
        _miniStat(
            '<70%',
            '${filtered.where((t) {
              final p = (t['completionRate'] ?? 0) as num;
              return t['actualValue'] != null && p < 70;
            }).length}'),
        _miniStat(
            'Chưa có DS',
            '${filtered.where((t) => t['actualValue'] == null).length}'),
      ];
      if (mobileLayout) {
        return Wrap(
          spacing: 6,
          ...
          children: items
              .map((c) => SizedBox(
                  width: (constraints.maxWidth - 6) / 2, child: c))
              .toList(),
        );
      }
      return Row(children: [
  ...
  List<Widget> _targetsTabMobileSlivers(List<Map<String, dynamic>> filtered) {
    if (filtered.isEmpty) {
      return [
        HrmScrollSlivers.fillRemaining(
          child: _emptyState('Chưa giao chỉ tiêu cho nhân viên nào',
    ...
    return [
      SliverToBoxAdapter(
        child: _buildTargetsListBody(filtered, shrinkWrap: true),
      ),
    ];
  }

  Widget _buildTargetsListBody(List<Map<String, dynamic>> filtered,
      {bool shrinkWrap = false}) {
    if (filtered.isEmpty) {
      return _emptyState('Chưa giao chỉ tiêu cho nhân viên nào',
          Icons.assignment_outlined);
    }
    return RepaintBoundary(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, i) => _buildTargetCard(filtered[i]),
      ),
    );
  }
  ...
  Widget _buildTiersEditor(
      List<Map<String, dynamic>> tiers,
      int rateType,
      int criteriaType,
      void Function(int) onRateTypeChanged,
    ...
    switch (rateType) {
      case 0:
        valueHeader = 'VNĐ/1 point vượt';
        hintText = 'VNĐ/point';
        example =
            'Ví dụ: Từ 100% → 120%, thưởng 50.000d cho mởi point vượt. Đến % nhập -1 hoặc 8 = không giới hạn.';
        break;
      case 1:
        valueHeader = '% trên doanh thu vượt';
        hintText = '% doanh thu';
        example =
            'Ví dụ: Từ 100% → 120%, thưởng 5% trên phần doanh thu vượt chỉ tiêu.';
        break;
      case 2:
        valueHeader = 'Giá trị VNĐ';
        hintText = 'Số tiền VNĐ';
        example = 'Ví dụ: Đạt ≥100%, thưởng cố định 2.000.000d.';
        break;
      case 3:
        valueHeader = '% lương hoàn thành';
        hintText = '%';
        example = 'Ví dụ: Đạt ≥120%, thưởng 10% lương hoàn thành.';
        break;
      default:
        valueHeader = 'Giá trị';
    ...
    if (!validValues.contains(rateType)) {
      WidgetsBinding.instance.addPostFrameCallback(
          (_) => onRateTypeChanged(dropdownItems.first.value!));
    }
      ...
      Row(children: [
        ...
        TextButton.icon(
          onPressed: () => ss(() => tiers.add({
                'fromPct': tiers.isEmpty
                    ? 100
                    : (tiers.last['toPct'] == -1 ? 120 : tiers.last['toPct']),
                'toPct': -1,
                'rate': 0,
                'rateType': rateType
              })),
              ...
              return Container(
                ...
                child: Row(children: [
                  Expanded(
                    ...
                    onChanged: (v) =>
                        tiers[i]['fromPct'] = num.tryParse(v) ?? 0,
                  )),
                  Expanded(
                      child: TextFormField(
                    initialValue:
                  ...
                  Expanded(
                      ...
                      child: TextFormField(
                        ...
                        onChanged: (v) =>
                            tiers[i]['rate'] = parseFormattedNumber(v) ?? 0,
                      )),
                  SizedBox(
                      width: 32,
                      child: IconButton(
  ...
  Widget _buildPenaltyTiersEditor(
      List<Map<String, dynamic>> tiers,
      int rateType,
      void Function(int) onRateTypeChanged,
      void Function(void Function()) ss) {
          ...
          if (tiers.isEmpty)
            ...
            ...tiers.asMap().entries.map((e) {
              ...
              return Container(
                ...
                decoration: BoxDecoration(
                    border:
                        Border(top: BorderSide(color: Colors.red.shade100))),
                child: Row(children: [
                  Expanded(
                      child: TextFormField(
                  ...
                  Expanded(
                    ...
                    keyboardType: TextInputType.number,
                    onChanged: (v) =>
                        tiers[i]['toPct'] = num.tryParse(v) ?? 100,
                  )),
                  Expanded(
                      flex: 2,
                      child: TextFormField(
  ...
  void _showAddTargetDialog() {
    ...
    final assignedIds =
        _targets.map((t) => t['employeeId']?.toString()).toSet();
    final available = _employees
        .where((e) => !assignedIds.contains(e['id']?.toString()))
        .toList();
    String searchText = '';
    final isMobile = Responsive.isMobile(context);
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, ss) {
        ...
        Future<void> onSave() async {
          final batch = selectedEmpIds
              .map((empId) => {
                    'employeeId': empId,
                    'criteriaType': criteriaType,
                    'targetValue': parseFormattedNumber(targetCtrl.text) ?? 0,
                    'completionSalary':
                        parseFormattedNumber(salaryCtrl.text) ?? 0,
        ...
        final formBody = Column(
            ...
            children: [
              ...
              Row(children: [
                Expanded(
                    child: _field(targetCtrl, 'Chỉ tiêu (s?)', number: true)),
                const SizedBox(width: 12),
                Expanded(
                    child:
              ...
              const SizedBox(height: 16),
              _buildPenaltyTiersEditor(
                  penaltyTiers,
                  penaltyRateType,
                  (v) => ss(() {
                        penaltyRateType = v;
                        for (final t in penaltyTiers) {
                          t['rateType'] = v;
                        }
                      }),
                  ss),
  ...
  void _showEditTargetDialog(Map<String, dynamic> t) {
    ...
    int penaltyRateType = penaltyTiers.isNotEmpty
        ? ((penaltyTiers.first['rateType'] ?? 1) as num).toInt()
        : 1;
    int bonusRateType =
        tiers.isNotEmpty ? ((tiers.first['rateType'] ?? 0) as num).toInt() : 0;
    final isMobile = Responsive.isMobile(context);
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, ss) {
        Future<void> onSave() async {
          final data = {
            ...
            'targetValue': parseFormattedNumber(targetCtrl.text) ?? 0,
            'actualValue': actualCtrl.text.isNotEmpty
                ? parseFormattedNumber(actualCtrl.text)
                : null,
        ...
        final formBody = Column(
            ...
            children: [
              ...
              _buildTiersEditor(
                  tiers,
                  bonusRateType,
                  ((t['criteriaType'] ?? 0) as num).toInt(),
                  (v) => ss(() {
                        bonusRateType = v;
                        for (final bt in tiers) {
                          bt['rateType'] = v;
                        }
                      }),
                  ss),
  ...
  /// Tìm sẽ bậc thưởng tối đa trong tất cả targets
  ...
  Widget _buildPersistedSalaryTab(ThemeData theme) {
    final rows = _filteredSalaries;
    final perm = Provider.of<PermissionProvider>(context, listen: false);
    final pendingCount =
        rows.where((s) => s['isApproved'] != true).length;
    final totalGross = rows.fold<double>(
        0, (s, r) => s + ((r['grossIncome'] ?? 0) as num).toDouble());
    final totalNet = rows.fold<double>(
        0, (s, r) => s + ((r['netIncome'] ?? 0) as num).toDouble());
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          _sectionLabel('Bảng lương đã tính',
              subtitle: 'Số liệu chính thức từ hệ thống'),
          Row(
            children: [
              _salaryModeBadge(false),
              const Spacer(),
              if (_isPeriodLocked)
                OutlinedButton.icon(
          ...
          HrmPageChrome.horizontalStatCards(
            cards: [
              _statCard('Nhân viên', '${rows.length}', Icons.people_outline),
              _statCard('Tổng gross', _cur.format(totalGross),
                  Icons.payments_outlined),
              _statCard('Thực nhận', _cur.format(totalNet),
                  Icons.account_balance_wallet_outlined),
              _statCard('Chờ duyệt', '$pendingCount',
                  Icons.pending_actions_outlined),
            ],
          ),
          const SizedBox(height: 16),
          if (rows.isEmpty)
            _emptyState('Chưa có bảng lương cho bộ lọc hiện tại',
                Icons.receipt_long)
          else if (Responsive.isMobile(context))
            ...rows.map((s) => _buildPersistedSalaryCard(s))
          else
            Container(
              width: double.infinity,
              decoration: BoxDecoration(
  ...
  Widget _buildPersistedSalaryCard(Map<String, dynamic> s) {
    final approved = s['isApproved'] == true;
    final score = ((s['totalKpiScore'] ?? 0) as num).toDouble();
    return Card(
      margin: const EdgeInsets.only(bottom: 10),
  ...
  Widget _buildSalaryTab(ThemeData theme) {
    ...
    if (_hasPersistedSalaries) {
      return _buildPersistedSalaryTab(theme);
    }

    final maxTiers = _maxTierCount;

    return LayoutBuilder(builder: (context, constraints) {
      final tableMinWidth = constraints.maxWidth > 0
      ...
      return SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          _sectionLabel(
            'Dự kiến lương',
            subtitle: _isPeriodLocked
                ? 'Nhấn "Tính lương KPI" để chốt số chính thức'
                : 'Khóa chu kỳ trước khi tính lương',
          ),
          Row(
            children: [
          ...
          RepaintBoundary(
            ...
            child: Builder(builder: (_) {
              ...
              final safePage =
                  _salaryPage.clamp(1, totalPages == 0 ? 1 : totalPages);
                        ...
                        child: ClipRRect(
                          ...
                          child: SingleChildScrollView(
                            ...
                            child: ConstrainedBox(
                              constraints:
                                  BoxConstraints(minWidth: tableMinWidth - 40),
                              child: DataTable(
                                columnSpacing: 18,
                                horizontalMargin: 16,
                                headingRowHeight: 52,
                                ...
                                rows: [
                                  ...
                                  ...pageItems.asMap().entries.map((entry) {
                                    ...
                                    final t = entry.value;
                                    final tgt = ((t['targetValue'] ?? 0) as num)
                                        .toDouble();
                                    final act = ((t['actualValue'] ?? 0) as num)
                                        .toDouble();
                                    final pct =
                                        ((t['completionRate'] ?? 0) as num)
                                            .toDouble();
                                    final completionSalary =
                                        ((t['completionSalary'] ?? 0) as num)
                                            .toDouble();
                                    final tierBonuses = _calcTierBonuses(t);
                                    final penaltyBonus = _calcPenaltyBonus(t);

                                    final salaryHT = pct >= 100
                                        ? completionSalary
                                        : (completionSalary * pct / 100);
                                    final totalTierBonus = tierBonuses.fold<
                                            double>(
                                        0,
                                        (s, b) =>
                                            s +
                                            ((b['bonus'] as num?)?.toDouble() ??
                                                0));
                                    final totalSalary = math.max(
                                        0.0,
                                        salaryHT +
                                            penaltyBonus +
                                            totalTierBonus);
                          ...
                          children: [
                            IconButton(
                                ...
                                onPressed: safePage > 1
                                    ? () => setState(() => _salaryPage = 1)
                                    : null,
                                visualDensity: VisualDensity.compact),
                            IconButton(
                                icon: const Icon(Icons.chevron_left, size: 20),
                                onPressed: safePage > 1
                                    ? () => setState(() => _salaryPage--)
                                    : null,
                                visualDensity: VisualDensity.compact),
                            Container(
                              padding: const EdgeInsets.symmetric(
                            ...
                            IconButton(
                                ...
                                onPressed: safePage < totalPages
                                    ? () =>
                                        setState(() => _salaryPage = totalPages)
                                    : null,
                                visualDensity: VisualDensity.compact),
                          ],
                        ),
  ...
  Widget _buildSalaryMobileCards(List<Map<String, dynamic>> pageItems,
      int startIdx, List<Map<String, dynamic>> allItems) {
    final totalSalaryAll = allItems.fold<double>(0, (s, t) {
      final pct = ((t['completionRate'] ?? 0) as num).toDouble();
      final cs = ((t['completionSalary'] ?? 0) as num).toDouble();
      final salaryHT = pct >= 100 ? cs : cs * pct / 100;
      final tierBonus = _calcTierBonuses(t).fold<double>(
          0, (s2, b) => s2 + ((b['bonus'] as num?)?.toDouble() ?? 0));
      final penBonus = _calcPenaltyBonus(t);
            ...
            child:
              ...
              Row(children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    gradient: LinearGradient(colors: [
                      pctColor.withValues(alpha: 0.15),
                      pctColor.withValues(alpha: 0.05)
                    ]),
                    borderRadius: BorderRadius.circular(10),
                  ),
                  child: Text(tr('${pct.toStringAsFixed(1)}%'),
  ...
  Future<bool> _approveAllSalaries() async {
    final ids = _filteredSalaries
    ...
    if (mounted) {
      NotificationOverlayManager().showError(
          title: 'Lỗi', message: res['message']?.toString() ?? 'Duyệt thất bại');
    }
    return false;
  }
  ...
  Widget _statCard(String label, String value, IconData icon, [Color? accent]) {
    ...
    return Container(
      ...
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          Icon(icon, size: 18, color: c.withValues(alpha: 0.85)),
  ...
  Widget _buildGSheetConnectionSection(
      ThemeData theme, Map<String, dynamic> period) {
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Wrap(spacing: 8, runSpacing: 8, children: [
          ...
          if (hasConfig) ...[
            ...
            OutlinedButton.icon(
              onPressed:
                  _creatingTemplate ? null : () => _createGSheetTemplate(),
  ...
  Widget _buildEmployeeMappingSection(ThemeData theme) {
    ...
    final configured = _targets
        .where((t) =>
            ...
            t['googleCellPosition'].toString().isNotEmpty)
        .toList();
    final unconfigured = _targets
        .where((t) =>
            t['googleCellPosition'] == null ||
            t['googleCellPosition'].toString().isEmpty)
        .toList();
    ...
    return Container(
      ...
      child: Column(children: [
        ...
        if (configured.isNotEmpty && unconfigured.isNotEmpty)
          Divider(height: 24, color: Colors.orange.shade100),
  ...
  Widget _buildAutoSyncSection(ThemeData theme, Map<String, dynamic> period) {
    ...
    if (timeSlotsJson != null && timeSlotsJson.isNotEmpty) {
      try {
        timeSlots = List<String>.from(json.decode(timeSlotsJson));
      } catch (_) {}
    }
        ...
        final formBody = Column(
            ...
            children: [
              ...
              isMobile
                  ? Column(children: [
                      availableSheets.isNotEmpty
                          ? DropdownButtonFormField<String>(
                              initialValue:
                                  availableSheets.contains(sheetNameCtrl.text)
                                      ? sheetNameCtrl.text
                                      : null,
                              decoration: InputDecoration(
                          ...
                          child: OutlinedButton.icon(
                            onPressed: testing
                                ...
                                : () async {
                                    ...
                                    final res =
                                        await _api.testKpiGSheetConnection(url);
                                    if (ctx.mounted) {
                                      ss(() {
                                        testing = false;
                                        if (res['isSuccess'] == true) {
                                          ...
                                          if (sheets is List) {
                                            ...
                                            if (availableSheets.isNotEmpty &&
                                                sheetNameCtrl.text.isEmpty) {
                                              sheetNameCtrl.text =
                                                  availableSheets.first;
                                            }
                                          }
                                        } else {
                                          testResult = {
                  ...
                  : Row(children: [
                      Expanded(
                        child: availableSheets.isNotEmpty
                            ? DropdownButtonFormField<String>(
                                initialValue:
                                    availableSheets.contains(sheetNameCtrl.text)
                                        ? sheetNameCtrl.text
  ...
  Future<void> _testGSheetConnection(String url) async {
    ...
    if (res['isSuccess'] == true) {
      ...
      NotificationOverlayManager()
          .showError(title: 'Lỗi', message: res['message'] ?? 'Lỗi');
    }
  }
  ...
  Future<void> _createGSheetTemplate() async {
    ...
    try {
      ...
      if (res['isSuccess'] == true) {
        ...
        NotificationOverlayManager().showError(
            title: 'Lỗi', message: res['message'] ?? 'Lỗi tạo sheet mẫu');
      }
    } catch (e) {
      if (!mounted) return;
  ...
  Future<void> _uploadCredentials() async {
    ...
    final res =
        await _api.uploadGSheetCredentials(file.bytes!.toList(), file.name);
    if (!mounted) return;
    ...
    if (res['isSuccess'] == true) {
      ...
      if (data?['configured'] == true) {
        ...
        NotificationOverlayManager().showError(
            title: 'Lỗi',
            message: data?['message'] ?? res['message'] ?? 'Lỗi upload');
      }
    } else {
      NotificationOverlayManager()
          .showError(title: 'Lỗi', message: res['message'] ?? 'Lỗi upload');
    }
  }
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, ss) {
        Future<void> onSave() async {
          ...
          final res =
              await _api.copyKpiGSheetConfig(_selPeriodId!, selectedSourceId!);
          if (ctx.mounted) Navigator.pop(ctx);
  ...
  void _showBatchEditCellDialog() {
    ...
    Future<void> onSave(BuildContext ctx) async {
      final mappings = controllers.entries
          .map((e) => {
                'employeeId': e.key,
        ...
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              final data = {
                'googleSheetUrl':
                    _buildGSheetUrl(period['googleSpreadsheetId']),
                'googleSheetName': period['googleSheetName'] ?? '',
                'autoSyncEnabled': period['autoSyncEnabled'] == true,
                'autoSyncTimeSlots': period['autoSyncTimeSlots'],
                'employeeMappings': [
                  {'employeeId': empId, 'cellPosition': cellCtrl.text.trim()}
                ],
              };
                ...
                actions: [
                  ...
                  FilledButton(
                    onPressed: () async {
                      ...
                      final res =
                          await _api.saveKpiGSheetConfig(_selPeriodId!, data);
                      if (res['isSuccess'] == true && mounted) _loadData();
                    },
                    style: FilledButton.styleFrom(backgroundColor: _accent),
                    child: Text(tr('Thêm')),
  ...
  Future<void> _importExcelActuals() async {
    ...
    try {
      ...
      // Parse rows: Col A = Mã NV, Col C = Tổng KPI (doanh sẽ thực tế)
      ...
      for (int i = 1; i < table.rows.length; i++) {
        ...
        final actual =
            double.tryParse(actualStr.replaceAll(RegExp(r'[^0-9.-]'), '')) ?? 0;
      ...
      // Gửi dữ liệu lên API
      final res =
          await _api.importKpiExcelActuals(_selPeriodId!, data: importData);
      if (mounted) {
        if (res['isSuccess'] == true) {
  ...
  Future<void> _downloadExcelTemplate() async {
    try {
      final bytes =
          await _api.downloadKpiExcelTemplate(_selPeriodId!, _targets);
      if (bytes != null) {
