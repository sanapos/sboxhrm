...
class _ShiftRegistrationScreenState extends State<ShiftRegistrationScreen> {
  ...
  List<dynamic> get _activeEmployees {
    ...
    return _filteredEmployeesByBranch.where((e) {
  ...
  List<Map<String, dynamic>> _getEmployeesForShiftDay(
      dynamic shift, DateTime date) {
    final shiftId = shift['id']?.toString();
    final dateStr = DateFormat('yyyy-MM-dd').format(date);
    return _workSchedules.where((ws) {
      return ws['shiftId']?.toString() == shiftId &&
          ws['date']?.toString().substring(0, 10) == dateStr;
    }).toList();
  }

  List<Map<String, dynamic>> _getShiftsForEmployeeDay(
      dynamic employee, DateTime date) {
    final empId = employee['userId']?.toString() ?? employee['id']?.toString();
    final dateStr = DateFormat('yyyy-MM-dd').format(date);
    return _workSchedules.where((ws) {
      return ws['employeeUserId']?.toString() == empId &&
          ws['date']?.toString().substring(0, 10) == dateStr;
    }).toList();
  }
  ...
  List<Map<String, dynamic>> _getPendingForShiftDay(
    ...
    return _pendingRegistrations.where((r) {
      return r['shiftId']?.toString() == shiftId &&
          r['date']?.toString().substring(0, 10) == dateStr &&
  ...
  Widget build(BuildContext context) {
    final pendingCount = _pendingRegistrations
  ...
  Widget _buildWeekNavigator() {
    ...
    final label =
        '${DateFormat('dd/MM').format(_weekStart)} - ${DateFormat('dd/MM/yyyy').format(endDate)}';
    return Container(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildSectionTitle(Icons.work_history, 'Theo ca làm việc',
              'Ca cố định → chọn nhân viên theo thứ', const Color(0xFF0891B2)),
          const SizedBox(height: 8),
  ...
  Widget _buildShiftTable(
      List<String> dayNames, List<DateTime> days, String todayStr) {
    return SingleChildScrollView(
  ...
  TableRow _shiftRow(dynamic shift, List<DateTime> days, String todayStr) {
    ...
    return TableRow(children: [
      ...
      for (final day in days)
        _shiftDayCell(
            shift, day, DateFormat('yyyy-MM-dd').format(day) == todayStr),
    ]);
  }
    ...
    return TableCell(
      child: InkWell(
        ...
        child: Container(
          ...
          decoration:
              BoxDecoration(color: isToday ? const Color(0xFFF0FDFA) : null),
          child:
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          child: Row(
            ...
            children: [
              IconButton(
                  ...
                  onPressed:
                      safePage > 1 ? () => setState(() => _empPage = 1) : null),
              IconButton(
                  icon: const Icon(Icons.chevron_left),
                  onPressed:
  ...
  TableRow _employeeRow(dynamic emp, List<DateTime> days, String todayStr) {
    ...
    return TableRow(children: [
      ...
      for (final day in days)
        _employeeDayCell(
            emp, day, DateFormat('yyyy-MM-dd').format(day) == todayStr),
    ]);
  }
    ...
    return TableCell(
      child: InkWell(
        ...
        child: Container(
          ...
          padding: const EdgeInsets.all(4),
          decoration:
              BoxDecoration(color: isToday ? const Color(0xFFFAF5FF) : null),
          child:
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...shifts.map((ws) {
  ...
  Widget _dayHeaderCell(
      String dayName, DateTime date, bool isToday, Color baseColor) {
    return TableCell(
        child: Container(
  ...
  void _showAssignEmployeeDialog(dynamic shift, DateTime date) {
    ...
    List<dynamic> filtered = List.from(_filteredEmployeesByBranch);
    final assignedIds = _getEmployeesForShiftDay(shift, date)
        .map((ws) => ws['employeeUserId']?.toString())
        .toSet();
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setDialogState) {
        void filter() {
          ...
          setDialogState(() {
            filtered = _filteredEmployeesByBranch.where((e) {
              final n =
                  (e['fullName'] ?? e['name'] ?? '').toString().toLowerCase();
              final c = (e['employeeCode'] ?? e['code'] ?? '')
                  .toString()
                  .toLowerCase();
                    ...
                    child: filtered.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (ctx, i) {
                              ...
                              final empId = emp['userId']?.toString() ??
                                  emp['id']?.toString() ??
                                  '';
  ...
  void _showAssignShiftDialog(dynamic emp, DateTime date) {
    ...
    final assignedShiftIds = _getShiftsForEmployeeDay(emp, date)
        .map((ws) => ws['shiftId']?.toString())
        .toSet();
                  ...
                  itemBuilder: (ctx, i) {
                    ...
                    return ListTile(
                      ...
                      onTap: done
                          ...
                          : () async {
                              Navigator.pop(ctx);
                              await _assignEmployeeToShift(empId, sId, date);
                            },
                    );
                  },
                ),
  ...
  Future<void> _assignEmployeeToShift(
  ...
  Future<void> _approveRegistration(String? id) async {
    ...
    try {
      final result = await _apiService
          .approveScheduleRegistration(id, {'isApproved': true});
      if (mounted) {
        if (result['isSuccess'] == true) {
          NotificationOverlayManager()
  ...
  Future<void> _rejectRegistration(String? id) async {
    ...
    try {
      final result = await _apiService.approveScheduleRegistration(
          id, {'isApproved': false, 'rejectionReason': reasonCtrl.text});
      if (mounted) {
        if (result['isSuccess'] == true) {
          NotificationOverlayManager()
