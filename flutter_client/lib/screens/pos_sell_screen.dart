...
import '../utils/pos_product_quick_note_memory.dart';
...
import '../widgets/pos/pos_online_orders_toolbar_button.dart';
...
/// Mật độ bán hàng chuẩn cảm ứng (≥44px hit, chống tràn chữ).
abstract final class _KiotLayout {
  static const topBarHeight = 52.0;
  static const bottomBarHeight = 48.0;
  static const cartRowHeight = 88.0;
  static const cartHeaderHeight = 36.0;
  static const sidePadding = 12.0;
  static const sectionGap = 12.0;
  /// Min tap target — Material / Apple HIG ~44–48.
  static const topBarActionTap = BoxConstraints(minWidth: 48, minHeight: 48);
  static const topBarActionGap = SizedBox(width: 8);
  static const tapMin = 48.0;
  static const qtyHit = 40.0;
  ...
  static const compactSectionGap = 10.0;

  static const wDel = 40.0;
  static const wNameMin = 140.0;
  static const wQty = 120.0;
  static const wUnit = 68.0;
  static const wPrice = 80.0;
  static const wTotal = 96.0;

  static double get tableMinWidth =>
      wDel + wNameMin + wQty + wUnit + wPrice + wTotal + 24;
}
...
class _PosPaymentSource {
  ...
  factory _PosPaymentSource.fromTingee() {
    return const _PosPaymentSource(
      key: 'tingee',
      label: 'Tingee QR',
      methodLabel: 'Tingee',
...
class _SellCartLine {
  ...
  double get toppingExtraPerUnit =>
      toppings.fold<double>(0, (s, t) => s + t.price * t.qty);
  ...
  String? get noteWithToppings {
    final block = posToppingNoteBlock(
      toppings: toppings.map((t) => (name: t.name, qty: t.qty, price: t.price)),
      lineNote: lineNote,
    );
...
class _CartTopping {
  ...
  static _CartTopping? tryParse(dynamic raw) {
    ...
    final qtyRaw = m['qty'] ?? m['Qty'] ?? m['quantity'];
    final parsedQty = qtyRaw is num
        ? qtyRaw.toInt()
...
/// Nhãn POS khi không chọn CRM. XML HĐĐT vẫn dùng [kPosWalkInInvoiceName].
const kPosWalkInCustomerLabel = 'Người tiêu dùng';
const kPosWalkInInvoiceName = 'Bán cho người tiêu dùng';

bool _isWalkInCustomerQuery(String q) {
  final t = q.trim();
  return t.isEmpty || t == kPosWalkInCustomerLabel || t == kPosWalkInInvoiceName;
}
...
class _SellInvoiceTab {
  ...
  double paidAmount = 0;
  /// Cọc đặt chỗ đã thu quỹ — trừ công nợ, không đưa vào dòng TT mới.
  double reservationDepositApplied = 0;
  ...
  /// Tăng mỗi lần user sửa giỏ — hydrate/GET chậm không được đè món vừa chọn.
  int cartEpoch = 0;
  ...
  /// Đơn tách bill từ bàn — thanh toán không đóng phiên bàn gốc.
  String? splitFromOrderId;
  /// Kênh bán gốc từ server (vd. QR online) — không ghi đè khi lưu draft.
  ...
  final _surchargeCtrl = TextEditingController(text: tr('0'));
  final _deliveryFeeCtrl = TextEditingController(text: tr('0'));
  final _paidCtrl = TextEditingController();
  final _customerSearchCtrl = TextEditingController(text: kPosWalkInCustomerLabel);
  ...
  void reset({
    ...
    _surchargeCtrl.text = '0';
    _deliveryFeeCtrl.text = '0';
    _paidCtrl.clear();
    _customerSearchCtrl.text = kPosWalkInCustomerLabel;
    _deliveryAddressCtrl.clear();
    _deliveryPhoneCtrl.clear();
    _deliveryPartnerCtrl.clear();
    _deliveryProvinceCtrl.clear();
    _deliveryDistrictCtrl.clear();
    _deliveryWardCtrl.clear();
    if (store != null) applyFeeDefaults(store);
  }
  ...
  void applyFeeDefaults(PosSellStoreSettings store) {
    if (store.enableSurcharge && store.surchargeDefault > 0) {
      surchargeIsPercent = store.surchargeIsPercent;
      surchargeInput = store.surchargeDefault;
      _surchargeCtrl.text =
          PosSellStoreSettings.formatAmount(store.surchargeDefault);
      applySurcharge(0);
    }
    if (store.enableDeliveryFee && store.deliveryFeeDefault > 0) {
      deliveryFee = store.deliveryFeeDefault;
      _deliveryFeeCtrl.text =
          PosSellStoreSettings.formatAmount(store.deliveryFeeDefault);
    }
  }
}
...
class _PosSellScreenState extends State<PosSellScreen>
  ...
  final _cartNoteEditorKeys = <int, GlobalKey>{};
  Timer? _customerSearchDebounce;
  Timer? _draftSyncDebounce;
  final _productGridKey = GlobalKey<PosSellProductGridState>();
  final _floorPaneKey = GlobalKey<PosResourceFloorScreenState>();
  /// Chip ghi chú đã học theo productId (máy này) — gộp với catalog.
  final Map<String, List<String>> _learnedQuickNotes = {};
  /// Giữ state sơ đồ / thực đơn khi đổi tab (không dispose → không reload API).
  bool _floorPaneKept = true;
  bool _catalogPaneKept = false;
  /// Cùng instance widget → parent setState không rebuild lưới/sơ đồ (A6/A7).
  Widget? _cachedDesktopProductPane;
  String? _cachedProductPaneSig;
  Widget? _cachedDesktopFloorPane;
  String? _cachedFloorPaneSig;
  ...
  PosEInvoiceSettings _eInvoiceSettings = const PosEInvoiceSettings();
  PosSellStoreSettings _storeSettings = const PosSellStoreSettings();
  bool _appliedFeeDefaultsOnce = false;
  ...
  List<MapEntry<String, String>> _enabledShippingCarriers = const [];
  bool _splitBillBusy = false;
  bool _parking = false;
  ...
  /// Bàn máy này vừa TT / trả trống — sơ đồ tô Free ngay.
  Set<String> _floorFreedResourceIds = {};
  Timer? _floorQuietRefreshTimer;
  ...
  Map<String, dynamic>? _paymentGatewaySettings;
  ...
  void _toggleCartRowExpand(int rowId, _CartRowExpand mode) {
    ...
    setState(() {
      ...
      if (mode == _CartRowExpand.note) {
        ...
        unawaited(_ensureLearnedNotes(line));
      }
    });
  ...
  List<String> _quickNotesFor(_SellCartLine line) => mergePosQuickNotes(
        line.product.saleQuickNotes,
        _learnedQuickNotes[line.product.id] ?? const [],
      );

  Future<void> _ensureLearnedNotes(_SellCartLine line) async {
    final id = line.product.id;
    if (id.isEmpty || _learnedQuickNotes.containsKey(id)) return;
    final loaded =
        await PosProductQuickNoteMemory.load(_storeId?.trim() ?? '', id);
    if (!mounted) return;
    _learnedQuickNotes[id] = loaded;
    _initLineNoteSelection(line);
    setState(() {});
  }

  Future<void> _learnQuickNote(_SellCartLine line, String note) async {
    final t = note.trim();
    if (t.isEmpty) return;
    final merged = mergePosQuickNotes(line.product.saleQuickNotes, [t]);
    line.product = line.product.copyWith(saleQuickNotes: merged);
    _learnedQuickNotes[line.product.id] = mergePosQuickNotes(
      _learnedQuickNotes[line.product.id] ?? const [],
      [t],
    );
    for (final other in _tab.cart) {
      if (other.product.id == line.product.id) {
    ...
    await PosProductQuickNoteMemory.remember(
      _storeId?.trim() ?? '',
      line.product.id,
      t,
    );
  ...
  void _initLineNoteSelection(_SellCartLine line) {
    final split = splitPosLineNote(line.lineNote, _quickNotesFor(line));
  ...
  void _commitLineNote(_SellCartLine line, {bool scheduleSave = true}) {
    if (!_guardReadOnlyEdit()) return;
    final before = _kitchenLineSignature(line);
    _applyLineNoteFromPicker(line);
    _renotifyKitchenOnLineChange(line, before);
    if (scheduleSave) _scheduleDraftAutosave();
  }

  /// Phần nội dung bếp cần biết của 1 dòng: ghi chú + topping.
  String _kitchenLineSignature(_SellCartLine line) =>
      '${(line.lineNote ?? '').trim()}|'
      '${line.toppings.map((t) => '${t.id}x${t.safeQty}').join(',')}';

  /// Số lượng chờ báo bếp chỉ tính theo SL, nên sửa ghi chú / topping của món
  /// đã báo là bếp không bao giờ biết: bấm Báo bếp chỉ báo «không có thay
  /// đổi». Đổi thì trả dòng về chưa báo để phiếu mới ra kèm nội dung đúng.
  void _renotifyKitchenOnLineChange(_SellCartLine line, String signatureBefore) {
    if (!_isTableOrderMode) return;
    if (line.kitchenSentQty <= 0) return;
    if (_kitchenLineSignature(line) == signatureBefore) return;
    line.kitchenSentQty = 0;
    NotificationOverlayManager().showWarning(
      title: 'Món đã đổi',
      message: tr(
        '${line.product.name}: bấm Báo bếp để gửi lại ghi chú/topping cho bếp',
      ),
    );
  }
  ...
  bool _tingeeEventMatchesTab(_SellInvoiceTab tab, Map<String, dynamic> event) {
    final eventOrderId =
        (event['orderId'] ?? event['OrderId'] ?? '').toString().trim();
    final eventOrderNo =
        (event['orderNo'] ?? event['OrderNo'] ?? '').toString().trim();
    final eventExternal = (event['externalOrderId'] ??
            event['ExternalOrderId'] ??
            '')
        .toString()
        .trim();
    final draftId = (tab.draftOrderId ?? '').toString().trim();
    final draftNo = (tab.draftOrderNo ?? '').trim();
    final extId = _tingeeExternalIdOf(tab);

    if (eventOrderId.isNotEmpty &&
        draftId.isNotEmpty &&
        eventOrderId.toLowerCase() == draftId.toLowerCase()) {
      return true;
    }
    if (eventOrderNo.isNotEmpty) {
      if (draftNo.isNotEmpty && eventOrderNo == draftNo) return true;
      if (extId.isNotEmpty && eventOrderNo == extId) return true;
    }
    if (eventExternal.isNotEmpty && extId.isNotEmpty && eventExternal == extId) {
      return true;
    }
    final eventTable =
        (event['tableName'] ?? event['TableName'] ?? '').toString().trim();
    if (eventTable.isNotEmpty) {
      final cur = formatPosTableLabel(
        areaName: tab.serviceAreaName,
        tableName: tab.serviceResourceName,
      );
      if (cur.isNotEmpty && eventTable == cur) return true;
    }
    return false;
  }

  bool _tingeeEventMatchesCurrentTab(Map<String, dynamic> event) =>
      _tingeeEventMatchesTab(_tab, event);

  String _tingeeExternalIdOf(_SellInvoiceTab tab) {
    final no = (tab.draftOrderNo ?? '').trim();
    if (no.isNotEmpty) return no;
    return (tab.tingeeTransferExternalId ?? '').trim();
  }

  bool _tingeeBelongsToOtherTab(Map<String, dynamic> event) {
    for (final t in _tabs) {
      if (identical(t, _tab)) continue;
      if (_tingeeEventMatchesTab(t, event)) return true;
    }
    return false;
  }

  bool _isWaitingTingeePayment() {
    if (_tab.cart.isEmpty) return false;
    if (_hasTingeePaymentSelected()) return true;
    return _shouldShowPaymentQrOverlay() &&
        (_resolveTransferPaymentQr()?.isTingee ?? false);
  }

  bool _tingeeEventHasOrderIdentity(Map<String, dynamic> event) {
    String pick(dynamic a, dynamic b) => (a ?? b ?? '').toString().trim();
    return pick(event['orderId'], event['OrderId']).isNotEmpty ||
        pick(event['orderNo'], event['OrderNo']).isNotEmpty ||
        pick(event['externalOrderId'], event['ExternalOrderId']).isNotEmpty;
  }

  bool _canAutoCheckoutOnTingee(Map<String, dynamic> event) {
    if (_checkingOut || _parking || _tab.cart.isEmpty) return false;
    if (!PosPaymentGatewayApi.isTingeeEnabled(_paymentGatewaySettings)) {
      return false;
    }
    if (_tingeeEventMatchesCurrentTab(event)) return true;
    if (!_isWaitingTingeePayment() || _tingeeBelongsToOtherTab(event)) {
      return false;
    }
    // Đơn bàn/online khác: server tự hoàn tất — không checkout giỏ hiện tại.
    if (_tingeeEventHasOrderIdentity(event)) return false;
    return true;
  }


  void _ensureTingeePaymentLine() {
    if (!_hasTingeePaymentSelected()) {
      final due = _grandTotal;
      if (_tab.paymentLines.isEmpty) {
        _tab.paymentLines.add(_SellPaymentLine(sourceKey: 'tingee'));
      } else {
        _tab.paymentLines.first.sourceKey = 'tingee';
      }
      final first = _tab.paymentLines.first;
      if (first.amount <= 0 && due > 0) {
        first.amount = due;
        first.amountCtrl.text = due > 0 ? _moneyFmt.format(due) : '';
      }
    } else {
      for (final pay in _tab.paymentLines) {
        if (!_isTingeeSourceKey(pay.sourceKey)) continue;
        if (pay.amount <= 0 && _grandTotal > 0) {
          pay.amount = _grandTotal;
          pay.amountCtrl.text = _moneyFmt.format(_grandTotal);
        }
      }
    }
    _syncPaidAmount();
  ...
  void _onTingeePaymentConfirmed(Map<String, dynamic> event) {
    ...
    if (_canAutoCheckoutOnTingee(event)) {
      _completeTingeePaidOrder(
        message.isNotEmpty
            ? message
            : tr('Đang hoàn tất đơn và in hóa đơn…'),
      );
      return;
    }
    if (_isWaitingTingeePayment()) {
      setState(() {
    ...
    NotificationOverlayManager().show(
      title: tr('Đã nhận chuyển khoản'),
      message: message.isNotEmpty
          ? message
          : tr('Xem tab Xác nhận CK nếu đơn đang mở máy khác'),
  ...
  void initState() {
    ...
    void scrollSearch() {
      ...
      final ctx = _productSearchFocus.context;
      if (ctx != null) posScrollIntoViewAboveIme(ctx);
    }
    _productSearchFocus.addListener(scrollSearch);
    _floorSearchFocus.addListener(() {
      if (!_floorSearchFocus.hasFocus) return;
      final ctx = _floorSearchFocus.context;
      if (ctx != null) posScrollIntoViewAboveIme(ctx);
    });
    _customerSearchFocus.addListener(() {
      if (!_customerSearchFocus.hasFocus) return;
      final ctx = _customerSearchFocus.context;
      if (ctx != null) posScrollIntoViewAboveIme(ctx);
    });
    ...
    _qrOnlineSignalSub = SignalRService().onPosFloorChanged.listen((event) {
      ...
      if (reason == 'qronlineorder' || reason == 'qronlinestatus') {
        if (_qrOnlineFeatureEnabled && mounted) {
    ...
    PosPaymentGatewayListener.instance
        .addListener(_onTingeePaymentConfirmed);
  ...
  void _onPendingQrOnlineOrdersChanged() {
    _maybeOpenPendingQrOnlineOrders();
  }

  void _onPendingQrOnlineDraftChanged() {
    unawaited(_maybeOpenPendingQrOnlineDraft());
  }

  Future<void> _maybeOpenPendingQrOnlineDraft() async {
    final id = NavigationNotifier.pendingOpenQrOnlineDraftId.value;
    if (id == null || id.isEmpty || !_sellReady || !mounted) return;
    NavigationNotifier.pendingOpenQrOnlineDraftId.value = null;
    final openPay = NavigationNotifier.pendingOpenQrOnlinePay.value;
    NavigationNotifier.pendingOpenQrOnlinePay.value = false;
    setState(() {
    ...
    await _openDraftOrder(id, silent: true, forceClaim: true);
    if (!mounted) return;
    final orderJson = await _fetchDraftOrderJson(id);
    if (orderJson != null) {
      await _ensureOnlineOrderCustomer(PosSaleOrder.fromJson(orderJson));
    }
    if (!mounted) return;
    if (_tab.draftOrderId == id && openPay) {
      setState(() => _tabletPaymentStage = _tab.cart.isNotEmpty);
    }
  }
  ...
  Future<void> _bootstrapSellScreen() async {
    ...
    // Catalog decode song song lúc chờ industry — lần mở lưới/sơ đồ không đụng disk lại.
    final earlyStoreId = _storeId?.trim();
    if (earlyStoreId != null && earlyStoreId.isNotEmpty) {
      unawaited(PosSellCatalogCache.instance.warmup(earlyStoreId));
    }

    unawaited(() async {
      await PosPrintOrchestrator.instance.refreshConfig();
      await _refreshLabelPrinterReady();
    }());
    unawaited(PosProductPrinterService.instance.preload());

    final storeFut = PosSellStoreSettings.load(peekServer: false);
    ...
    final eInvFut = _api.getPosEInvoiceSettings();
    final gatewayFut = _api.getPosPaymentGatewaySettings();
    final shippingFut = _api.getPosShippingEnabled();

    var store = await storeFut;
    ...
    store = store.withServerTax(industry?.extraJson);
    store = store.withServerReceiptStore(industry?.extraJson);
    store = store.withServerFees(industry?.extraJson);
    store = store.withServerVietQr(industry?.extraJson);
    if (!PosSellStoreSettings.hasServerTax(industry?.extraJson)) {
      store.persistSellTax(_api);
    }
    ...
    setState(() {
      ...
      _sellReady = true;
      if (_useFloorAsPrimary) _catalogPaneKept = true;
    });
    _scheduleCatalogPrefetch();
    final warmupStoreId = _storeId?.trim();
    if (warmupStoreId != null &&
        warmupStoreId.isNotEmpty &&
        warmupStoreId != earlyStoreId) {
      unawaited(PosSellCatalogCache.instance.warmup(warmupStoreId));
    }
    ...
    final eInvRes = await eInvFut;
    final gatewayRes = await gatewayFut;
    final shippingRes = await shippingFut;
    if (!mounted) return;

    final shippingCarriers = _parseEnabledShippingCarriers(shippingRes);
    ...
    Map<String, dynamic>? gatewaySettings;
    if (gatewayRes['isSuccess'] == true && gatewayRes['data'] is Map) {
      gatewaySettings =
          Map<String, dynamic>.from(gatewayRes['data'] as Map);
    }
    if (PosPaymentGatewayApi.isTingeeEnabled(gatewaySettings)) {
      var refresh = await _api.getPosBankAccounts();
      if (refresh['isSuccess'] != true) {
        refresh = await _api.getBankAccounts();
      }
      if (refresh['isSuccess'] == true && refresh['data'] is List) {
        accounts.clear();
        for (final raw in refresh['data'] as List) {
          if (raw is! Map) continue;
          final account =
              BankAccount.fromJson(Map<String, dynamic>.from(raw));
          if (account.isActive) accounts.add(account);
        }
      }
    }
    ...
    if (PosPaymentGatewayApi.isTingeeEnabled(gatewaySettings)) {
      sources.add(_PosPaymentSource.fromTingee());
    }
    ...
    setState(() {
      ...
      _thermalPrintSettings = thermal;
      _industrySettings = industry;
      if (eInvRes['isSuccess'] == true && eInvRes['data'] is Map) {
        _eInvoiceSettings = PosEInvoiceSettings.fromJson(
            Map<String, dynamic>.from(eInvRes['data'] as Map));
        _quickIssueEInvoice = _eInvoiceSettings.defaultIssueAtCheckout;
      }
      _sellMode = sellMode;
      ...
      _paymentGatewaySettings = gatewaySettings;
      _priceLists = priceLists;
      _enabledShippingCarriers = _orderShippingCarriers(
        shippingCarriers,
        preferAhamoveFirst: industry?.sellProfile.usesFloorPlan == true,
      );
      for (final t in _tabs) {
        t.vatRate = store.defaultVatRate;
      ...
      if (!_appliedFeeDefaultsOnce) {
        for (final t in _tabs) {
          if (t.cart.isEmpty && t.draftOrderId == null) {
            t.applyFeeDefaults(store);
          }
        }
        _appliedFeeDefaultsOnce = true;
      }
      _syncPaidAmount();
    });

    unawaited(PosPrintConfigSession.instance.warmUpIfStale(
    ...
    _scheduleCatalogPrefetch();
    _maybeOpenPendingQrOnlineOrders();
    _maybeOpenPendingQrOnlineDraft();
  ...
  Future<void> _bootstrapQrOnlineToolbar() async {
    ...
    setState(() => _qrOnlineFeatureEnabled = enabled);
    if (enabled) await _refreshQrOnlinePending();
  }

  Future<void> _refreshQrOnlinePending() async {
    final res = await _api.getPosQrOnlineOrders(status: 'pending');
    if (!mounted) return;
    if (res['isSuccess'] != true || res['data'] is! Map) return;
    final data = Map<String, dynamic>.from(res['data'] as Map);
    final raw = data['orders'] ?? data['Orders'];
    final list = <PosOnlineToolbarOrder>[];
    if (raw is List) {
      for (final e in raw) {
        if (e is! Map) continue;
        final j = Map<String, dynamic>.from(e);
        list.add(PosOnlineToolbarOrder(
          ...
          total: (j['total'] as num?)?.toDouble() ??
              ...
              0,
          statusLabel: (j['statusLabel'] ?? j['StatusLabel'] ?? tr('Chờ xác nhận'))
              .toString(),
        ));
      }
    }
    setState(() => _qrOnlinePending = list);
  }

  Future<void> _openQrOnlineOrdersScreen({String? highlightId}) async {
    final id = highlightId?.trim();
    if (id != null && id.isNotEmpty) {
      NavigationNotifier.notificationHighlightId.value = id;
    }
    await Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => PosHubScope(
          embeddedInHub: PosHubScope.of(context),
          pushedSubPage: true,
          child: PosQrOnlineOrdersScreen(highlightOrderId: id),
        ),
      ),
    );
    if (mounted && _qrOnlineFeatureEnabled) {
      await _refreshQrOnlinePending();
    }
  }

  Widget? _buildOnlineOrdersToolbarButton({
    ...
    return PosOnlineOrdersToolbarButton(
      ...
      iconColor: iconColor,
      labeled: labeled,
      onOpenAll: () => unawaited(_openQrOnlineOrdersScreen()),
      onOpenOrder: (o) =>
          unawaited(_openQrOnlineOrdersScreen(highlightId: o.id)),
    );
  }

  void _maybeOpenPendingQrOnlineOrders() {
    if (!NavigationNotifier.pendingOpenQrOnlineOrders.value) return;
    if (PosHubScope.of(context)) return;
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      if (!mounted) return;
      await _openQrOnlineOrdersScreen(highlightId: highlight);
    });
  }
  ...
  Future<void> _bootstrapCustomerDisplay() async {
    ...
    if (mounted) {
      setState(() => _hasSecondaryCustomerDisplay = hasSecondary);
    } else {
      _hasSecondaryCustomerDisplay = hasSecondary;
    }
    if (!CustomerDisplaySync.instance.enabled && !hasSecondary) return;
    ...
    if (hasSecondary && CustomerDisplaySync.instance.config.autoOpenOnPos) {
  ...
  Future<void> _loadPaymentSources() async {
    final gatewayRes = await _api.getPosPaymentGatewaySettings();
  ...
  void dispose() {
    ...
    NavigationNotifier.pendingOpenQrOnlineOrders
        .removeListener(_onPendingQrOnlineOrdersChanged);
    NavigationNotifier.pendingOpenQrOnlineDraftId
    ...
    _floorQuietRefreshTimer?.cancel();
  ...
  /// Nút Back Android = cùng logic nút quay về trên UI.
  Future<bool> _onSystemBack() async {
    ...
    // Khớp nút «Về SBOX HRM» / trang chủ trên thanh bán hàng.
    _goMobileSellHome();
    return true;
  }
    ...
    if (event.logicalKey == LogicalKeyboardKey.f4) {
      ...
      if (_tab.customer == null &&
        ...
        _tab._customerSearchCtrl.selection = TextSelection(
          baseOffset: 0,
          extentOffset: _tab._customerSearchCtrl.text.length,
        );
      }
      return true;
    }
  ...
  /// Chip «In bếp» lúc TT: tắt khi không còn món chưa báo. Mode auto bật lại
  /// nếu còn món mới. Tránh in thêm phiếu chế biến lúc thanh toán.
  void _syncCheckoutKitchenChip() {
    final pending = _tab.cart.any((l) => l.kitchenPendingQty > 0);
    if (!pending) {
      _quickPrintKitchen = false;
      return;
    }
    if (_printSettings.kitchenSlipPrintMode.shouldPrintOnCheckout) {
      _quickPrintKitchen = true;
    }
  }

  Future<void> _loadPrintSettings() async {
    final prevWarehouseId = _printSettings.warehouseTemplateId;
    ...
    if (mounted) {
      ...
      if (s.warehouseTemplateId != prevWarehouseId) {
  ...
  void _onSellIndustryChanged() {
    _cachedDesktopProductPane = null;
    _cachedProductPaneSig = null;
    _cachedDesktopFloorPane = null;
    _cachedFloorPaneSig = null;
  ...
  void _scheduleCatalogPrefetch() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted || _catalogPaneKept || !_useFloorAsPrimary) return;
      setState(() => _catalogPaneKept = true);
    });
  }

  PosSellProfile get _sellProfile =>
      _industrySettings?.sellProfile ?? PosSellProfile.retail;
  ...
  bool _tableAttachStale([int? epoch]) =>
      !mounted || (epoch != null && epoch != _tableAttachEpoch);
  /// Đang mở bàn từ sơ đồ — giữ sơ đồ, không nháy giỏ trống.
  bool _tableAttachBusy = false;
  ...
  final ValueNotifier<(double, int)> _floorActiveTotals =
      ValueNotifier((0, 0));
  double get _floorActiveSubtotal => _floorActiveTotals.value.$1;
  int get _floorActiveOpenCount => _floorActiveTotals.value.$2;
  /// Tablet/desktop 2 cột: true = màn thanh toán riêng (sau khi bấm
  /// "Thanh toán"), false = chọn hàng + giỏ.
  ...
  /// Máy có display phụ thật (Android DisplayManager / Chrome Multi-Screen).
  bool _hasSecondaryCustomerDisplay = false;
  ...
  void _scheduleCustomerDisplayPublish({int delayMs = 180}) {
    if (!_hasSecondaryCustomerDisplay &&
        !CustomerDisplaySync.instance.enabled) {
      return;
    }
    _customerDisplayPublishTimer?.cancel();
    final epoch = ++_customerDisplayPublishEpoch;
    _customerDisplayPublishTimer = Timer(Duration(milliseconds: delayMs), () {
      unawaited(_publishCustomerDisplay(epoch));
    });
  }
  ...
  Future<void> _publishCustomerDisplay([int? epoch]) async {
    final my = epoch ?? _customerDisplayPublishEpoch;
    bool stillValid() => my == _customerDisplayPublishEpoch;
    ...
    await sync.publishActive(
      ...
      storeName: _warehouseBranchName,
      promoItems: _customerDisplayPromos,
      paymentQrUrl: _customerDisplayPaymentQrUrl(),
      paymentStatus: _customerDisplayPaymentStatus(),
      stillValid: stillValid,
    );
  }

  static bool _isTingeeSourceKey(String key) => key == 'tingee';

  bool _hasTingeePaymentSelected() {
    for (final pay in _tab.paymentLines) {
      if (_isTingeeSourceKey(pay.sourceKey)) return true;
    }
    return false;
  }

  bool _hasTransferPaymentSelected() {
    for (final pay in _tab.paymentLines) {
      if (_isTingeeSourceKey(pay.sourceKey)) return true;
      final src = _sourceByKey(pay.sourceKey);
      if (src.methodType == PaymentMethodType.bankTransfer ||
          src.methodType == PaymentMethodType.vietQR) {
        return true;
      }
    }
    return false;
  ...
  ({double amount, String? preferredAccountId, String description, bool isTingee})?
      _resolveTransferPaymentQr() {
    if (_grandTotal <= 0) return null;
    if (_bankAccounts.isEmpty) return null;

    _SellPaymentLine? bankLine;
    String? preferredAccountId;
    var tingeeLine = false;
    for (final pay in _tab.paymentLines) {
      final src = _sourceByKey(pay.sourceKey);
      if (_isTingeeSourceKey(pay.sourceKey)) {
        bankLine = pay;
        tingeeLine = true;
        break;
      }
      if (src.methodType == PaymentMethodType.bankTransfer ||
          src.methodType == PaymentMethodType.vietQR) {
        bankLine = pay;
        preferredAccountId = src.bankAccountId;
        break;
      }
    }
    if (bankLine == null) return null;

    String? preferredForTingee;
    if (tingeeLine) {
      final va = (_paymentGatewaySettings?['tingeeVaAccountNumber'] ?? '')
          .toString()
          .trim();
      final tingeeAcc = PosVietQrHelper.resolveTingeeQrAccount(
        _accountsForTransferQr(tingee: true),
        vaAccountNumber: va,
        preferredId: _storeSettings.vietQrBankAccountId,
      );
    ...
    final resolvedAccountId = preferredAccountId ??
        preferredForTingee ??
        _storeSettings.vietQrBankAccountId;
    final account = PosVietQrHelper.resolveAccount(
      _accountsForTransferQr(tingee: tingeeLine),
      preferredId: resolvedAccountId,
    );
    if (account == null) return null;

    final amount = bankLine.amount > 0
        ? bankLine.amount
        : (_dueAmount > 0
            ? _dueAmount
            : (_tab.reservationDepositApplied > 0 ? 0.0 : _grandTotal));
    if (amount <= 0) return null;

    final description = tingeeLine
        ? _tingeeExternalOrderId()
    ...
    return (
      amount: amount,
      preferredAccountId: resolvedAccountId,
      description: description,
      isTingee: tingeeLine,
    );
  }

  List<BankAccount> _accountsForTransferQr({required bool tingee}) {
    if (!tingee) return _bankAccounts;
    final va = (_paymentGatewaySettings?['tingeeVaAccountNumber'] ?? '')
        .toString()
        .trim();
    return PosVietQrHelper.withTingeeVaAccount(
      _bankAccounts,
      vaAccountNumber: va,
      preferredId: _storeSettings.vietQrBankAccountId,
    );
  ...
  String? _customerDisplayPaymentStatus() {
    if (_useFloorAsPrimary && !_tabletPaymentStage) return null;
    if (!_hasTransferPaymentSelected()) return null;
    if (_customerDisplayPaymentQrUrl() == null) return null;
    return 'waiting';
  }
  ...
  Future<void> _reloadPosBankAccounts() async {
    var banksRes = await _api.getPosBankAccounts();
    if (banksRes['isSuccess'] != true) {
      banksRes = await _api.getBankAccounts();
    }
    if (!mounted || banksRes['isSuccess'] != true || banksRes['data'] is! List) {
      return;
    }
    final accounts = <BankAccount>[];
    for (final raw in banksRes['data'] as List) {
      if (raw is! Map) continue;
      final account = BankAccount.fromJson(Map<String, dynamic>.from(raw));
      if (account.isActive) accounts.add(account);
    }
    if (!mounted) return;
  ...
  Future<void> _ensureTingeeTransferIntent() async {
    if (!PosPaymentGatewayApi.isTingeeEnabled(_paymentGatewaySettings)) {
      return;
    }
    if (!_hasTingeePaymentSelected()) return;
    if (_useFloorAsPrimary && !_tabletPaymentStage) return;
    if (_grandTotal <= 0) return;

    final externalId = _tingeeExternalOrderId();
    final tableLabel = formatPosTableLabel(
      areaName: _tab.serviceAreaName,
      tableName: _tab.serviceResourceName,
    );
    var amount = _grandTotal;
    for (final pay in _tab.paymentLines) {
      if (_isTingeeSourceKey(pay.sourceKey) && pay.amount > 0) {
        amount = pay.amount;
        break;
      }
    }
    ...
    await PosPaymentGatewayApi(_api).createIntent(
      ...
      tableName: tableLabel.isEmpty ? null : tableLabel,
      saleOrderId: _tab.draftOrderId,
    );
    _startTingeeIntentPoll();
  }

  void _startTingeeIntentPoll() {
    _tingeeIntentPoll?.cancel();
    _tingeeIntentPoll = Timer.periodic(const Duration(seconds: 3), (_) {
      unawaited(_pollTingeeIntentConfirmed());
    });
  }

  void _stopTingeeIntentPoll() {
    _tingeeIntentPoll?.cancel();
    _tingeeIntentPoll = null;
  }

  Future<void> _pollTingeeIntentConfirmed() async {
    if (!mounted || _checkingOut || _parking) return;
    if (!_isWaitingTingeePayment()) {
      _stopTingeeIntentPoll();
      return;
    }
    final rows = await PosPaymentGatewayApi(_api).listIntents(status: 'Confirmed');
    if (!mounted || _checkingOut) return;
    final ext = _tingeeExternalIdOf(_tab);
    final expected = _resolveTransferPaymentQr()?.amount ?? _grandTotal;
    for (final r in rows) {
      final orderNo = (r['orderNo'] ?? '').toString().trim();
      final external = (r['externalOrderId'] ?? '').toString().trim();
  ...
  /// QR màn phụ: chỉ khi đã vào bước thanh toán + chọn CK/VietQR/Tingee.
  String? _customerDisplayPaymentQrUrl() {
    if (_useFloorAsPrimary && !_tabletPaymentStage) return null;
    final qr = _resolveTransferPaymentQr();
    if (qr == null) return null;

    final account = PosVietQrHelper.resolveAccount(
      _accountsForTransferQr(tingee: qr.isTingee),
      preferredId: qr.preferredAccountId,
    );
    if (account == null) return null;
  ...
  Future<void> _enterPaymentStageAsync() async {
    ...
    if (PosPaymentGatewayApi.isTingeeEnabled(_paymentGatewaySettings) &&
      ...
      await _reloadPosBankAccounts();
    }
    if (_hasTingeePaymentSelected()) {
      unawaited(_ensureTingeeTransferIntent());
    }
  }

  bool _shouldShowPaymentQrOverlay() {
    if (_paymentQrOverlayDismissed) return false;
    if (!_hasTransferPaymentSelected()) return false;
    return _resolveTransferPaymentQr() != null;
  }

  Widget _buildPaymentQrWaitingOverlay() {
    final qr = _resolveTransferPaymentQr();
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      PosVietQrPaymentPanel(
                        accounts: _accountsForTransferQr(tingee: qr.isTingee),
                        amount: qr.amount,
                        preferredAccountId: qr.preferredAccountId,
                        description: qr.description,
                        title: panelTitle,
                        lockAccountSelection: qr.isTingee,
                      ),
                      const SizedBox(height: 10),
                      Text(
                        tr(
                          'Khách quét mã trên màn phụ. Bạn có thể thu gọn để tiếp tục thao tác.',
                        ),
                      ...
                      OutlinedButton.icon(
                        ...
                        label: Text(tr('Thu gọn — làm việc khác')),
                      ),
                    ],
                  ),
                ),
  ...
  Future<void> _openSystemNotifications() async {
    if (NavigationNotifier.mainLayoutReady.value) {
      ...
      // Quay lại bán hàng: đếm lại sau một nhịp (user có thể đã đọc).
  ...
  Future<void> _openCustomerDisplay() async {
    ...
    sync.ensureViewerCode();
    await _refreshCustomerDisplayPromos();
    _scheduleCustomerDisplayPublish();

    if (kIsWeb) {
      final ok = await sync.openSecondary();
      final hasSecondary = ok || await sync.hasSecondaryDisplay();
      if (mounted) {
        setState(() => _hasSecondaryCustomerDisplay = hasSecondary);
      }
      if (!mounted) return;
      if (ok) {
        NotificationOverlayManager().showSuccess(
          ...
          message: tr('Hóa đơn khách đang hiện trên màn hình thứ 2'),
        );
        return;
      }
      final reason = sync.lastOpenReason;
      NotificationOverlayManager().showWarning(
        title: 'Chưa mở được màn thứ 2',
        message: tr(_webCustomerDisplayFailMessage(reason)),
      );
      if (!mounted) return;
      await _showCustomerDisplayViewerLinkDialog();
      return;
    }
  ...
  String _webCustomerDisplayFailMessage(String reason) {
    switch (reason) {
      case 'no-second-screen':
        return 'Không thấy màn hình thứ 2. Cắm thêm màn rồi bấm lại, hoặc copy link sang máy/TV khác.';
      case 'popup-blocked':
        return 'Trình duyệt chặn cửa sổ. Cho phép popup cho trang này rồi bấm lại.';
      case 'permission-denied':
        return 'Cần cho phép quản lý cửa sổ trên mọi màn hình, rồi bấm lại.';
      case 'no-api':
        return 'Trình duyệt này không đặt được cửa sổ sang màn khác. Dùng Chrome, hoặc copy link sang máy/TV.';
      default:
        return 'Không mở được màn khách trên màn thứ 2. Có thể copy link sang máy khác.';
    }
  }

  Future<void> _showCustomerDisplayViewerLinkDialog() async {
    final link = CustomerDisplaySync.instance.viewerBrowserLink;
    final code = CustomerDisplaySync.instance.ensureViewerCode();
    await showDialog<void>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(tr('Link màn hình phụ')),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          crossAxisAlignment: CrossAxisAlignment.stretch,
          children: [
            Text(
              tr(
                'Mở link này trên máy/TV khác (trình duyệt — không cần đăng nhập). '
                'Giữ máy thu ngân đang bán hàng để đồng bộ.',
              ),
        ...
        actions: [
          ...
          FilledButton.icon(
            onPressed: () async {
              ...
              NotificationOverlayManager().showSuccess(
                title: 'Đã copy link',
                message: tr('Dán trên trình duyệt máy khác'),
              );
            },
  ...
  Future<void> _attachFloorResult(Map<String, dynamic> result) async {
    ...
    void applyPauseMeta(_SellInvoiceTab tab) {
      ...
      if (paidFromDeposit != null && paidFromDeposit > 0) {
        tab.reservationDepositApplied = paidFromDeposit;
    ...
    if (orderId != null && orderId.isNotEmpty) {
      ...
      if (resourceId != null && resourceId.isNotEmpty) {
        _floorFreedResourceIds = {
          for (final e in _floorFreedResourceIds)
            if (e.toLowerCase() != resourceId.toLowerCase()) e,
        };
      }
      ...
      // Giữ sơ đồ đến khi hydrate xong — tránh nháy logo / Chọn món / Thanh toán.
      if (switchingTable) {
        _tab.reset(
      ...
      if (isSplitBill && (splitFrom ?? '').isNotEmpty) {
        _tab.splitFromOrderId = splitFrom;
      }
      ...
      if (_tableAttachStale(attachEpoch)) {
        if (mounted) setState(() => _tableAttachBusy = false);
        return;
      }
      ...
      final alreadyHydrated = _tab.cart.length == _tab.serverLineCount;
      if (!alreadyHydrated) {
        await _verifyTableCartHydrated(orderId, attachEpoch: attachEpoch);
      }
      if (_tableAttachStale(attachEpoch)) {
        if (mounted) setState(() => _tableAttachBusy = false);
        return;
      ...
      setState(() {
        ...
        _catalogPaneKept = true;
        _mobileSellKept = true;
        _tabletPaymentStage = isSplitBill && _tab.cart.isNotEmpty;
        _mobileProductPickerOpen = false;
        _suspendDraftAutosave = false;
      });
      if (_tab.localDirty) _scheduleDraftAutosave();
      if (isSplitBill && _tab.cart.isEmpty) {
        NotificationOverlayManager().showWarning(
          title: 'Hóa đơn tách trống',
          message: tr('Thêm món hoặc xóa hóa đơn này — bàn gốc vẫn giữ món còn lại'),
        );
  ...
  void _scheduleFloorQuietRefresh([
    Duration delay = const Duration(milliseconds: 1600),
  ]) {
    _floorQuietRefreshTimer?.cancel();
    _floorQuietRefreshTimer = Timer(delay, () {
      if (!mounted) return;
      _floorPaneKey.currentState?.refreshQuiet();
    });
  }

  void _markFloorResourceFreed(String? resourceId) {
    final id = (resourceId ?? '').trim();
    if (id.isEmpty) return;
    setState(() {
      _floorFreedResourceIds = {..._floorFreedResourceIds, id};
    });
  }

  /// Sơ đồ báo cờ lạc quan đã khớp server / bị máy khác ghi đè.
  void _reconcileOptimisticFloorFlags({
    Set<String> dropFreed = const {},
    Set<String> dropKitchen = const {},
    Set<String> dropBill = const {},
  }) {
    if (dropFreed.isEmpty && dropKitchen.isEmpty && dropBill.isEmpty) return;
    bool hit(String e, Set<String> drop) =>
        drop.any((d) => d.toLowerCase() == e.toLowerCase());
    setState(() {
      if (dropFreed.isNotEmpty) {
        _floorFreedResourceIds = {
          for (final e in _floorFreedResourceIds)
            if (!hit(e, dropFreed)) e
        };
      }
      if (dropKitchen.isNotEmpty) {
        _kitchenClearedResourceIds = {
          for (final e in _kitchenClearedResourceIds)
            if (!hit(e, dropKitchen)) e
        };
      }
      if (dropBill.isNotEmpty) {
  ...
  /// Về sơ đồ: hiện floor ngay (optimistic), unlock/autosave chạy nền.
  ...
  Future<void> _returnToFloorMap() async {
    ...
    // Đánh dấu nhả trước — chặn heartbeat/autosave gắn lại khóa + tô «tạm rời» trên sơ đồ.
    ...
    setState(() {
      ...
      _mobileProductPickerOpen = false;
      _tableAttachBusy = false;
      // Giữ suspend đến khi nền xong unlock — tránh autosave gắn lại khóa.
    });
    _scheduleFloorQuietRefresh();
    _scheduleCustomerDisplayPublish();

    unawaited(_finishLeaveTableInBackground(
      wasViewOnly: wasViewOnly,
      hasDraft: hasDraft,
      draftId: draftId,
      emptyCart: emptyCart,
      resourceId: rid,
  ...
  /// Autosave idle + unlock + free empty seat — không chặn UI sơ đồ.
  /// Unlock fail: giữ releasedOrderIds (park local), retry thêm, không rollback.
  Future<void> _finishLeaveTableInBackground({
    required bool wasViewOnly,
    ...
    if (!wasViewOnly && hasDraft && draftId != null && draftId.isNotEmpty) {
      var unlocked = await _unlockDraftQuietly(draftId);
      if (!unlocked) {
        await Future<void>.delayed(const Duration(seconds: 2));
        unlocked = await _unlockDraftQuietly(draftId);
      }
      if (!unlocked && mounted && epoch == _tableAttachEpoch) {
        NotificationOverlayManager().showWarning(
          title: 'Đang nhả khóa nền',
          message: tr(
              'Đã về sơ đồ — máy khác thử «Lấy quyền» nếu vẫn báo đang sửa'),
        );
      }
    }

    if (emptyCart && !hasDraft && (resourceId ?? '').isNotEmpty) {
      try {
        await _api.freePosServiceResource(resourceId!);
      } catch (_) {}
    } else if (emptyCart && !hasDraft && sessionId != null && sessionId.isNotEmpty) {
      try {
        await _api.closePosResourceSession(sessionId);
      } catch (_) {}
    }
  ...
  void _onFloorResourceFreed(String resourceId) {
    _draftAutosaveTimer?.cancel();
    _suspendDraftAutosave = true;
    _markFloorResourceFreed(resourceId);
    ...
    final draftId = _tab.draftOrderId;
    final orphanStaleCart = _tab.cart.isNotEmpty &&
        (_tab.serviceResourceId == null || _tab.serviceResourceId!.isEmpty) &&
        (draftId == null ||
            draftId.isEmpty ||
            _floorReleasedOrderIds.contains(draftId));
    // Luôn tắt dirty nếu đang gắn đúng bàn — kể cả khi đang xem sơ đồ.
    if (matchResource) {
      _tab.localDirty = false;
    }
    if (!matchResource && !orphanStaleCart) {
      _suspendDraftAutosave = false;
      return;
    }
    setState(() {
  ...
  Future<void> _kitchenSendCurrentTable() async {
    ...
    var n = (data['sentLines'] as num?)?.toInt() ?? 0;
    var already = data['alreadyAllSent'] == true || n <= 0;
    var remoteVer = data['lockVersion'] is num
    ...
    // Client còn món mới nhưng server bảo alreadyAllSent → thường do draft
    // chưa kịp ghi Qty. Thử lưu lại + KitchenSend 1 lần — KHÔNG đánh dấu
    // kitchenSentQty=qty (trước đây làm mất phần tăng SL, không báo được nữa).
    if (sendTicketLines.isNotEmpty && already) {
      debugPrint(
        'POS kitchen: client pending=${sendTicketLines.length} but server alreadyAllSent — retry persist',
      );
      _tab.localDirty = true;
      final savedRetry = await _persistDraftAutosave(
        forTab: _tab,
        showLockError: true,
      ...
      if (!mounted) return;
      if (savedRetry) {
        final res2 = await _api.kitchenSendPosResourceSession(
          sid,
          deviceId: _posDeviceId,
          deviceName: _posDeviceName,
        );
        ...
        if (res2['isSuccess'] == true) {
          final data2 = res2['data'] is Map ? res2['data'] as Map : const {};
          n = (data2['sentLines'] as num?)?.toInt() ?? 0;
          already = data2['alreadyAllSent'] == true || n <= 0;
          remoteVer = data2['lockVersion'] is num
              ? (data2['lockVersion'] as num).toInt()
              : int.tryParse('${data2['lockVersion']}') ?? remoteVer;
        }
      }
      if (sendTicketLines.isNotEmpty && already) {
        NotificationOverlayManager().showError(
          title: 'Chưa báo được phần mới',
          message: tr(
            'Số lượng tăng chưa đồng bộ lên server — thử Báo bếp lại sau 1–2 giây',
          ),
        );
        return;
      }
    ...
    final shouldPrint = sendTicketLines.isNotEmpty &&
        !already &&
        _printSettings.kitchenSlipPrintMode.shouldPrintOnBaoBep;
    if (sendTicketLines.isNotEmpty && !already) {
      if (mounted) {
        setState(() {
          for (final l in pendingLines) {
            ...
            l.kitchenCancelPendingQty = 0;
            // Không đánh dấu xuất kho đã in khi chỉ báo bếp — nếu không
            // thanh toán (mode auto) sẽ lọc hết dòng và không in phiếu kho.
          }
          _syncCheckoutKitchenChip();
      ...
      unawaited(_persistDraftAutosave(forTab: _tab, retryOnConflict: true));

      // Mở lại nút ngay khi server đã nhận món — in chạy nền (A7 không đơ).
      if (_kitchenSending && mounted) {
        setState(() => _kitchenSending = false);
      }
      final rid = _tab.serviceResourceId;
      ...
      NotificationOverlayManager().showSuccess(
        title: 'Đã gửi báo bếp',
        message: tr(
          '${sendTicketLines.length} món · ${_tab.serviceResourceName ?? ''}',
        ),
        relatedEntityType: kPosPrintNotifyKind,
      );
      unawaited(_dispatchKitchenNotifyPrint(
        shouldPrint: shouldPrint,
        sendTicketLines: sendTicketLines,
        pendingLines: List<_SellCartLine>.from(pendingLines),
      ));
      return;
    }

    // Không còn phiếu client — thật sự không có món mới.
    if (sendTicketLines.isNotEmpty) {
      // Không bao giờ đánh dấu sent khi vẫn còn pending client.
      return;
    }
    if (mounted) {
      setState(() {
    ...
    NotificationOverlayManager().showSuccess(
      ...
      if (mounted && _kitchenSending) {
        setState(() => _kitchenSending = false);
      }
    }
  }

  /// In phiếu bếp / tem ly sau khi server đã nhận món — không chặn nút Thông báo.
  Future<void> _dispatchKitchenNotifyPrint({
    required bool shouldPrint,
    required List<KitchenTicketLine> sendTicketLines,
    required List<_SellCartLine> pendingLines,
  }) async {
    var kitchenOk = true;
    if (shouldPrint) {
      var enqueuedPerPrinterFail = false;
      kitchenOk = await printKitchenCompactSlip(
        ...
        onCloudHang: ({
          ...
          _enqueueFailedKitchenPrint(
            PendingKitchenPrintJob(
              ...
              errorMessage:
                  'Chưa in sau 60s ($printerName) — chọn máy khác hoặc in lại',
            ),
          );
          NotificationOverlayManager().showWarning(
            title: 'Phiếu bếp treo',
            message: tr(
              'Máy $printerName chưa in (${lines.length} món) — mở hàng chờ để in lại',
            ),
            relatedEntityType: kPosPrintNotifyKind,
            duration: const Duration(seconds: 5),
            onTap: () {
              if (mounted) unawaited(_openPendingPrintQueue());
            },
          );
        },
        onPrinterFailed: ({
          ...
          enqueuedPerPrinterFail = true;
          if (!mounted) return;
          _enqueueFailedKitchenPrint(
            PendingKitchenPrintJob(
              id:
                  'kitchen_fail_${printerId}_${DateTime.now().millisecondsSinceEpoch}',
              isCancel: false,
              tableName: formatPosTableLabel(
                areaName: _tab.serviceAreaName,
                tableName: _tab.serviceResourceName,
      ...
      if (!kitchenOk && !enqueuedPerPrinterFail) {
        _enqueueFailedKitchenPrint(
          PendingKitchenPrintJob(
    ...
    if (_printSettings.shouldPrintCupOnKitchenSend) {
  ...
  List<KitchenTicketLine> _collectKitchenCancelLines() {
    ...
    // Gộp cùng SP + đơn vị + ghi chú. Gộp theo tên làm hai món khác ghi chú
    // dồn thành một dòng (bếp hủy nhầm phần còn giữ), và hai SP trùng tên bị
    // gán chung productId nên phiếu hủy chạy sai máy in.
    final merged = <String, KitchenTicketLine>{};
    for (final l in list) {
      final key = '${l.productId ?? l.productName}'
          '|${l.unitName ?? ''}|${l.note ?? ''}';
      ...
      if (prev == null) {
        ...
        merged[key] = KitchenTicketLine(
          ...
          sentBefore: prev.sentBefore ?? l.sentBefore,
  ...
  Future<void> _printProvisionalBill() async {
    ...
    try {
      ...
      if (!billMarked && _isTableOrderMode) {
        NotificationOverlayManager().showWarning(
          title: 'Chưa lưu trạng thái tạm tính',
          message: tr(
            'Đã tô màu ${_sellProfile.resourceNoun} tạm — kiểm tra lại nếu reload mất màu',
          ),
        );
      }
  ...
  static String _kitchenVoidReasonNote(CancelReturnReasonResult r) {
    final parts = <String>[];
    final reason = r.reason.trim();
    if (reason.isNotEmpty) parts.add('Lý do: $reason');
    final detail = r.detailNote?.trim();
  ...
  Future<bool> _voidKitchenSentLines(
    ...
    final reasonNote = _kitchenVoidReasonNote(resolved);
    final printLines = [
    ...
    final shouldPrint =
        _printSettings.kitchenSlipPrintMode != PosKitchenSlipPrintMode.off;
    var enqueuedPerPrinterFail = false;
    final ok = !shouldPrint
        ? true
        : await printKitchenCompactSlip(
            ...
            onPrinterFailed: ({
              ...
              _enqueueFailedKitchenPrint(
                PendingKitchenPrintJob(
                  id:
                      'kcancel_fail_${printerId}_${DateTime.now().millisecondsSinceEpoch}',
                  isCancel: true,
                  tableName: formatPosTableLabel(
                    areaName: _tab.serviceAreaName,
                    tableName: _tab.serviceResourceName,
  ...
  bool get _cupLabelActionEnabled =>
      _cupLabelPendingCount > 0 && _hasReadyLabelPrinter;

  Future<void> _refreshLabelPrinterReady() async {
    final ready = await hasReadyCupLabelPrinter();
    if (!mounted) return;
    if (ready != _hasReadyLabelPrinter) {
      setState(() => _hasReadyLabelPrinter = ready);
    }
  }
  ...
  Future<void> _printCupLabelsForLines(
    ...
    for (final l in lines) {
      ...
      final toppings = l.toppings
          .map((t) => posToppingNameWithQty(t.name, t.safeQty))
          .join(', ');
    ...
    final ready = await hasReadyCupLabelPrinter();
    if (mounted && ready != _hasReadyLabelPrinter) {
      setState(() => _hasReadyLabelPrinter = ready);
    }
    if (!ready) {
      if (showFeedback) {
    ...
    if (mounted) {
      setState(() {
        ...
        // Xóa phiếu tem treo trùng bàn/HĐ — tránh in lại chồng sau khi đã OK.
        _failedCupPrints.removeWhere((j) {
          final sameTable = (j.tableLabel ?? '') == table;
          final sameOrder = (j.orderNo ?? '') == orderNo;
          return sameTable || sameOrder;
        });
      });
    }
    if (showFeedback) {
  ...
  Future<void> _openSessionRedeem() async {
    ...
    final redeemed = await showPosSessionRedeemSheet(
      ...
      saleOrderId: _tab.draftOrderId,
      sellers: _sellSellers,
      requireStaff: _industrySettings?.requireStaffOnService == true,
      enableRedeem: _industrySettings?.enableSessionPacks == true,
      initialTab: 0,
    );
  ...
  Future<void> _applyInvoiceSlotsPayload(
    ...
    for (final raw in rawItems) {
      ...
      if (prev == null) {
        ...
        tab.applyFeeDefaults(_storeSettings);
      }
  ...
  Future<void> _onSelectedTabActivated() async {
    ...
    // HĐ slot trống: không chiếm chỗ. Đơn bàn / tách bill trống vẫn phải claim khóa để thêm món.
    if (tab.cart.isEmpty && tab.serverLineCount == 0 && !tab.isTableBound) {
      if (tab.draftReadOnly) {
        setState(() {
          tab.draftReadOnly = false;
          tab.lockedByLabel = null;
    ...
    if (lockRes['statusCode'] == 409) {
      // HĐ tách / bàn trống: cướp khóa để thêm món hoặc xóa (không phải máy kia đang bán).
      if (tab.isTableBound && tab.cart.isEmpty && tab.serverLineCount == 0) {
        final forced = await _api.lockPosSaleDraft(
          orderId,
          deviceId: deviceId,
          deviceName: deviceName,
          force: true,
        ...
        if (!mounted) return;
        if (forced['isSuccess'] == true) {
          setState(() {
            tab.draftReadOnly = false;
            tab.lockedByLabel = null;
            if (forced['data'] is Map<String, dynamic>) {
              _applyLockMetaFromMap(
                  forced['data'] as Map<String, dynamic>, tab: tab);
            }
  ...
  void _recalcTotals({bool publishDisplay = true}) {
    ...
    if (_storeSettings.enableSurcharge) {
      _tab.applySurcharge(afterOrderDiscount);
    }
    if (publishDisplay) _scheduleCustomerDisplayPublish();
  }
  ...
  double get _grandTotal {
    final fees = (_storeSettings.enableSurcharge ? _tab.surchargeAmount : 0) +
        (_storeSettings.enableDeliveryFee ? _tab.deliveryFee : 0);
    if (_storeSettings.taxMode == PosSellTaxMode.perItem) {
      ...
      return (base - _tab.voucherDiscount - _tab.pointsDiscount + fees)
  ...
  double _effectivePaidAmountFor(_SellInvoiceTab tab) {
    final deposit = tab.reservationDepositApplied;
    if (tab.paymentLines.isNotEmpty) {
      return deposit +
          tab.paymentLines.fold(0.0, (a, p) => a + p.amount);
    }
  ...
  /// Còn phải thu sau khi trừ cọc đặt bàn (chưa kể dòng TT).
  double get _amountAfterDeposit =>
      (_grandTotal - _tab.reservationDepositApplied).clamp(0.0, double.infinity);
  ...
  void _notifyPaymentUi(VoidCallback? onMutate) {
    ...
    // Đổi CK/VietQR/Tingee/số tiền → cập nhật QR trên màn phụ.
    _scheduleCustomerDisplayPublish();
    if (_hasTingeePaymentSelected()) {
      unawaited(_ensureTingeeTransferIntent());
    }
  ...
  bool _validateStockQty(
    ...
    if (p.hasRecipe) {
      ...
      final err = comboStockErrorMessage(
        combo: p.copyWith(comboLines: p.recipeLines),
        requiredComboQty: requiredQty,
        componentReserved: reserved,
        kindLabel: 'Định lượng',
      );
      if (err != null) {
        NotificationOverlayManager().showError(
          title: 'Không đủ nguyên vật liệu',
          message: err,
        );
        return false;
      }
  ...
  Future<bool> _refreshCartStockBeforeCheckout() async {
    ...
    for (final line in _tab.cart) {
      ...
      // getPosProduct cũ có thể thiếu comboLines — giữ bản đã có trên giỏ nếu fresh rỗng.
      final mergedCombo = (freshProduct.comboLines != null &&
              freshProduct.comboLines!.isNotEmpty)
          ? freshProduct.comboLines
          : line.product.comboLines;
      final mergedRecipe = (freshProduct.recipeLines != null &&
              freshProduct.recipeLines!.isNotEmpty)
          ? freshProduct.recipeLines
          : line.product.recipeLines;
      line.product = freshProduct.copyWith(
        comboLines: mergedCombo,
        recipeLines: mergedRecipe,
      );
  ...
  Future<void> _addPick(
    ...
    if (!perm.canPosOrder()) {
      NotificationOverlayManager().showWarning(
        ...
        message: tr('Tài khoản không được order / sửa giỏ hàng'),
      );
      return;
    }
    _playSellPickClick();
    ...
    if (line != null &&
        ...
        (line.product.hasToppingGroups ||
            ...
            line.product.allowToppings)) {
      await _openCartLineToppings(line);
    }
    if (line != null && mounted) {
      await _maybeAssignLineStaff(line);
    }
  }
  ...
  bool _lineNeedsStaffPick(_SellCartLine line) {
    ...
    return p.productType == PosProductType.service || p.hasCommission;
  }

  Future<void> _maybeAssignLineStaff(_SellCartLine line) async {
    if (!_lineNeedsStaffPick(line)) return;
    await _assignLineStaff(line);
  }

  Future<void> _assignLineStaff(_SellCartLine line) async {
    if (_sellSellers.isEmpty) return;
    final current = line.staffAssignments
        .map((a) => PosStaffAssignment(
    ...
    if (current.isEmpty &&
        ...
        line.assignedEmployeeId!.isNotEmpty) {
      current.add(PosStaffAssignment(
        assignedEmployeeId: line.assignedEmployeeId!,
        assignedEmployeeName: line.assignedEmployeeName ?? '',
      ));
    }
    final picked = await PosLineStaffAssignSheet.show(
      context: context,
      slots: PosLineStaffAssignSheet.slotsForProduct(line.product),
      sellers: _sellSellers,
      current: current,
      requireService: _industrySettings?.requireStaffOnService == true,
    );
    if (picked == null || !mounted) return;
    setState(() {
      line.staffAssignments = picked
          .map((a) => _CartStaffAssign(
                componentProductId: a.componentProductId,
                assignedEmployeeId: a.assignedEmployeeId,
                assignedEmployeeName: a.assignedEmployeeName,
              ))
      ...
      if (picked.isNotEmpty) {
        line.assignedEmployeeId = picked.first.assignedEmployeeId;
        line.assignedEmployeeName = picked.first.assignedEmployeeName;
      } else {
        line.assignedEmployeeId = null;
        line.assignedEmployeeName = null;
      }
    });
    _scheduleDraftAutosave();
  }

  String? _staffLabelForLine(_SellCartLine line) {
    if (line.staffAssignments.isNotEmpty) {
      return line.staffAssignments
          .map((a) => a.assignedEmployeeName)
          .where((n) => n.trim().isNotEmpty)
          .join(', ');
    }
    return line.assignedEmployeeName;
  }
  ...
  Future<void> _openDraftOrder(
    ...
    if (existing >= 0) {
      ...
      if (forceClaim) {
        ...
        if (_tableAttachStale(attachEpoch) || !ok) {
          if (mounted) _suspendDraftAutosave = false;
          return;
        }
        setState(() {
          _tab.draftReadOnly = false;
        ...
        if (_tableAttachStale(attachEpoch) || access == null) {
          if (mounted) _suspendDraftAutosave = false;
          return;
        }
        setState(() {
  ...
  bool _guardReadOnlyEdit({String? message}) {
    ...
    if (!perm.canPosOrder()) {
      NotificationOverlayManager().showWarning(
        title: 'Không có quyền',
        message: message ?? tr('Tài khoản không được order / sửa giỏ hàng'),
      );
      return false;
    }
  ...
  void _scheduleDraftAutosave({Duration delay = const Duration(milliseconds: 700)}) {
    if (_checkingOut || _parking) return;
    final tab = _tab;
  ...
  Future<bool> _persistDraftAutosave({
    ...
    if (!mounted) return false;
    if (!Provider.of<PermissionProvider>(context, listen: false).canPosOrder()) {
      return false;
    }
    ...
    final epochAtStart = tab.cartEpoch;
    final draftIdAtStart = tab.draftOrderId;
    final hadCartAtStart = tab.cart.isNotEmpty;
    ...
    try {
      ...
      // Thanh toán / về sơ đồ / reset tab trong lúc await — không gắn lại draft (tránh món «ma»).
      if (_checkingOut || _parking || _suspendDraftAutosave) {
        if (orderId != null && orderId.isNotEmpty) {
          _floorReleasedOrderIds.add(orderId);
        }
        return false;
      }
      if (orderId != null && _floorReleasedOrderIds.contains(orderId)) {
        return false;
      }
      if (draftIdAtStart != null &&
          draftIdAtStart.isNotEmpty &&
          _floorReleasedOrderIds.contains(draftIdAtStart)) {
        return false;
      }
      if (hadCartAtStart &&
          tab.cart.isEmpty &&
          (tab.draftOrderId == null || tab.draftOrderId!.isEmpty) &&
          tab.cartEpoch != epochAtStart) {
        if (orderId != null && orderId.isNotEmpty) {
          _floorReleasedOrderIds.add(orderId);
        }
        return false;
  ...
  Future<void> _pullServerOrderIntoTab(
    ...
    final orderId = tab.draftOrderId;
    if (orderId == null || orderId.isEmpty) return;
    if (_floorReleasedOrderIds.contains(orderId)) {
      // Đơn vừa TT / nhả bàn — không kéo món cũ vào giỏ.
      if (identical(tab, _tab) && mounted) {
        setState(() {
          tab.reset(
              defaultVatRate: _storeSettings.defaultVatRate, store: _storeSettings);
          tab.sellerEmployeeId = _defaultSellerEmployeeId;
          _syncPaidAmount();
        });
      }
  ...
  Future<void> _hydrateTabCartFromOrder(
    ...
    if (_tableAttachStale(attachEpoch)) return;
    // Món user vừa chọn khi mở bàn (GET/hydrate chậm) — giữ rồi merge sau snapshot server.
    final preserve = (tab.localDirty && tab.cart.isNotEmpty)
        ? List<_SellCartLine>.from(tab.cart)
        : <_SellCartLine>[];
    ...
    final originalRemainingIds = {for (final l in tab.cart) l.rowId};
    ...
    final viewsByProduct = <String, List<PosProductUnitView>>{};
    final needApiViews = <String, PosProduct>{};
    for (final line in order.lines) {
      if (line.productId.isEmpty) continue;
      final p = products[line.productId];
      if (p == null) continue;
      if (!posProductHasEmbeddedSellViews(p)) {
        needApiViews.putIfAbsent(p.id, () => p);
      }
    }
    if (needApiViews.isNotEmpty) {
      final loaded = await Future.wait(
        needApiViews.values.map((p) => loadPosSellUnitViews(_api, p)),
      );
      if (_tableAttachStale(attachEpoch)) return;
      var i = 0;
      for (final p in needApiViews.values) {
        viewsByProduct[p.id] = loaded[i++];
      }
    }
    ...
    for (final line in order.lines) {
      ...
      var views = posProductHasEmbeddedSellViews(p)
          ...
          : (viewsByProduct[p.id] ?? const <PosProductUnitView>[]);
      if (views.isEmpty) {
        views = await loadPosSellUnitViews(_api, p);
        if (_tableAttachStale(attachEpoch)) return;
      ...
      final prevWh = tab.cart
          .where((c) =>
              ...
              (c.unitId ?? '') == (view.unitId ?? ''))
          .map((c) => c.warehouseSlipPrintedQty)
          .firstOrNull;
      cartLine.warehouseSlipPrintedQty =
          (prevWh ?? 0).clamp(0.0, line.qty).toDouble();
      cartLine.toppings = line.toppings
          .map((t) => _CartTopping(id: t.id, name: t.name, price: t.price, qty: t.qty))
          .toList();
    ...
    if (_tableAttachStale(attachEpoch)) return;
    final extra = tab.cart
        .where((l) => !originalRemainingIds.contains(l.rowId))
        .toList();
    final keepLocal = preserve.isNotEmpty || extra.isNotEmpty;
    setState(() {
      for (final c in List<_SellCartLine>.from(tab.cart)) {
        if (!extra.contains(c)) c.dispose();
      }
      tab.cart
        ..clear()
        ..addAll(cartLines)
      ...
      tab._surchargeCtrl.text = order.surchargeAmount ==
              order.surchargeAmount.roundToDouble()
          ? order.surchargeAmount.toStringAsFixed(0)
          : order.surchargeAmount.toStringAsFixed(2);
      tab.deliveryFee = order.deliveryFee;
      tab._deliveryFeeCtrl.text =
          order.deliveryFee == order.deliveryFee.roundToDouble()
      ...
      if ((order.splitFromOrderId ?? '').isNotEmpty) {
        tab.splitFromOrderId = order.splitFromOrderId;
        tab.resourceSessionId = null;
      } else {
        tab.splitFromOrderId = null;
      ...
      if (order.isDelivery ||
          ...
          (order.deliveryPhone ?? '').trim().isNotEmpty) {
        final phone = (order.deliveryPhone ?? order.customerPhone ?? '').trim();
        final addr = (order.deliveryAddress ?? '').trim();
        if (phone.isNotEmpty) {
          tab.deliveryPhone = phone;
          tab._deliveryPhoneCtrl.text = phone;
        }
        if (addr.isNotEmpty) {
          tab.deliveryAddress = addr;
          tab._deliveryAddressCtrl.text = addr;
        }
        if ((order.deliveryProvince ?? '').trim().isNotEmpty) {
          tab.deliveryProvince = order.deliveryProvince!.trim();
          tab._deliveryProvinceCtrl.text = tab.deliveryProvince!;
        }
        if ((order.deliveryDistrict ?? '').trim().isNotEmpty) {
          tab.deliveryDistrict = order.deliveryDistrict!.trim();
          tab._deliveryDistrictCtrl.text = tab.deliveryDistrict!;
        }
        if ((order.deliveryWard ?? '').trim().isNotEmpty) {
          tab.deliveryWard = order.deliveryWard!.trim();
          tab._deliveryWardCtrl.text = tab.deliveryWard!;
        }
        if ((order.deliveryPartner ?? '').trim().isNotEmpty) {
          tab.deliveryPartner = order.deliveryPartner!.trim();
          tab._deliveryPartnerCtrl.text = tab.deliveryPartner!;
        }
      }
      if (order.customerId != null && order.customerId!.isNotEmpty) {
        ...
        final guestName = (order.customerName ?? '').trim();
        if (guestName.isNotEmpty && !_isWalkInCustomerQuery(guestName)) {
          tab.customer = null;
          tab._customerSearchCtrl.text = guestName;
        } else {
          tab.customer = null;
          tab._customerSearchCtrl.text = kPosWalkInCustomerLabel;
        }
      }
      if (order.paidAmount > 0 && tab.reservationDepositApplied <= 0) {
        tab.reservationDepositApplied = order.paidAmount;
        tab.paidAmount = order.paidAmount;
      }
      if (identical(tab, _tab)) {
        _syncPaidAmount();
      }
  ...
  Future<void> _verifyTableCartHydrated(
    String orderId, {
    int? attachEpoch,
  }) async {
    // User đang sửa — bỏ verify để không đè giỏ.
    if (_tab.localDirty || _tableAttachStale(attachEpoch)) return;
    await _ensureDeviceReady();
    if (_tableAttachStale(attachEpoch)) return;
    final orderJson = await _fetchDraftOrderJson(orderId);
    if (_tableAttachStale(attachEpoch) || orderJson == null) return;
    ...
    setState(() {
      _tab.serverLineCount = remoteLines;
      _tab.serverTotal = remoteTotal;
      if (!_tab.localDirty) _tab.localDirty = false;
    });
  ...
  String _cartSyncFingerprint(_SellInvoiceTab tab) {
    ...
    for (final c in tab.cart) {
      parts.add([
        ...
        c.toppings.map((t) => '${t.id}:${t.price}x${t.safeQty}').join(','),
      ].join(':'));
    }
    return parts.join('|');
  }
  ...
  String _orderSyncFingerprint(PosSaleOrder order) {
    ...
    for (final l in order.lines) {
      parts.add([
        ...
        l.toppings.map((t) => '${t.id}:${t.price}x${t.qty}').join(','),
      ].join(':'));
    }
    return parts.join('|');
  }
  ...
  void _invalidateStaleDraftTab(
    ...
    setState(() {
      tab.reset(defaultVatRate: _storeSettings.defaultVatRate, store: _storeSettings);
      tab.sellerEmployeeId = _defaultSellerEmployeeId;
      if (index == _activeTab) {
        _customerSuggestions = [];
        _syncPaidAmount();
  ...
  Future<void> _loadDraftIntoActiveTab(
    ...
    if (forceClaim) {
      ...
      if (_tableAttachStale(attachEpoch) || !ok) {
        if (mounted && !_tableAttachStale(attachEpoch)) {
          setState(() {
            if (switchingOrder) {
              _tab.reset(defaultVatRate: _storeSettings.defaultVatRate, store: _storeSettings);
              _tab.sellerEmployeeId = _defaultSellerEmployeeId;
  ...
  Future<void> _openSampleCatalog() async {
    ...
    _productGridKey.currentState?.reload();
    await _addPick(
      PosPurchaseLookupPick(product: created),
      mergeIfSame: true,
    );
  }

  final Map<String, PosProductVariant> _variantByKey = {};

  Future<PosProductVariant?> _resolveVariant(String productId, String variantId) async {
    final key = '$productId|$variantId';
    final hit = _variantByKey[key];
    ...
    for (final e in res['data'] as List) {
      if (e is! Map) continue;
      final v = PosProductVariant.fromJson(Map<String, dynamic>.from(e));
      _variantByKey['$productId|${v.id}'] = v;
    }
    return _variantByKey[key];
  }
  ...
  /// Mặc định: chỉ tiền mặt — chọn CK / cả hai bằng nút nhanh.
  ...
  void _distributePaymentsToDue() {
    ...
    final due = (_grandTotal - _tab.reservationDepositApplied)
  ...
  void _onSurchargeInputChanged(String raw, {VoidCallback? onMutate}) {
    if (!_guardReadOnlyEdit()) return;
    final v = _parseMoneyInput(raw);
    setState(() {
      _tab.surchargeInput = v;
      if (!_tab.paidManuallyEdited && !_tab.paymentsManuallyEdited) {
        _syncPaidAmount();
      } else {
        _recalcTotals();
  ...
  void _setSurchargeMode(bool isPercent, {VoidCallback? onMutate}) {
    if (!_guardReadOnlyEdit()) return;
    if (_tab.surchargeIsPercent == isPercent) return;
    setState(() {
      _tab.surchargeIsPercent = isPercent;
      _tab.surchargeInput = 0;
      _tab._surchargeCtrl.text = '0';
      if (!_tab.paidManuallyEdited && !_tab.paymentsManuallyEdited) {
        _syncPaidAmount();
      } else {
        _recalcTotals();
  ...
  void _onDeliveryFeeInputChanged(String raw, {VoidCallback? onMutate}) {
    if (!_guardReadOnlyEdit()) return;
    final v = _parseMoneyInput(raw);
    setState(() {
      _tab.deliveryFee = v;
  ...
  Widget _surchargeModeChip(String label, bool isPercent, {VoidCallback? onMutate}) {
    final selected = _tab.surchargeIsPercent == isPercent;
    return Material(
      color: selected ? PosTheme.kiotBlueLight : Colors.white,
      borderRadius: BorderRadius.circular(4),
      child: InkWell(
        onTap: () => _setSurchargeMode(isPercent, onMutate: onMutate),
        borderRadius: BorderRadius.circular(4),
        child: Container(
  ...
  Future<void> _showReturnGoodsDialog(_SellCartLine line) async {
    ...
    final ticket = kitchenVoid > 0
        ? KitchenTicketLine(
            ...
            productId: line.product.id,
            sentBefore: line.kitchenSentQty,
            lineKey: '${line.rowId}',
          )
        : null;
  ...
  Future<void> _adjustQty(_SellCartLine line, double delta) async {
    ...
    if (next > 0) {
      final err = PosQtyRules.validate(line.product, next, action: 'Đổi SL');
      if (err != null) {
  ...
  void _onCustomerSearchChanged(String q) {
    _customerSearchDebounce?.cancel();
    if (_isWalkInCustomerQuery(q) || q.trim().length < 2) {
  ...
  /// Đơn QR online: tìm/tạo khách CRM từ tên + SĐT + địa chỉ giao trên đơn.
  Future<void> _ensureOnlineOrderCustomer(PosSaleOrder order) async {
    final name = (order.customerName ?? '').trim();
    final phone =
        (order.deliveryPhone ?? order.customerPhone ?? '').replaceAll(RegExp(r'\D'), '');
    final address = (order.deliveryAddress ?? '').trim();
    if (name.isEmpty && phone.length < 8) return;

    PosCustomer? found;
    if (phone.length >= 8) {
      final res = await _api.getPosCustomers(search: phone, pageSize: 8);
      if (res['isSuccess'] == true && res['data'] is Map) {
        final items = (res['data'] as Map)['items'] as List? ?? [];
        for (final e in items) {
          if (e is! Map) continue;
          final c = PosCustomer.fromJson(Map<String, dynamic>.from(e));
          final p = (c.phone ?? '').replaceAll(RegExp(r'\D'), '');
          if (p.isNotEmpty && p == phone) {
            found = c;
            break;
          }
        }
    ...
    if (found == null && name.isNotEmpty) {
      final body = <String, dynamic>{
        'name': name,
        if (phone.length >= 8) 'phone': phone,
        if (address.isNotEmpty) 'deliveryAddress': address,
        if ((order.deliveryProvince ?? '').trim().isNotEmpty)
          'province': order.deliveryProvince!.trim(),
        if ((order.deliveryWard ?? '').trim().isNotEmpty)
          'ward': order.deliveryWard!.trim(),
        'note': 'QR online',
      };
      final res = await _api.createPosCustomer(body);
      if (res['isSuccess'] == true && res['data'] is Map) {
        found = PosCustomer.fromJson(
            Map<String, dynamic>.from(res['data'] as Map));
      }
    }
    ...
    setState(() {
      if (found != null) {
        _tab.customer = found;
        _tab._customerSearchCtrl.text = found.name;
      } else if (name.isNotEmpty) {
        _tab._customerSearchCtrl.text = name;
      }
      if (phone.length >= 8) {
        _tab.deliveryPhone = phone;
        _tab._deliveryPhoneCtrl.text = phone;
      }
      if (address.isNotEmpty) {
        _tab.deliveryAddress = address;
        _tab._deliveryAddressCtrl.text = address;
      }
      if ((order.deliveryProvince ?? '').trim().isNotEmpty) {
        _tab.deliveryProvince = order.deliveryProvince!.trim();
        _tab._deliveryProvinceCtrl.text = _tab.deliveryProvince!;
      }
      if ((order.deliveryWard ?? '').trim().isNotEmpty) {
        _tab.deliveryWard = order.deliveryWard!.trim();
        _tab._deliveryWardCtrl.text = _tab.deliveryWard!;
      }
      _tab.orderSalesChannel = order.salesChannel ?? _tab.orderSalesChannel;
    });
    _scheduleDraftAutosave();
  }
  ...
  void _selectCustomer(PosCustomer c) {
    setState(() {
      ...
      if (_sellMode == _SellMode.delivery) {
        ...
        final addr = (c.deliveryAddress ?? '').trim().isNotEmpty
            ? c.deliveryAddress
            : c.address;
        if (addr != null && addr.isNotEmpty) {
          _tab.deliveryAddress = addr;
          _tab._deliveryAddressCtrl.text = addr;
        }
        if ((c.province ?? '').trim().isNotEmpty) {
          _tab.deliveryProvince = c.province!.trim();
          _tab._deliveryProvinceCtrl.text = c.province!.trim();
        }
        if ((c.ward ?? '').trim().isNotEmpty) {
          _tab.deliveryWard = c.ward!.trim();
          _tab._deliveryWardCtrl.text = c.ward!.trim();
        }
        _tab.deliveryDistrict = null;
        _tab._deliveryDistrictCtrl.clear();
      }
    });
  }
  ...
  Future<bool> _collectSerialsBeforeCheckout() async {
    for (final line in _tab.cart) {
      final err = PosQtyRules.validate(line.product, line.qty, action: 'Thanh toán');
      if (err != null) {
        NotificationOverlayManager().showError(title: 'Số lượng', message: tr(err));
        return false;
      }
    ...
    return <String, dynamic>{
      'lines': tab.cart.map((c) {
        final line = <String, dynamic>{
          ...
          if (c.assignedEmployeeId != null && c.assignedEmployeeId!.isNotEmpty)
            'assignedEmployeeId': c.assignedEmployeeId,
          if (c.staffAssignments.isNotEmpty)
            'staffAssignments': c.staffAssignments
                .map((a) => {
                      if (a.componentProductId != null)
                        'componentProductId': a.componentProductId,
                      'assignedEmployeeId': a.assignedEmployeeId,
                      'assignedEmployeeName': a.assignedEmployeeName,
                    })
      ...
      'payments': paymentLines
          ...
          .toList(),
      'complete': complete,
      if (complete && _eInvoiceSettings.enabled) ...{
        'issueEInvoice': _eInvoiceSettings.askAtCheckout
            ? _quickIssueEInvoice
            : _eInvoiceSettings.defaultIssueAtCheckout,
        if (tab.customer != null)
          'eInvoiceBuyer': {
            'name': tab.customer!.name,
            if ((tab.customer!.taxCode ?? '').isNotEmpty)
              'taxCode': tab.customer!.taxCode,
            if ((tab.customer!.companyName ?? '').isNotEmpty)
              'companyName': tab.customer!.companyName,
            if ((tab.customer!.address ?? '').isNotEmpty)
              'address': tab.customer!.address,
            if ((tab.customer!.email ?? '').isNotEmpty)
              'email': tab.customer!.email,
            if ((tab.customer!.phone ?? '').isNotEmpty)
              'phone': tab.customer!.phone,
          },
      },
      'customerName': tab.customer?.name ??
          (!_isWalkInCustomerQuery(tab._customerSearchCtrl.text)
              ? tab._customerSearchCtrl.text.trim()
              : kPosWalkInInvoiceName),
      ...
      if (tab.splitFromOrderId == null && tab.resourceSessionId != null)
      ...
      if (identical(tab, _tab) && _sellMode == _SellMode.delivery) ...{
        ...
        if (tab.deliveryProvince != null && tab.deliveryProvince!.isNotEmpty)
          'deliveryProvince': tab.deliveryProvince,
        if (tab.deliveryDistrict != null && tab.deliveryDistrict!.isNotEmpty)
          'deliveryDistrict': tab.deliveryDistrict,
        if (tab.deliveryWard != null && tab.deliveryWard!.isNotEmpty)
          'deliveryWard': tab.deliveryWard,
      },
      'salesChannel': (tab.splitFromOrderId ?? '').isNotEmpty
          ? 'Tách bill'
          : (tab.orderSalesChannel ??
              (identical(tab, _tab)
                  ? (_sellMode == _SellMode.delivery
                      ? 'Bán giao hàng'
                      : _sellMode == _SellMode.normal
  ...
  Future<void> _openSplitBill() async {
    if (_checkingOut || _parking || _splitBillBusy) return;
    if (!_isTableOrderMode) return;
    final sessionId = _tab.resourceSessionId;
    if (sessionId == null || sessionId.isEmpty) {
      NotificationOverlayManager().showWarning(
        title: 'Không tách được',
        message: tr('Mở bàn rồi mới tách bill'),
      );
      return;
    }
    if (_tab.cart.isEmpty) return;
    setState(() => _splitBillBusy = true);
    try {
      if (!await _awaitDraftAutosaveIdle(
          busyMessage: 'Đợi lưu xong rồi tách bill')) {
        return;
      }
      final oid = _tab.draftOrderId;
      if (oid == null || oid.isEmpty) {
        NotificationOverlayManager().showWarning(
          title: 'Không tách được',
          message: tr('Đơn bàn chưa lưu — thử lại sau 1 giây'),
        );
        return;
      }
      ...
      if (orderRes['isSuccess'] != true || orderRes['data'] is! Map) {
        NotificationOverlayManager().showError(
          title: 'Không tách được',
          message: orderRes['message']?.toString() ?? 'Không tải được đơn',
        );
        return;
      }
      var order = PosSaleOrder.fromJson(
          Map<String, dynamic>.from(orderRes['data'] as Map));
      var lines = order.lines
          .where((l) => (l.id ?? '').trim().isNotEmpty)
          .toList();
      if (lines.isEmpty && _tab.cart.isNotEmpty) {
        await _persistDraftAutosave(
          forTab: _tab,
          showLockError: true,
          retryOnConflict: true,
        ...
        if (!mounted) return;
        final again = await _api.getPosSale(oid);
        if (again['isSuccess'] == true && again['data'] is Map) {
          order = PosSaleOrder.fromJson(
              Map<String, dynamic>.from(again['data'] as Map));
          lines = order.lines
              .where((l) => (l.id ?? '').trim().isNotEmpty)
              .toList();
        }
      ...
      if (lines.isEmpty && _tab.cart.isNotEmpty) {
        lines = [
          for (final c in _tab.cart)
            PosSaleOrderLine(
              productId: c.product.id,
              productName: c.product.name,
              unitName: c.unitLabel,
              qty: c.qty,
              unitPrice: c.unitPrice,
              lineTotal: c.lineTotal,
            ),
      ...
      if (lines.isEmpty) {
        NotificationOverlayManager().showWarning(
          title: 'Không tách được',
          message: tr('Đơn chưa có món đã lưu — thêm món, đợi lưu rồi tách lại'),
        );
        return;
      }
      ...
      final data = res['data'] as Map? ?? {};
      final newId = data['newSaleOrderId']?.toString();
      if (newId == null || newId.isEmpty) return;
      NotificationOverlayManager().showSuccess(
        title: 'Đã tách bill',
        message: tr('Thanh toán phần này — món còn lại giữ bàn'),
      );
      await _openDraftOrder(newId, silent: true, forceClaim: true);
      if (!mounted) return;
      final openedEmpty = _tab.cart.isEmpty;
      setState(() {
        _tab.splitFromOrderId =
            data['splitFromOrderId']?.toString() ?? oid;
        _tab.resourceSessionId = null;
        _tab.serviceResourceName =
            _tab.serviceResourceName ?? _tab.serviceAreaName;
        _floorMapVisible = false;
        _tabletPaymentStage = !openedEmpty;
      });
      if (openedEmpty) {
        NotificationOverlayManager().showWarning(
          title: 'Hóa đơn tách trống',
      ...
      if (mounted) setState(() => _splitBillBusy = false);
    }
  }

  Future<void> _onReturnTablesMenu() async {
    if (!await confirmPosOwnerPassword(context)) return;
    if (!mounted) return;
    if (_isSplitBill) {
      await _deleteCurrentSplitBill();
      return;
    }
    final busy = _floorPaneKey.currentState?.busyTableCount ?? 0;
    final canCurrent = _isTableOrderMode;
    final canAll = _useFloorAsPrimary && busy > 0;
    if (canCurrent && canAll && busy > 1) {
      final pick = await showDialog<String>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: Text(tr('Trả về bàn trống')),
          content: Text(tr('Chọn bàn hiện tại hoặc tất cả bàn đang mở.')),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(ctx),
              child: Text(tr('Huỷ')),
            ...
            TextButton(
              ...
              child: Text(tr('Tất cả ($busy)')),
            ),
            FilledButton(
              style: FilledButton.styleFrom(backgroundColor: Colors.red),
              onPressed: () => Navigator.pop(ctx, 'current'),
              child: Text(tr('Bàn hiện tại')),
            ),
          ],
        ),
      );
      if (pick == 'current') await _returnCurrentTableToEmpty();
      if (pick == 'all') {
        await _floorPaneKey.currentState
            ?.returnAllTablesToEmpty(requireOwnerPassword: false);
      }
      return;
    }
    if (canCurrent) {
      await _returnCurrentTableToEmpty();
      return;
    }
    if (canAll) {
      await _floorPaneKey.currentState
  ...
  Future<void> _deleteCurrentSplitBill() async {
    if (_checkingOut || _parking || _splitBillBusy) return;
    if (!_isSplitBill) {
      await _returnCurrentTableToEmpty();
      return;
    }
    final name = (_tab.draftOrderNo ?? '').trim().isEmpty
        ? 'hóa đơn tách'
        : _tab.draftOrderNo!;
    final lineCount = _tab.cart.length;
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        ...
        content: Text(tr(
          lineCount > 0
              ? 'Xóa hóa đơn tách ($lineCount món). Bàn gốc vẫn giữ món còn lại.'
              : 'Hóa đơn tách đang trống. Xóa hóa đơn này, bàn gốc không đổi.',
        )),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
    ...
    _suspendDraftAutosave = true;
    if (!await _awaitDraftAutosaveIdle(
        busyMessage: 'Đợi lưu xong rồi xóa hóa đơn')) {
      _suspendDraftAutosave = false;
      return;
    }

    final orderId = (_tab.draftOrderId ?? '').trim();
    if (orderId.isNotEmpty) {
      var del = await _api.cancelPosSaleDraft(orderId);
      if (del['isSuccess'] != true) {
        del = await _api.deletePosSale(orderId);
      }
      if (!mounted) return;
      if (del['isSuccess'] != true) {
        _suspendDraftAutosave = false;
        NotificationOverlayManager().showError(
          title: 'Không xóa được hóa đơn',
          message: del['message']?.toString() ?? 'Thử lại',
        );
        return;
      }
      _floorReleasedOrderIds.add(orderId);
    ...
    NotificationOverlayManager().showSuccess(
      title: 'Đã xóa hóa đơn tách',
      message: tr('Bàn gốc vẫn giữ món còn lại'),
    );
  }

  Future<void> _returnCurrentTableToEmpty() async {
    if (_checkingOut || _parking || _splitBillBusy) return;
    if (_isSplitBill) {
      await _deleteCurrentSplitBill();
      return;
    ...
    if (!_isTableOrderMode) {
      NotificationOverlayManager().showWarning(
        title: 'Chưa gắn bàn',
        message: tr('Mở bàn rồi mới trả về trống'),
      );
      return;
    }
    final name = (_tab.serviceResourceName ?? '').trim().isEmpty
        ? 'bàn'
        : _tab.serviceResourceName!;
    final lineCount = _tab.cart.length;
    final total = _grandTotal;
    final busyCart = lineCount > 0 || total > 0;
    if (busyCart) {
      final ok = await showDialog<bool>(
        context: context,
        builder: (ctx) => AlertDialog(
          title: Text(tr('Trả $name về trống?')),
          content: Text(tr(
            'Bàn còn $lineCount món'
            '${total > 0 ? ' · ${_moneyFmt.format(total)}đ' : ''}.\n'
    ...
    _suspendDraftAutosave = true;
    if (!await _awaitDraftAutosaveIdle(
        busyMessage: 'Đợi lưu xong rồi trả bàn')) {
      _suspendDraftAutosave = false;
      return;
    }
    ...
    final sid = (_tab.resourceSessionId ?? '').trim();
    final rid = (_tab.serviceResourceId ?? '').trim();
    if (sid.isNotEmpty) {
  ...
  String? _shippingCarrierCodeOrNull(String? partner) {
    final p = (partner ?? '').trim().toLowerCase();
    if (p.isEmpty) return null;
    if (p == 'ghn' || p.contains('giao hàng nhanh') || p.contains('giaohangnhanh')) {
      return 'Ghn';
    }
    if (p == 'ghtk' || p.contains('tiết kiệm') || p.contains('tietkiem')) {
      return 'Ghtk';
    }
  ...
  List<MapEntry<String, String>> _parseEnabledShippingCarriers(
    Map<String, dynamic> res,
  ) {
  ...
  List<MapEntry<String, String>> _orderShippingCarriers(
    ...
    final copy = [...list];
    copy.sort((a, b) => rank(a.key).compareTo(rank(b.key)));
    return copy;
  }

  void _ensureDeliveryPartnerSelected() {
    if (_enabledShippingCarriers.isEmpty) return;
    final cur = _shippingCarrierCodeOrNull(_tab.deliveryPartner);
    final ok = cur != null &&
        _enabledShippingCarriers.any((e) => e.key.toLowerCase() == cur.toLowerCase());
    if (ok) return;
    final first = _enabledShippingCarriers.first;
    _tab.deliveryPartner = first.key;
    _tab._deliveryPartnerCtrl.text = first.key;
  }

  Future<void> _reloadEnabledShippingCarriers() async {
    final preferAhamove = _sellProfile.usesFloorPlan;
    final res = await _api.getPosShippingEnabled();
    if (!mounted) return;
    setState(() {
      _enabledShippingCarriers = _orderShippingCarriers(
        _parseEnabledShippingCarriers(res),
        preferAhamoveFirst: preferAhamove,
      );
      if (_sellMode == _SellMode.delivery) {
        _ensureDeliveryPartnerSelected();
      }
    });
  }

  Future<void> _quoteDeliveryFee({VoidCallback? onMutate}) async {
    final code = _shippingCarrierCodeOrNull(_tab.deliveryPartner);
    if (code == null) {
      NotificationOverlayManager().showWarning(
        title: 'Chọn đối tác',
        message: _enabledShippingCarriers.isEmpty
            ? tr('Chưa bật đơn vị giao hàng — vào Thiết lập → Đơn vị giao hàng')
            : tr('Chọn đơn vị vận chuyển'),
      );
      return;
    }
    final addr = (_tab.deliveryAddress ?? '').trim();
    final phone = (_tab.deliveryPhone ?? '').trim();
    if (addr.isEmpty || phone.isEmpty) {
      NotificationOverlayManager().showWarning(
        title: 'Thiếu thông tin',
        message: tr('Nhập SĐT và địa chỉ giao hàng'),
      );
      return;
    }
    final province = (_tab.deliveryProvince ?? '').trim();
    final ward = (_tab.deliveryWard ?? '').trim();
    final fullAddr = [
      addr,
      if (ward.isNotEmpty) ward,
      if (province.isNotEmpty) province,
    ].join(', ');
    setState(() => _quotingShipFee = true);
    final res = await _api.quotePosShipping({
      'carrierCode': code,
      'toName': _tab.customer?.name ?? 'Khách',
      'toPhone': phone,
      'toAddress': fullAddr,
      'toProvince': province.isEmpty ? null : province,
      'toDistrict': _tab.deliveryDistrict ?? (ward.isEmpty ? null : ward),
      'toWard': ward.isEmpty ? null : ward,
      'weightGrams': 500,
      'codAmount': 0,
      'insuranceValue': _grandTotal,
    });
    if (!mounted) return;
    setState(() => _quotingShipFee = false);
    final data = res['data'];
    ...
    if (!ok) {
      final msg = data is Map
          ...
          : res['message'];
      NotificationOverlayManager().showError(
        title: 'Không ước tính được',
        message: '${msg ?? res}',
      );
      return;
    }
    final feeRaw = data['fee'] ?? data['Fee'] ?? 0;
    final fee = (feeRaw is num) ? feeRaw.toDouble() : double.tryParse('$feeRaw') ?? 0;
    setState(() {
      _tab.deliveryFee = fee;
      _tab._deliveryFeeCtrl.text = fee.toStringAsFixed(0);
    });
    onMutate?.call();
    NotificationOverlayManager().showSuccess(
      title: 'Phí ship',
      message: '${_moneyFmt.format(fee)} (${data['serviceName'] ?? data['ServiceName'] ?? code})',
    );
  }

  Future<void> _maybeOfferCreateShipment({
    required String? orderId,
    required String? partner,
    required String orderNo,
  }) async {
    final code = _shippingCarrierCodeOrNull(partner);
    if (orderId == null || orderId.isEmpty || code == null || !mounted) return;
    final create = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(tr('Tạo vận đơn?')),
        content: Text(tr(
            'Tạo vận đơn $code cho đơn $orderNo ngay sau thanh toán?')),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
    ...
    if (ok) {
      ...
      NotificationOverlayManager().showSuccess(
        ...
        message: tracking.toString().isEmpty
            ? code
            : 'Mã: $tracking',
      );
    } else {
  ...
  Future<void> _checkout() async {
    ...
    if (_industrySettings?.requireStaffOnService == true) {
      for (final line in _tab.cart) {
        final p = line.product;
        final combo = p.comboLines ?? const <PosComboLine>[];
        if (p.productType == PosProductType.service &&
            (line.assignedEmployeeId == null ||
                line.assignedEmployeeId!.isEmpty) &&
            line.staffAssignments.isEmpty) {
          NotificationOverlayManager().showWarning(
            title: 'Chưa chọn nhân viên',
            message: tr('«${p.name}» là dịch vụ — chọn NV để tính hoa hồng'),
          );
          await _assignLineStaff(line);
          return;
        }
        if (p.productType == PosProductType.combo) {
          for (final c in combo) {
            if (c.componentProductType.toLowerCase() != 'service') continue;
            final hit = line.staffAssignments.any(
                (a) => a.componentProductId == c.componentProductId);
            if (!hit &&
                (line.assignedEmployeeId == null ||
                    line.assignedEmployeeId!.isEmpty)) {
              NotificationOverlayManager().showWarning(
                title: 'Chưa chọn nhân viên',
                message: tr(
                    'Combo «${p.name}»: chọn NV cho «${c.componentProductName}»'),
              );
              await _assignLineStaff(line);
              return;
            }
    ...
    try {
      ...
      // Không tắt bàn F&B — lỗi mẫu sai nằm ở chỗ chọn máy (Sunmi trong), không phải chỗ này.
      final kitchenAuto = _quickPrintKitchen &&
          _printSettings.kitchenSlipPrintMode != PosKitchenSlipPrintMode.off &&
          _tab.cart.any((l) => l.kitchenPendingQty > 0);
      final kitchenAutoLines = kitchenAuto
          ? _tab.cart
              .where((l) => l.kitchenPendingQty > 0)
              .map((l) => KitchenTicketLine(
                    productName: l.product.name,
                    qty: l.kitchenPendingQty,
                    unitName: l.unitLabel,
      ...
      final kitchenAutoTable = formatPosTableLabel(
        areaName: printAreaName,
        tableName: printTableName,
      );

      // Đóng phiên + in nền — không chặn UI sau khi API TT thành công.
      if ((_tab.splitFromOrderId ?? '').isEmpty &&
          paidSessionId != null &&
          paidSessionId.isNotEmpty) {
        unawaited(() async {
          try {
      ...
      if (useFloor) {
        _markFloorResourceFreed(printResourceId);
        _scheduleFloorQuietRefresh();
      }
      _scheduleCustomerDisplayPublish();
      ...
      unawaited(_maybeOfferCreateShipment(
        orderId: shipOrderId,
        partner: shipPartner,
        orderNo: orderNo,
      ));
      ...
      if (orderId != null &&
          (wantInvoicePrint || warehouseAuto || kitchenAutoLines.isNotEmpty)) {
        ...
        // Tuần tự: hóa đơn → bếp (nếu auto) → xuất kho.
        unawaited(() async {
          if (wantInvoicePrint) {
          ...
          if (kitchenAutoLines.isNotEmpty) {
            var enqueuedPerPrinterFail = false;
            final kitchenOk = await printKitchenCompactSlip(
              tableName: kitchenAutoTable,
              isCancel: false,
              lines: kitchenAutoLines,
              senderName: _kitchenSenderName(),
              orderNo: orderNo.isNotEmpty ? orderNo : null,
              // Sau TT: không toast thêm (đã toast lúc Báo bếp nếu có).
              showFeedback: false,
              waitForCompletion: false,
              skipDedup: false,
              onCloudHang: ({
                ...
                _enqueueFailedKitchenPrint(
                  PendingKitchenPrintJob(
                    ...
                    printerId: printerId,
                    printerName: printerName,
                    errorMessage: 'Chưa in báo bếp sau TT ($printerName)',
                  ),
                );
              },
              onPrinterFailed: ({
                ...
                _enqueueFailedKitchenPrint(
                  PendingKitchenPrintJob(
                    id:
                        'kitchen_pay_fail_${printerId}_${DateTime.now().millisecondsSinceEpoch}',
                    isCancel: false,
                    tableName: kitchenAutoTable,
                    senderName: _kitchenSenderName(),
                    orderNo: orderNo,
            ...
            if (!kitchenOk && !enqueuedPerPrinterFail) {
              _enqueueFailedKitchenPrint(
                PendingKitchenPrintJob(
                  id: 'kitchen_pay_${DateTime.now().millisecondsSinceEpoch}',
                  isCancel: false,
                  tableName: kitchenAutoTable,
                  senderName: _kitchenSenderName(),
                  orderNo: orderNo,
                  lines: kitchenAutoLines,
                  errorMessage: 'In báo bếp sau TT thất bại',
                ),
              );
            }
          }
  ...
  List<PosSellStockLineDelta> _cartStockLinesFromCart() {
    ...
    for (final l in _tab.cart) {
      if (l.product.productType == PosProductType.service &&
          !l.product.hasRecipe) {
        continue;
      }
      ...
      if (l.product.hasRecipe) {
        for (final cl in l.product.recipeLines ?? const <PosComboLine>[]) {
          raw.add(
            PosSellStockLineDelta(
              productId: cl.componentProductId,
              qty: cl.qty * baseQty,
  ...
  List<PosSaleOrderLine> _warehouseSlipPendingLinesFromCart() {
    ...
    for (final l in _tab.cart) {
      ...
      if (l.product.hasRecipe) {
        lines.addAll(
          expandComboToWarehouseLines(
            combo: l.product.copyWith(comboLines: l.product.recipeLines),
            comboQty: pending,
            lineNote: l.lineNote,
          ),
        );
  ...
  /// Id job in gốc nằm trong id phiếu treo (`kitchen_hang_<jobId>`…).
  /// Hỏi server khi thu ngân bấm In lại — Agent chậm vẫn có thể đã in xong.
  String? _pendingKitchenSourceJobId(String id) {
    const prefixes = [
      'kitchen_hang_',
      'kcancel_hang_',
      'kitchen_pay_hang_',
    ...
    for (final p in prefixes) {
      ...
      final jobId = id.substring(p.length);
      return jobId.length >= 32 ? jobId : null;
    }
    return null;
  }

  /// Server đã in xong / đã hủy phiếu này → gỡ khỏi hàng chờ, không in lại.
  Future<bool> _kitchenPendingSettledOnServer(PendingKitchenPrintJob job) async {
    final jobId = _pendingKitchenSourceJobId(job.id);
    if (jobId == null) return false;
    try {
      final res = await _api.getPosPrintJob(jobId);
      if (res['isSuccess'] != true || res['data'] is! Map) return false;
      final data = res['data'] as Map;
      final status = data['status']?.toString() ?? '';
      if (status != 'Completed' && status != 'Cancelled') return false;
      // Hủy vì Agent treo / hết hạn hàng đợi = chưa ra giấy → vẫn phải in lại.
      final code = (data['errorCode']?.toString() ?? '').toUpperCase();
      if (status == 'Cancelled' &&
          (code == 'STUCK_NO_REQUEUE' ||
              code == 'STALE_QUEUED' ||
              code == 'MAX_ATTEMPTS')) {
        return false;
      }
      debugPrint('Kitchen pending: bỏ ${job.id} — server báo $status');
      return true;
    } catch (_) {
      // Mất mạng: giữ phiếu treo, lần sau hỏi lại.
      return false;
    }
  }
  ...
  Future<void> _openPendingPrintQueue() async {
    ...
    if (printers.isEmpty) {
      try {
        ...
        debugPrint('pending print load printers: ');
      }
    }
    if (!mounted) return;
    if (printers.isEmpty) {
    ...
    await showPendingWarehousePrintSheet(
      ...
      onRetrySale: (job, {overridePrinterId, overridePrinter}) async {
        return printPosSaleOrder(
          ...
          skipDedup: true,
          preferDevicePrintOnly:
              overridePrinter == null && (overridePrinterId ?? '').isEmpty,
          showFeedback: true,
      ...
      onRetryKitchen: (job, {overridePrinterId, overridePrinter}) async {
        final userPicked = overridePrinter != null ||
            (overridePrinterId ?? '').trim().isNotEmpty;
        // Phiếu treo dựng sau 60s, Agent chậm vẫn in sau đó. Bấm in lại 4–5
        // phiếu treo kiểu này là bếp nhận cả chồng phiếu trùng — hỏi server
        // trước, đã ra giấy thì chỉ gỡ khỏi hàng chờ.
        if (!userPicked && await _kitchenPendingSettledOnServer(job)) {
          if (!mounted) return true;
          _removeFailedKitchenPrint(job);
          NotificationOverlayManager().showInfo(
            title: 'Phiếu đã in rồi',
            message: tr(
              '${job.tableName}: máy in đã in phiếu này — bỏ khỏi hàng chờ',
            ),
            relatedEntityType: kPosPrintNotifyKind,
          );
          return true;
        }
        // In lại cùng máy treo: vẫn ép override để không fan-out lại về máy lỗi
        // rồi dồn default. User chọn máy khác: ép máy đó (không cần gán SP).
        final pickId = userPicked
            ? (overridePrinterId ?? overridePrinter?.id)
            : (job.printerId ?? '').trim().isNotEmpty
                ? job.printerId
                : null;
  ...
  Future<void> _maybePrintOrder(
    ...
    try {
      ...
      final PosSaleOrder saleForHang = order!;
      final ok = await printPosSaleOrder(
        context: context,
        order: order,
        ...
        mergeSameItems: _printSettings.mergeSameItems,
        copies: 1, // Sau TT luôn 1 liên; in lại / cài đặt copies dùng nút in tay
        templateId: _printSettings.templateId,
        ...
        onCloudHang: ({
          ...
          _enqueueFailedSalePrint(
            saleForHang,
            errorMessage: 'Chưa in sau 30s ($printerName)',
          );
          NotificationOverlayManager().showWarning(
            title: 'Hóa đơn treo',
            message: tr(
  ...
  Future<void> _openPrintSettings() async {
    if (Responsive.isMobile(context)) {
      ...
      if (result != null && mounted) {
        ...
        PosPrintConfigSession.instance.invalidate(warehouseTemplateOnly: false);
  ...
  Widget _buildVietQrPaymentSection({
    ...
    final qr = _resolveTransferPaymentQr();
    if (qr == null) return const SizedBox.shrink();
    if (!_storeSettings.showVietQrAtPayment && !qr.isTingee) {
      return const SizedBox.shrink();
    }

    final panelTitle =
        qr.isTingee ? 'Tingee QR thanh toán' : 'VietQR thanh toán';
    final dialogTitle = qr.isTingee
        ? 'Quét Tingee QR thanh toán'
  ...
  String? _buildVietQrImageUrlForOrder(PosSaleOrder order) {
    ...
    return PosVietQrHelper.qrImageUrlForOrder(
      order,
  ...
  Future<void> _openPosMenu() async {
    ...
    final menuMaxH = MediaQuery.sizeOf(context).height - topRight.dy - 16;
    final action = await showMenu<String>(
      ...
      constraints: BoxConstraints(
        minWidth: 240,
        maxWidth: 300,
        maxHeight: menuMaxH.clamp(280, 900),
      ),
      items: [
        ...
        if (_isTableOrderMode &&
          ...
          PopupMenuItem(
            value: 'split_bill',
            enabled: !_splitBillBusy && !_checkingOut && !_parking,
            child: ListTile(
              dense: true,
        ...
        if (_isSplitBill)
          PopupMenuItem(
            value: 'delete_split',
            enabled: !_checkingOut && !_parking && !_splitBillBusy,
            child: ListTile(
              dense: true,
              leading: Icon(Icons.delete_outline,
                  size: 20, color: Colors.red.shade700),
              title: Text(tr('Xóa hóa đơn tách')),
              contentPadding: EdgeInsets.zero,
            ),
          ),
        if (!_isSplitBill &&
            (_isTableOrderMode ||
                (_useFloorAsPrimary &&
                    (_floorPaneKey.currentState?.busyTableCount ?? 0) > 0)))
          PopupMenuItem(
            value: 'return_table',
            enabled: !_checkingOut && !_parking && !_splitBillBusy,
            child: ListTile(
        ...
        PopupMenuItem(
          ...
          child: ListTile(
            dense: true,
            leading: Icon(
              _isPosFullscreen ? Icons.fullscreen_exit : Icons.fullscreen,
              size: 20,
            ),
    ...
    switch (action) {
      ...
      case 'split_bill':
        await _openSplitBill();
      case 'delete_split':
        await _deleteCurrentSplitBill();
      case 'return_table':
        await _onReturnTablesMenu();
      case 'toggle_merge':
      ...
      case 'more_hub':
        NavigationNotifier.posHubTab.value = 4;
        ...
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => const PosHubScope(
              embeddedInHub: false,
              pushedSubPage: true,
              child: PosPaymentGatewaySettingsScreen(),
            ),
          ),
        );
      case 'kds':
        ...
        await Navigator.of(context).push(
          MaterialPageRoute(
            builder: (_) => const PosHubScope(
              embeddedInHub: false,
              pushedSubPage: true,
              child: PosPrinterSettingsHubScreen(),
            ),
          ),
        );
  ...
  Widget build(BuildContext context) {
    ...
    if (!canSell) {
      return Scaffold(
        body: Center(child: Text(tr('Bạn không có quyền truy cập màn hình bán hàng'))),
      );
    }
      ...
      body: Builder(
        builder: (context) {
          final body = LayoutBuilder(
            ...
            // Mọi ngành / không-mobile: trái catalog (hoặc sơ đồ), phải hóa đơn.
            return _buildUnifiedSplitFlow(perm);
          },
          );
  ...
  Widget _buildFloorMenuModeTabs({bool compact = false}) {
    ...
    final floorIcon = switch (_sellProfile) {
      PosSellProfile.salon => Icons.event_seat_outlined,
    ...
    final catalogIcon = switch (_sellProfile) {
      PosSellProfile.restaurant => Icons.restaurant_menu_outlined,
      ...
      child: Row(
        ...
        children: [
          ...
          tab(
            selected: !onFloor,
            icon: catalogIcon,
            label: _sellProfile.catalogTabLabel,
            onTap: () {
              if (!onFloor) return;
              setState(() {
                _floorMapVisible = false;
    ...
    final bar = DecoratedBox(
      ...
      child: SizedBox(
        ...
        child: PosPurchaseProductSearchBar(
          ...
          hintText: tr(_sellProfile.searchHint),
  ...
  Widget _buildTopBar({bool desktopChrome = false}) {
    ...
    final onFloor = floorPrimary && _floorMapVisible;
  ...
  Widget _buildDesktopInvoiceTabsCompact() {
    if (!_showRetailInvoiceTabs) {
      return ConstrainedBox(
        ...
        child: Text(
          tr(_tab.serviceResourceName ?? 'Đơn bàn'),
          maxLines: 1,
          overflow: TextOverflow.ellipsis,
          style: const TextStyle(
            color: Colors.white,
  ...
  /// Insert [gap] between top-bar action widgets.
  List<Widget> _spacedTopBarActions(List<Widget?> actions) {
    final out = <Widget>[];
    for (final w in actions) {
      if (w == null) continue;
      if (out.isNotEmpty) out.add(_KiotLayout.topBarActionGap);
      out.add(w);
    }
    return out;
  }
  ...
  Widget _buildKiotCartList() {
    final Widget body;
    if (_tableAttachBusy && _tab.cart.isEmpty) {
      body = const Center(
        child: CircularProgressIndicator(color: PosTheme.kiotBlue),
      );
    } else if (_tab.cart.isEmpty) {
      body = PosEmptyCartBrand(
        hint: tr(_isSplitBill
            ? 'Hóa đơn tách chưa có món — thêm từ thực đơn hoặc xóa hóa đơn'
            : _sellProfile.emptyCartHint),
        below: _isSplitBill
            ? OutlinedButton.icon(
                onPressed: (_checkingOut || _parking || _splitBillBusy)
                    ? null
                    : () => unawaited(_deleteCurrentSplitBill()),
                icon: const Icon(Icons.delete_outline),
                label: Text(tr('Xóa hóa đơn tách')),
      ...
      body = Scrollbar(
            thumbVisibility: _tab.cart.length > 5,
  ...
  Widget _buildKiotCartRow(_SellCartLine line, int cartIndex) {
    ...
    final meta = <Widget>[];
    if (!noteExpanded && line.lineNote != null && line.lineNote!.isNotEmpty) {
      meta.add(Text(
        tr('Ghi chú: ${line.lineNote}'),
        maxLines: 2,
        overflow: TextOverflow.ellipsis,
        style: const TextStyle(fontSize: 12, height: 1.25, color: _kiotBlue),
      ));
    }
    ...
    if (!noteExpanded &&
        line.product.productType == PosProductType.combo &&
        line.product.showComboComponentsOnSell &&
        (line.product.comboLines?.isNotEmpty ?? false)) {
      meta.add(Text(
        tr('+ ${formatComboComponentsSubtitle(line.product.comboLines!, comboQty: line.qty)}'),
        maxLines: 2,
        overflow: TextOverflow.ellipsis,
    ...
    // Một hàng: Xóa | Tên | − SL + | ĐVT | Thành tiền (giống app POS cảm ứng).
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Material(
              ...
              children: [
                Row(
                  ...
                  children: [
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              if (_lineNeedsStaffPick(line))
                                InkWell(
                                  onTap: () => unawaited(_assignLineStaff(line)),
                                  child: Padding(
                                    padding: const EdgeInsets.only(top: 2),
                                    child: Text(
                                      tr(_staffLabelForLine(line)?.isNotEmpty == true
                                          ? 'NV: ${_staffLabelForLine(line)}'
                                          : 'Chọn NV hoa hồng'),
                                      style: TextStyle(
                                        fontSize: 11,
                    ...
                    const SizedBox(width: 4),
                    // Cột SL cố định — không lệch theo độ dài tên / tiền.
                    SizedBox(
                      width: _KiotLayout.qtyHit * 2 + 32,
                      height: _KiotLayout.qtyHit,
  ...
  Widget _buildCartLineNoteEditor(_SellCartLine line) {
    final key = _cartNoteEditorKeys.putIfAbsent(line.rowId, GlobalKey.new);
    return KeyedSubtree(
      key: key,
      child: Listener(
        ...
        child: Column(
          ...
          children: [
            PosLineQuickNotesPicker(
              quickNotes: _quickNotesFor(line),
            ...
            if (line.product.allowToppings &&
              ...
              const SizedBox(height: 6),
              ...line.product.toppingOptions.map((o) {
                final current = line.toppings
                    .where((t) => t.id == o.toppingProductId)
                    .firstOrNull;
                return _toppingQtyRow(
                  name: o.toppingProductName,
                  unitPrice: o.extraPrice,
                  qty: current?.safeQty ?? 0,
                  onChanged: (next) => _setLineToppingQty(
                    line,
            ...
            if (line.product.hasToppingGroups) ...[
              ...
              if (line.toppings.isNotEmpty)
                Text(
                  tr(line.toppings
                      .map((t) =>
                          '↳ ${posToppingNameWithQty(t.name, t.safeQty)} (+${_moneyFmt.format(t.price * t.safeQty)})')
                      .join('\n')),
                  style: const TextStyle(
                    fontSize: 12,
        ...
        children: [
          ...
          Padding(
            ...
            child: _buildCheckoutActions(
              ...
              busy: _checkingOut || _parking || _provisionalPrinting,
          ...
          child: Padding(
            ...
            child: Row(
              ...
              children: [
                ...
                const Icon(Icons.expand_more_rounded, size: 16, color: _kiotBlue),
              ],
            ),
          ),
        ),
      ...
      children: [
            ...
            children: [
                  ...
                  decoration: InputDecoration(
                    ...
                    suffixIcon: (_tab.customer != null ||
                        ...
                        ? IconButton(
                            ...
                            onPressed: () => setState(() {
                              _tab.customer = null;
                              _tab._customerSearchCtrl.text =
                                  kPosWalkInCustomerLabel;
                              _customerSuggestions = [];
                              _tab.pointsToRedeem = 0;
                              _tab.pointsDiscount = 0;
                              _tab._pointsCtrl.clear();
              ...
              if (_tab.customer != null)
                IconButton(
                  ...
                  icon: const Icon(Icons.history, size: 20, color: _kiotBlue),
                  onPressed: _openSessionRedeem,
                ),
              IconButton(
        ...
        if (_storeSettings.enableSurcharge) ...[
          ...
          Row(
            ...
            children: [
              SizedBox(
                width: 82,
                child: Text(tr(_storeSettings.surchargeDisplayName),
                    style: TextStyle(fontSize: 12)),
              ),
              _surchargeModeChip('%', true, onMutate: onMutate),
              const SizedBox(width: 4),
              _surchargeModeChip('đ', false, onMutate: onMutate),
              const SizedBox(width: 8),
              Expanded(
                child: PosNoSoftKeyboardField(
                  controller: _tab._surchargeCtrl,
                  enabled: !_tab.draftReadOnly,
                  allowDecimal: true,
                  textAlign: TextAlign.right,
                  keypadTitle: _storeSettings.surchargeDisplayName,
                  decoration: InputDecoration(
                    isDense: true,
                    filled: true,
                    fillColor: Colors.white,
                    suffixText: tr(_tab.surchargeIsPercent ? '%' : 'đ'),
                    border: const OutlineInputBorder(),
                    contentPadding:
                        const EdgeInsets.symmetric(horizontal: 8, vertical: 8),
                  ),
                  style: const TextStyle(fontSize: 13),
                  onChanged: (v) =>
                      _onSurchargeInputChanged(v, onMutate: onMutate),
                  onSubmitted: (_) => _scheduleDraftAutosave(),
                ),
              ),
            ],
          ),
          if (_tab.surchargeAmount > 0)
            Padding(
              padding: const EdgeInsets.only(top: 4),
              child: Text(
                tr('${_storeSettings.surchargeDisplayName}: +${_moneyFmt.format(_tab.surchargeAmount)}'),
                textAlign: TextAlign.right,
                style: TextStyle(fontSize: 12, color: Colors.orange.shade800),
              ),
            ),
        ],
        if (_storeSettings.enableDeliveryFee) ...[
          const SizedBox(height: 8),
          Row(
            crossAxisAlignment: CrossAxisAlignment.center,
            children: [
              SizedBox(
                width: 82,
                child: Text(tr('Phí GH'), style: TextStyle(fontSize: 12)),
              ),
              Expanded(
                child: PosNoSoftKeyboardField(
                  controller: _tab._deliveryFeeCtrl,
                  enabled: !_tab.draftReadOnly,
                  allowDecimal: false,
                  textAlign: TextAlign.right,
                  keypadTitle: 'Phí giao hàng',
                  decoration: InputDecoration(
                    isDense: true,
                    filled: true,
                    fillColor: Colors.white,
                  ...
                  style: const TextStyle(fontSize: 13),
                  onChanged: (v) =>
                      _onDeliveryFeeInputChanged(v, onMutate: onMutate),
                  onSubmitted: (_) => _scheduleDraftAutosave(),
                ),
              ),
            ],
        ...
        if (_sellMode == _SellMode.delivery) ...[
          ...
          VnAdminAddressFields(
            provinceCtrl: _tab._deliveryProvinceCtrl,
            wardCtrl: _tab._deliveryWardCtrl,
            dense: true,
            onChanged: () {
              _tab.deliveryProvince =
                  _tab._deliveryProvinceCtrl.text.trim().isEmpty
                      ? null
                      : _tab._deliveryProvinceCtrl.text.trim();
              _tab.deliveryWard = _tab._deliveryWardCtrl.text.trim().isEmpty
                  ? null
                  : _tab._deliveryWardCtrl.text.trim();
              // Cấu trúc hành chính mới: không còn quận/huyện riêng.
              _tab.deliveryDistrict = null;
              _tab._deliveryDistrictCtrl.clear();
              onMutate?.call();
            },
          ...
          const SizedBox(height: 6),
          if (_enabledShippingCarriers.isEmpty)
            Padding(
              padding: const EdgeInsets.only(bottom: 6),
              child: Text(
                tr('Chưa bật đơn vị giao hàng — Thiết lập → Đơn vị giao hàng'),
                style: TextStyle(fontSize: 12, color: Colors.orange.shade800),
              ),
            )
          else
            DropdownButtonFormField<String>(
              value: () {
                ...
                final hit = _enabledShippingCarriers.any(
                    (e) => e.key.toLowerCase() == cur.toLowerCase());
                return hit ? cur : null;
              }(),
              isExpanded: true,
              decoration: InputDecoration(
                hintText: tr('Đối tác giao hàng'),
                isDense: true,
                border: OutlineInputBorder(),
                contentPadding: EdgeInsets.symmetric(horizontal: 10, vertical: 8),
              ),
              items: [
                for (final e in _enabledShippingCarriers)
                  DropdownMenuItem(value: e.key, child: Text(e.value)),
              ],
              onChanged: (v) {
                setState(() {
                  _tab.deliveryPartner = v;
                  _tab._deliveryPartnerCtrl.text = v ?? '';
                });
                onMutate?.call();
              },
            ),
          ...
          Align(
            alignment: Alignment.centerLeft,
            child: TextButton.icon(
              onPressed: _quotingShipFee || _enabledShippingCarriers.isEmpty
                  ? null
                  : () => _quoteDeliveryFee(onMutate: onMutate),
              icon: _quotingShipFee
                  ? const SizedBox(
                      width: 14,
                      height: 14,
              ...
              label: Text(tr('Ước tính phí ship')),
            ),
          ),
        ],
        const SizedBox(height: 8),
        ...
        if (_tab.reservationDepositApplied > 0)
          _summaryRow(
            'Cọc đặt bàn',
            '-${_moneyFmt.format(_tab.reservationDepositApplied)}',
          ),
        _summaryRow(
          'Khách cần trả',
          _moneyFmt.format(_amountAfterDeposit),
          bold: true,
          blue: true,
        ),
  ...
  String? get _transferSourceKey {
    final preferTingee =
        PosPaymentGatewayApi.preferTingee(_paymentGatewaySettings);
    if (preferTingee) {
      for (final s in _paymentSources) {
        if (s.key == 'tingee') return s.key;
      }
    } else {
      for (final s in _paymentSources) {
        if (s.key.startsWith('vietqr_')) return s.key;
      }
    }
    for (final s in _paymentSources) {
      if (s.key != _PosPaymentSource.cashKey) return s.key;
    }
    return null;
  }

  String get _payQuickMode {
    final keys = _tab.paymentLines.map((l) => l.sourceKey).toSet();
    final hasCash = keys.contains(_PosPaymentSource.cashKey);
    final hasBank = keys.any((k) => k != _PosPaymentSource.cashKey);
    if (hasCash && hasBank) return 'mixed';
    if (hasBank) return 'transfer';
    return 'cash';
  }

  _SellPaymentLine _payLine(String key, double amount) {
    final line = _SellPaymentLine(sourceKey: key);
    line.amount = amount;
    line.amountCtrl.text = amount > 0 ? _moneyFmt.format(amount) : '';
    return line;
  }

  void _applyPayQuickMode(String mode, {VoidCallback? onMutate}) {
    final due = (_grandTotal - _tab.reservationDepositApplied)
        .clamp(0.0, double.infinity);
    final bankKey = _transferSourceKey;
    setState(() {
      _tab.paymentsManuallyEdited = true;
      for (final p in _tab.paymentLines) {
        p.dispose();
      }
      _tab.paymentLines.clear();
      if (mode == 'transfer' && bankKey != null) {
        _tab.paymentLines.add(_payLine(bankKey, due));
      } else if (mode == 'mixed' && bankKey != null) {
        _tab.paymentLines.add(_payLine(_PosPaymentSource.cashKey, 0));
        _tab.paymentLines.add(_payLine(bankKey, due));
      } else {
        _tab.paymentLines.add(_payLine(_PosPaymentSource.cashKey, due));
      }
    });
    _notifyPaymentUi(onMutate);
  }

  Widget _buildPaymentQuickPicks({VoidCallback? onMutate}) {
    final mode = _payQuickMode;
    final hasBank = _transferSourceKey != null;
    Widget btn(String id, String label, IconData icon) {
      final selected = mode == id;
      final enabled = id == 'cash' || hasBank;
      return Expanded(
        child: Padding(
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        Text(tr('Khách thanh toán'),
            style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
        const SizedBox(height: 6),
        Row(
          children: [
            btn('cash', 'Tiền mặt', Icons.payments_outlined),
            btn('transfer', 'Chuyển khoản', Icons.account_balance_outlined),
            btn('mixed', 'Tiền mặt + CK', Icons.swap_horiz),
          ],
        ),
      ],
    );
    ...
    return Column(
      ...
      children: [
        Row(
          children: [
            Expanded(
              child: Text(tr('Số tiền từng nguồn'),
  ...
  void _onPaymentLineAmountChanged(
    ...
    setState(() {
      ...
      if (_payQuickMode == 'mixed' &&
          _tab.paymentLines.length >= 2 &&
          identical(pay, _tab.paymentLines.first)) {
        final due = (_grandTotal - _tab.reservationDepositApplied)
            .clamp(0.0, double.infinity);
        final rest = (due - pay.amount).clamp(0.0, due);
        final bank = _tab.paymentLines[1];
        bank.amount = rest;
        bank.amountCtrl.text = rest > 0 ? _moneyFmt.format(rest) : '';
      }
    });
    _notifyPaymentUi(onMutate);
  }
  ...
  Widget _qtyControls(_SellCartLine line) {
    ...
    Widget qtyBtn({
      ...
      // Ink + BoxDecoration: màu luôn hiện (tránh Material/InkWell web chỉ
      // "lên màu" sau khi bấm do surfaceTint / nested Material).
      return Material(
        ...
        child: InkWell(
          ...
          child: Ink(
            ...
            decoration: BoxDecoration(
              color: bg,
              borderRadius: BorderRadius.circular(10),
              border: Border.all(color: border ?? bg, width: 1),
            ),
    ...
    return Row(
      ...
      children: [
        ...
        SizedBox(
          ...
          height: hit,
          child: InkWell(
            onTap: () => unawaited(_editLineQty(line)),
    ...
    Widget parkBtn() {
      return OutlinedButton(
        ...
        style: OutlinedButton.styleFrom(
          ...
          overlayColor: WidgetStateProperty.resolveWith((states) {
            if (states.contains(WidgetState.pressed)) {
              return _kiotBlue.withValues(alpha: 0.18);
            }
            return null;
          }),
        ),
  ...
  /// Chip bật/tắt in hóa đơn / tem / bếp khi thanh toán (chỉ lần TT này).
  Widget _buildCheckoutQuickPrintChips({VoidCallback? onChanged}) {
    ...
    final showKitchenChip =
        _printSettings.kitchenSlipPrintMode != PosKitchenSlipPrintMode.off;
    final showCupChip = _printSettings.cupLabelPrintMode.enabled;
          ...
          children: [
            ...
            if (showKitchenChip) ...[
              ...
              chip(
                label: 'In bếp',
                icon: Icons.soup_kitchen_outlined,
                selected: _quickPrintKitchen,
                onSelected: (v) {
                  setState(() => _quickPrintKitchen = v);
                  onChanged?.call();
                },
              ),
            ],
            if (_eInvoiceSettings.showCheckoutChip) ...[
              const SizedBox(width: 12),
              chip(
                label: 'Xuất HĐĐT',
                icon: Icons.request_quote_outlined,
                selected: _quickIssueEInvoice,
                onSelected: (v) {
                  setState(() => _quickIssueEInvoice = v);
                  onChanged?.call();
                },
              ),
            ],
  ...
  Widget _buildMobileShell(PermissionProvider perm, bool isNormal) {
    ...
    return Stack(
      children: [
        ...
        if (_mobileProductPickerOpen)
          Positioned.fill(child: _buildMobileProductPickerOverlay()),
      ],
    );
  }
    ...
    return Material(
      ...
      child: SafeArea(
        child: Column(
          ...
          children: [
            AppBar(
              title: Text(tr(_sellProfile.pickCatalogLabel)),
              backgroundColor: Colors.white,
                  ...
                  child: Row(
                    children: [
                      ...
                      FilledButton(
                        ...
                        child: Text(
                          tr(_isTableOrderMode
                              ? 'Thêm vào ${_sellProfile.resourceNoun}'
                              : _sellProfile.pickCatalogLabel),
  ...
  Future<void> _pickerDraftSetQty(PosProduct product, double qty) async {
    ...
    var views = await loadPosSellUnitViews(_api, pick.product);
    views = applyPosPriceListToViews(
        views, pick.product, _currentPriceOverrides);
    if (!mounted || views.isEmpty) return;
    final view = pickUnitView(
          views,
          variantId: pick.variantId,
  ...
  Widget _buildDesktopProductPane() {
    final overrides = _currentPriceOverrides;
    final sig =
        '${_storeId ?? ''}|${_allowNegativeStock ? 1 : 0}|${_tab.priceListId ?? ''}|${identityHashCode(overrides)}';
    if (_cachedDesktopProductPane != null && _cachedProductPaneSig == sig) {
      return _cachedDesktopProductPane!;
    }
    _cachedProductPaneSig = sig;
    _cachedDesktopProductPane = RepaintBoundary(
      child: PosSellProductGrid(
        key: _productGridKey,
        api: _api,
        storeId: _storeId,
  ...
  void _onFloorSelect(Map<String, dynamic> result) {
    unawaited(_attachFloorResult(result));
  }
  ...
  Widget _buildDesktopFloorPane() {
    final sig = [
      ...
      _floorReleasedOrderIds.join(','),
      _floorFreedResourceIds.join(','),
    ].join('|');
    if (_cachedDesktopFloorPane != null && _cachedFloorPaneSig == sig) {
      return _cachedDesktopFloorPane!;
    }
    _cachedFloorPaneSig = sig;
    _cachedDesktopFloorPane = PosResourceFloorScreen(
      key: _floorPaneKey,
      embedded: true,
      showAppBar: false,
      manageMode: false,
  ...
  // ─── Tablet/desktop mọi ngành: trái = hàng hóa (hoặc sơ đồ F&B), phải = giỏ ───

  Widget _buildUnifiedSplitFlow(PermissionProvider perm) {
    if (_tabletPaymentStage) return _buildTabletPaymentStage(perm);
    return _buildTabletSplitStage(perm);
  }

  /// Alias tương thích — cùng [_buildUnifiedSplitFlow].
  Widget _buildTabletFnbFlow(PermissionProvider perm) =>
      _buildUnifiedSplitFlow(perm);

  /// 2 cột 6:4 — trái catalog/sơ đồ; phải giỏ + CTA thanh toán dưới.
  Widget _buildKeepAliveLeftPane() {
    if (!_useFloorAsPrimary) return _buildDesktopProductPane();
    final showFloor = _floorMapVisible;
    return Stack(
      fit: StackFit.expand,
      children: [
        if (_floorPaneKept)
          TickerMode(
            ...
            child: Offstage(
              ...
              child: IgnorePointer(
                ignoring: !showFloor,
                child: RepaintBoundary(child: _buildDesktopFloorPane()),
              ),
            ),
          ),
        if (_catalogPaneKept)
          TickerMode(
            ...
            child: Offstage(
              ...
              child: IgnorePointer(
                ignoring: showFloor,
                child: _buildDesktopProductPane(),
              ),
            ),
          ),
      ],
  ...
  Widget _buildTabletSplitStage(PermissionProvider perm) {
    final canPay = perm.canPosPay();
    final left = _buildKeepAliveLeftPane();
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
          ...
          children: [
            ...
            Expanded(
              child: Padding(
                ...
                child: _buildFloorOpsTotalsColumn(compact: compact),
              ),
            ),
            ..._spacedTopBarActions([
  ...
  Widget _buildFloorActiveTotalsChip() {
    return ValueListenableBuilder<(double, int)>(
      valueListenable: _floorActiveTotals,
      builder: (_, v, __) {
        return ConstrainedBox(
          constraints: const BoxConstraints(maxWidth: 140),
          child: Text(
            tr('${_moneyFmt.format(v.$1)}đ · ${v.$2} ${_sellProfile.resourceNounPlural}'),
            maxLines: 1,
            overflow: TextOverflow.ellipsis,
            style: const TextStyle(
              color: Colors.white,
  ...
  Widget _buildFloorOpsTotalsColumn({required bool compact}) {
    return ValueListenableBuilder<(double, int)>(
      valueListenable: _floorActiveTotals,
      builder: (_, v, __) {
        final totalLabel = 'Tổng ${_moneyFmt.format(v.$1)}đ';
        final openLabel = v.$2 > 0
            ? '${v.$2} ${_sellProfile.resourceNounPlural}'
            : '0 ${_sellProfile.resourceNounPlural}';
        return Column(
          mainAxisAlignment: MainAxisAlignment.center,
          crossAxisAlignment:
  ...
  void _onFloorActiveTotalsChanged(double subtotal, int openCount) {
    if (!mounted) return;
    final cur = _floorActiveTotals.value;
    if ((subtotal - cur.$1).abs() < 0.009 && openCount == cur.$2) {
      return;
    }
    _floorActiveTotals.value = (subtotal, openCount);
  }
  ...
  Widget _buildTabletOrderCartHeader() {
    ...
    final title = tableLabel.isNotEmpty
        ...
        : (_floorMapVisible && _useFloorAsPrimary
            ? 'Chọn ${_sellProfile.resourceNoun} để bán'
            : _sellProfile.invoiceTitle);
    return Container(
      height: 44,
      ...
      child: Row(
        children: [
          ...
          if (_useFloorAsPrimary && _isTableOrderMode)
            TextButton.icon(
              onPressed: () => unawaited(_returnToFloorMap()),
              icon: const Icon(Icons.logout_outlined, size: 16),
              label: Text(
                tr(_sellProfile.resourceNoun.isEmpty
                    ? 'Thoát'
                    : 'Thoát ${_sellProfile.resourceNoun}'),
                style: const TextStyle(fontSize: 12),
              ),
              style: TextButton.styleFrom(
  ...
  /// Chân hóa đơn: Xuất kho / Thông báo bếp + Thanh toán (Tạm tính ở màn thanh toán).
  /// Khung chữ cố định; số món / phiếu báo / tổng tiền / ghi chú cập nhật khi load.
      ...
      child: Column(
        ...
        children: [
          ...
          _buildInvoiceActionButtons(
            canPay: canPay,
            canOrder: canOrder,
            payLabel: 'Thanh toán (F9)',
            height: 52,
            onPay: _enterPaymentStage,
          ),
        ],
      ),
    );
  ...
  bool get _showWarehouseAction => _sellProfile.usesWarehouseSlip;

  bool get _showKitchenAction =>
      _isTableOrderMode && _sellProfile.usesKitchenNotify;

  Widget _invoiceSecondaryButton({
    required String label,
    String suffix = '',
    required VoidCallback? onPressed,
    required Color color,
    bool busy = false,
    bool keepLabelWhenBusy = false,
    double height = 48,
  }) {
    return OutlinedButton(
      onPressed: () {
  ...
  /// Hàng nút dưới hóa đơn: Xuất kho · Thông báo · Thanh toán.
  Widget _buildInvoiceActionButtons({
    required bool canPay,
    required bool canOrder,
    required VoidCallback onPay,
    String payLabel = 'Thanh toán',
    double height = 52,
    bool payIsPrimaryFilled = true,
  }) {
    final cartOk = _tab.cart.isNotEmpty;
    final busy = _checkingOut || _parking;
    final showWh = _showWarehouseAction;
    final showKitchen = _showKitchenAction;
    final notifyBusy = _kitchenSending || busy;
    final canNotify = showKitchen && _kitchenActionCount > 0 && !notifyBusy && canOrder;

    final secondaries = <Widget>[
      if (showWh)
        _invoiceSecondaryButton(
          label: _warehousePrinting
              ? 'Đang gửi…'
              : _hasWarehouseSlipPending()
                  ? 'Xuất kho'
                  : 'Đã báo kho',
          color: const Color(0xFF0F766E),
          busy: _warehousePrinting,
          height: height,
          onPressed: cartOk && !_warehousePrinting && !busy
              ? _onWarehouseSlipButtonTap
              : null,
        ),
        if (showKitchen)
        _invoiceSecondaryButton(
          label: 'Thông báo',
          suffix: _kitchenActionCount > 0 ? ' ($_kitchenActionCount)' : '',
          color: PosTheme.kiotBlue,
          busy: _kitchenSending,
          keepLabelWhenBusy: true,
          height: height,
          onPressed: canNotify
    ...
    final payEnabled = cartOk && canPay && !busy;
    ...
    if (secondaries.isEmpty) {
      return SizedBox(height: height, child: payBtn);
    }
    ...
    return Row(
      children: [
        for (var i = 0; i < secondaries.length; i++) ...[
          ...
          Expanded(flex: 2, child: secondaries[i]),
        ],
        const SizedBox(width: 8),
        Expanded(flex: 3, child: payBtn),
      ],
    );
  }
  ...
  Widget _buildTabletPaymentStage(PermissionProvider perm) {
    ...
    final body = Column(
      ...
      children: [
                        ...
                        child: Column(
                          ...
                          children: [
                            Container(
                              ...
                              child: Text(
                                tr('${_sellProfile.invoiceTitle} · ${_lineCountText()} · ${_moneyFmt.format(_grandTotal)}đ'),
                                style: const TextStyle(
                                  fontWeight: FontWeight.w700,
                                  fontSize: 13,
                                  color: PosTheme.textPrimary,
        ...
        Container(
          ...
          child: _tab.cart.isEmpty && _isSplitBill
              ? Row(
                  children: [
                    Expanded(
    ...
    if (!_shouldShowPaymentQrOverlay()) return body;
    return Stack(
      fit: StackFit.expand,
      children: [
        body,
        Positioned.fill(child: _buildPaymentQrWaitingOverlay()),
      ],
    );
  }
          ...
          children: [
            Expanded(
              ...
              child: FilledButton.icon(
                ...
                label: Text(
                  tr(_sellProfile.pickCatalogLabel),
                  style: const TextStyle(fontSize: 14),
                ),
                style: FilledButton.styleFrom(
            ...
            if (tableMode) ...[
              ...
              if (_printSettings.showCupLabelManualButton) ...[
                ...
                Expanded(
                  child: OutlinedButton.icon(
                    onPressed: !_cupLabelActionEnabled
                        ? null
  ...
  Widget _buildMobileSellCartBody() {
    ...
    if (_tab.cart.isEmpty) {
      if (_isTableOrderMode) {
        ...
        return PosEmptyCartBrand(
          hint: tr(table.isEmpty
              ? 'Tìm và thêm ${_sellProfile.lineUnit} vào phiếu'
              : '$table · ${_sellProfile.emptyCartHint}'),
          below: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              FilledButton.icon(
                ...
                label: Text(tr(_sellProfile.pickCatalogLabel)),
                style: FilledButton.styleFrom(
                  backgroundColor: PosTheme.kiotBlue,
                  padding: const EdgeInsets.symmetric(
      ...
      return PosEmptyCartBrand(
        hint: tr(_sellProfile.emptyCartHint),
      );
    }
  ...
  Widget _buildMobileCheckoutBar(PermissionProvider perm, bool canPay) {
    ...
    return Material(
      ...
      child: SafeArea(
        ...
        bottom: !inHub,
        child: Container(
          padding: EdgeInsets.fromLTRB(12, 8, 12, inHub ? 4 : 8),
          decoration: const BoxDecoration(
            border: Border(top: BorderSide(color: PosTheme.border)),
          ),
          child: Column(
            ...
            children: [
              Row(
                children: [
                  ...
                  if (_resolveTransferPaymentQr() != null)
                    IconButton(
                      visualDensity: VisualDensity.compact,
                      tooltip: tr('Mã QR thanh toán'),
                      onPressed: () {
                        final qr = _resolveTransferPaymentQr();
                        if (qr == null) return;
                        final panelTitle = qr.isTingee
                            ? 'Tingee QR thanh toán'
                            : 'VietQR thanh toán';
                        final dialogTitle = qr.isTingee
                            ? 'Quét Tingee QR thanh toán'
                            : 'Quét VietQR thanh toán';
                  ...
                  Text(
                    tr('${_moneyFmt.format(_grandTotal)} đ'),
                    style: const TextStyle(
                      fontSize: 18,
                      fontWeight: FontWeight.bold,
                      color: _kiotBlue,
              ...
              _buildInvoiceActionButtons(
                ...
                payLabel: 'Thanh toán',
                height: 48,
                payIsPrimaryFilled: false,
                onPay: () => _openMobilePaymentScreen(perm),
              ),
            ],
          ),
        ),
    ...
    setState(() {
      ...
      cloned.toppings = [
        for (final t in line.toppings)
          _CartTopping(id: t.id, name: t.name, price: t.price, qty: t.qty),
      ];
  ...
  void _mergeMobileCartLine(int cartIndex) {
    ...
    for (var i = 0; i < _tab.cart.length; i++) {
      ...
      final sameToppings = other.toppings.map((t) => '${t.id}:${t.price}x${t.safeQty}').join('|') ==
          line.toppings.map((t) => '${t.id}:${t.price}x${t.safeQty}').join('|');
  ...
  Future<void> _openMobileCustomerPicker() async {
    final qCtrl = TextEditingController(
      text: _isWalkInCustomerQuery(_tab._customerSearchCtrl.text)
          ? ''
          : _tab._customerSearchCtrl.text,
    );
    await showModalBottomSheet<void>(
                ...
                builder: (_, scroll) => Column(
                  children: [
                    ...
                    ListTile(
                      ...
                      title: Text(tr(kPosWalkInCustomerLabel)),
                    ...
                    child: ListView(
                      ...
                      children: [
                        _buildMobilePaymentActionTile(
                          ...
                          title: 'Khách hàng',
                          value: _tab.customer?.name ?? kPosWalkInCustomerLabel,
                          onTap: busy
                              ? () {}
                              : () async {
                  ...
                  SafeArea(
                    ...
                    child: Padding(
                      ...
                      child: _buildCheckoutActions(
                        canPay: canPay,
                        canOrder: perm.canPosOrder(),
                        busy: busy || _parking || _provisionalPrinting,
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    if (line.product.productType == PosProductType.combo &&
                      ...
                      Text(
                        tr(
                            '↳ ${formatComboComponentsSubtitle(line.product.comboLines!, comboQty: line.qty)}'),
                        maxLines: 2,
                        overflow: TextOverflow.ellipsis,
                        style: const TextStyle(
                          fontSize: 12,
  ...
  Future<void> _showMobileCartLineActions(
    ...
    var views = line.unitViews;
    if (views.length <= 1) {
      views = await loadPosSellUnitViews(_api, line.product);
      if (!mounted) return;
      views = applyPosPriceListToViews(
          views, line.product, _currentPriceOverrides);
      if (views.length > 1) {
        setState(() => line.unitViews = views);
      }
    }
                  ...
                  Wrap(
                    ...
                    children: [
                      for (final v in views)
                        ChoiceChip(
                          label: Text(tr(v.label)),
                          selected: v.viewKey == line.activeViewKey,
                          onSelected: (_) =>
                              Navigator.pop(ctx, 'unit:${v.viewKey}'),
                        ),
                    ],
                  ),
                ],
    ...
    if (action.startsWith('unit:')) {
      final key = action.substring(5);
      final view = views.where((v) => v.viewKey == key).firstOrNull;
      if (view != null) await _switchUnit(line, view);
      return;
    }
    switch (action) {
      case 'note':
  ...
  void _setLineToppingQty(
    ...
    if (!_guardReadOnlyEdit()) return;
    final sigBefore = _kitchenLineSignature(line);
    setState(() {
      line.toppings.removeWhere((t) => t.id == id);
      if (qty > 0) {
      ...
      _renotifyKitchenOnLineChange(line, sigBefore);
      _syncPaidAmount();
      _scheduleDraftAutosave();
      _scheduleCustomerDisplayPublish();
    });
        ...
        children: [
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(tr(name), style: const TextStyle(fontWeight: FontWeight.w600)),
                Text(
                  tr('+${_moneyFmt.format(unitPrice)} đ'
                      '${q > 1 ? ' · ${q} × = ${_moneyFmt.format(unitPrice * q)}' : ''}'),
                  style: const TextStyle(color: _kiotBlue, fontSize: 12),
                ),
              ],
            ),
          ),
          IconButton(
            visualDensity: VisualDensity.compact,
            onPressed: q <= 0 ? null : () => onChanged(q - 1),
          ...
          IconButton(
            visualDensity: VisualDensity.compact,
            onPressed: q >= 99 ? null : () => onChanged(q + 1),
  ...
  Future<void> _openCartLineToppings(_SellCartLine line) async {
    ...
    final qtyById = <String, int>{
      for (final t in line.toppings) t.id: t.safeQty,
    };
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    ConstrainedBox(
                      ...
                      child: ListView(
                        ...
                        children: options.map((o) {
                          final q = qtyById[o.toppingProductId] ?? 0;
                          return _toppingQtyRow(
                            name: o.toppingProductName,
                            unitPrice: o.extraPrice,
                            qty: q,
                            onChanged: (next) {
                              setModal(() {
                                if (next <= 0) {
                                  qtyById.remove(o.toppingProductId);
                                } else {
                                  qtyById[o.toppingProductId] = next;
                                }
                              });
                            },
                          );
                    ...
                    FilledButton(
                      onPressed: () {
                        final picks = options
                            .where((o) => (qtyById[o.toppingProductId] ?? 0) > 0)
                            .map((o) => _CartTopping(
                                  id: o.toppingProductId,
                                  name: o.toppingProductName,
                                  price: o.extraPrice,
                                  qty: qtyById[o.toppingProductId] ?? 1,
                                ))
                            .toList();
  ...
  Widget _modeTab(String label, _SellMode mode, IconData icon) {
    ...
    return InkWell(
      onTap: () {
        setState(() {
          _sellMode = mode;
          if (mode == _SellMode.delivery) {
            if (_enabledShippingCarriers.isEmpty) {
              unawaited(_reloadEnabledShippingCarriers());
            } else {
              _ensureDeliveryPartnerSelected();
            }
          }
