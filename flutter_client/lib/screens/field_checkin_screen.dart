...
class _FieldCheckInScreenState extends State<FieldCheckInScreen>
    ...
    try {
      ...
      if (_pendingTrackPoints.isNotEmpty) {
        await prefs.setString(
            _kPendingPointsKey, jsonEncode(_pendingTrackPoints));
      } else {
        await prefs.remove(_kPendingPointsKey);
      }
    } catch (_) {}
      ...
      setState(() {
        if (results[0]['isSuccess'] == true && results[0]['data'] != null) {
          _myAssignments = (results[0]['data'] as List)
              .map((e) =>
                  FieldLocationAssignment.fromJson(e as Map<String, dynamic>))
              .toList();
        }
        ...
        if (results[2]['isSuccess'] == true && results[2]['data'] != null) {
          _todayJourney = JourneyTracking.fromJson(
  ...
  List<FieldLocation> _sortLocationsByDistance(List<FieldLocation> input) {
    ...
    list.sort((a, b) {
      final aDist = _calculateDistance(
          _currentLat!, _currentLng!, a.latitude, a.longitude);
      final bDist = _calculateDistance(
          _currentLat!, _currentLng!, b.latitude, b.longitude);
  ...
  Future<void> _autoStartJourney() async {
    try {
      ...
      if (mounted && resp['isSuccess'] == true && resp['data'] != null) {
        setState(() => _todayJourney =
            JourneyTracking.fromJson(resp['data'] as Map<String, dynamic>));
        _startGpsTracking();
      }
    } catch (_) {}
  }
    ...
    try {
      final futures = <Future<Map<String, dynamic>>>[
        _isManager
            ...
            : _apiService.getMyFieldVisits(
                fromDate: _historyFrom, toDate: _historyTo),
      ...
      try {
        ...
        if (mounted) {
          ...
          if (_lastTrackedLat != null && _lastTrackedLng != null) {
            final dist = _calculateDistance(_lastTrackedLat!, _lastTrackedLng!,
          ...
          if (_pendingTrackPoints.length >= 2) {
            ...
            if (resp['isSuccess'] == true && resp['data'] != null) {
              ...
              if (rp != null) {
                try {
                  final raw = rp is String ? jsonDecode(rp) : rp;
                  if (raw is List) {
                    freshPoints = raw
                        .map((e) =>
                ...
                _todayJourney = JourneyTracking(
                  ...
                  totalDistanceKm:
                      (data['totalDistanceKm'] as num?)?.toDouble() ??
                          _todayJourney!.totalDistanceKm,
                  totalTravelMinutes: data['totalTravelMinutes'] ??
                      _todayJourney!.totalTravelMinutes,
                  totalOnSiteMinutes: data['totalOnSiteMinutes'] ??
                      _todayJourney!.totalOnSiteMinutes,
                  checkedInCount:
                      data['checkedInCount'] ?? _todayJourney!.checkedInCount,
  ...
  Future<void> _startJourney() async {
    ...
    if (resp['isSuccess'] == true && resp['data'] != null) {
      ...
      NotificationOverlayManager()
          .showWarning(title: 'Lỗi', message: resp['message'] ?? 'Lỗi');
    }
  }
  ...
  Future<void> _endJourney() async {
    ...
    if (_pendingTrackPoints.isNotEmpty) {
      await _apiService.trackJourneyPoints(
          List<Map<String, dynamic>>.from(_pendingTrackPoints));
      _pendingTrackPoints.clear();
      _savePendingPoints();
    }
        ...
        content: Column(
          ...
          children: [
            if (_todayJourney != null) ...[
              ...
              Text(
                  'Đã check-in: ${_todayJourney!.checkedInCount}/${_todayJourney!.assignedCount} điểm'),
            ],
    ...
    if (resp['isSuccess'] == true) {
      ...
      NotificationOverlayManager().showSuccess(
        ...
        message:
            'Hành trình đã kết thúc. ${_todayJourney?.distanceFormatted ?? ""} • ${_todayJourney?.checkedInCount ?? 0} điểm.',
      );
    } else {
      NotificationOverlayManager().showWarning(
        title: 'Lỗi kết thúc',
  ...
  Future<void> _doCheckIn(FieldLocationAssignment assignment) async {
                ...
                child: Row(children: [
                  ...
                  Column(
                      ...
                      children: [
                        ...
                        if (isOutsideRadius)
                          Text('Ngoài bán kính ${loc?.radius ?? 100}m',
                              style: const TextStyle(
    ...
    if (resp['isSuccess'] == true) {
      NotificationOverlayManager().showSuccess(
          title: 'Check-in',
          message: 'Thành công tại ${assignment.location?.name}');
      _loadMyData();
    } else {
      NotificationOverlayManager().showWarning(
          title: 'Lỗi', message: resp['message'] ?? 'Lỗi check-in');
    }
  }
  ...
  Future<void> _doCheckInAtFieldLocation(FieldLocation loc) async {
            ...
            Container(
              ...
              child: Row(children: [
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  if (isOutsideRadius)
                    Text('Ngoài bán kính ${loc.radius}m',
                        style: const TextStyle(
                            fontSize: 12, color: Color(0xFFEF4444))),
                ]),
              ]),
    ...
    if (resp['isSuccess'] == true) {
      NotificationOverlayManager().showSuccess(
          title: 'Check-in', message: 'Thành công tại ${loc.name}');
      _loadMyData();
    } else {
      NotificationOverlayManager().showWarning(
          title: 'Lỗi', message: resp['message'] ?? 'Lỗi check-in');
  ...
  Future<void> _doCheckOut(VisitReport visit) async {
    ...
    final meetingSummaryCtl = TextEditingController(
        text: (visit.reportData?['meetingSummary'] ?? '').toString());
    final customerStatusCtl = TextEditingController(
        text: (visit.reportData?['customerStatus'] ?? '').toString());
    final nextActionCtl = TextEditingController(
        text: (visit.reportData?['nextAction'] ?? '').toString());
    ...
    List<String> collectSuggestions(String Function(VisitReport v) pick) {
      final pool = <VisitReport>[
        ...
        ..._todayVisits
            .where((v) => v.id != visit.id && v.locationId == visit.locationId),
        ..._historyVisits.where((v) => v.locationId != visit.locationId),
      ];
      final seen = <String>{};
      final result = <String>[];
    ...
    final meetingSuggestions = collectSuggestions(
        (v) => (v.reportData?['meetingSummary'] ?? '').toString());
    final statusSuggestions = collectSuggestions(
        (v) => (v.reportData?['customerStatus'] ?? '').toString());
    final nextSuggestions = collectSuggestions(
        (v) => (v.reportData?['nextAction'] ?? '').toString());

    Widget suggestionChips(List<String> items, TextEditingController ctl,
            ...
            child: SingleChildScrollView(
              ...
              children: [
                    ...
                    Expanded(
                        child: Column(
                          ...
                          if (visit.checkInTime != null)
                            Text(
                                'Check-in lúc: ${DateFormat('HH:mm').format(visit.checkInTime!.toLocal())}',
                                style: TextStyle(
                                    color: Colors.grey[600], fontSize: 13)),
                        ])),
                  ],
                    ...
                    IconButton(
                        ...
                        onPressed: () async {
                          final p = await picker.pickImage(
                              ...
                              maxWidth: 1280);
                          if (p != null) {
                            setDialogState(() => selectedPhotos.add(p));
                          }
                        }),
                    IconButton(
                        visualDensity: VisualDensity.compact,
                        ...
                        onPressed: () async {
                          final ps = await picker.pickMultiImage(
                              imageQuality: 70, maxWidth: 1280);
                          setDialogState(() => selectedPhotos
                              .addAll(ps.take(5 - selectedPhotos.length)));
                        }),
                  ],
                ]),
                ...
                if (showMore) ...[
                  ...
                  suggestionChips(
                      meetingSuggestions, meetingSummaryCtl, setDialogState),
                  const SizedBox(height: 8),
                  TextField(
                  ...
                  suggestionChips(
                      statusSuggestions, customerStatusCtl, setDialogState),
                  const SizedBox(height: 8),
                  TextField(
                  ...
                  suggestionChips(
                      nextSuggestions, nextActionCtl, setDialogState),
                ],
                const SizedBox(height: 20),
                Row(children: [
                  Expanded(
  ...
  Future<void> _showVisitDetailSheet(VisitReport visit) async {
    final meetingSummary =
        (visit.reportData?['meetingSummary'] ?? '').toString().trim();
    final customerStatus =
        (visit.reportData?['customerStatus'] ?? '').toString().trim();
    final nextAction =
        (visit.reportData?['nextAction'] ?? '').toString().trim();
        ...
        child: SingleChildScrollView(
          ...
          children: [
            ...
            Row(children: [
              ...
              Expanded(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                    Text(visit.locationName ?? 'Điểm bán',
                        style: const TextStyle(
                            fontWeight: FontWeight.w700, fontSize: 16)),
            ...
            Wrap(spacing: 12, runSpacing: 6, children: [
              if (visit.checkInTime != null)
                Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(width: 4),
                  Text(
                      'Check-in: ${DateFormat('dd/MM HH:mm').format(visit.checkInTime!.toLocal())}',
              ...
              if (visit.checkOutTime != null)
                Row(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  const SizedBox(width: 4),
                  Text(
                      'Check-out: ${DateFormat('dd/MM HH:mm').format(visit.checkOutTime!.toLocal())}',
                      style: const TextStyle(fontSize: 12)),
                ]),
            ...
            if (meetingSummary.isEmpty &&
                ...
                nextAction.isEmpty &&
                (visit.reportNote == null || visit.reportNote!.isEmpty) &&
                visit.photos.isEmpty)
              Padding(
                ...
                child: Center(
                  ...
                  const SizedBox(height: 8),
                  Text('Chưa có báo cáo cho lượt này',
              ...
              reportField(
                  Icons.handshake, 'Tình hình gặp khách', meetingSummary),
              reportField(Icons.task_alt, 'Kết quả chăm sóc', customerStatus),
              reportField(
                  Icons.arrow_forward, 'Hành động tiếp theo', nextAction),
  ...
  Future<void> _reviewVisit(VisitReport visit) async {
        ...
        content: SizedBox(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ListTile(
              ...
              subtitle: Text(
                  '${visit.locationName ?? ""} • ${visit.timeSpentFormatted}'),
            ),
            const SizedBox(height: 8),
            TextField(
              controller: noteCtl,
              maxLines: 2,
              decoration: const InputDecoration(
                  labelText: 'Nhận xét', border: OutlineInputBorder()),
            ),
          ]),
        ),
        actions: [
    ...
    final resp =
        await _apiService.reviewFieldVisit(visit.id, {'reviewNote': result});
    if (!mounted) return;
    if (resp['isSuccess'] == true) {
      NotificationOverlayManager()
          .showSuccess(title: 'Duyệt', message: 'Đã duyệt');
      _loadManagerData();
    }
  }
                    ...
                    if (_todayJourney?.isActive == true)
                      Container(
                        ...
                        child: Row(mainAxisSize: MainAxisSize.min, children: [
                          ...
                          const SizedBox(width: 6),
                          Text(
                              'LIVE • ${_todayJourney?.distanceFormatted ?? ""}',
                              style: const TextStyle(
                                  color: Colors.white,
                                  fontSize: 11,
                                  fontWeight: FontWeight.bold)),
  ...
  Widget _buildJourneyTab() {
    ...
    return Stack(
      children: [
            ...
            MarkerLayer(markers: [
              ...
              ...() {
                ...
                return sorted.asMap().entries.map((e) {
                  ...
                  final v = e.value;
                  final label =
                      '${v.locationName ?? "Điểm"} · ${DateFormat('HH:mm').format(v.checkInTime!.toLocal())}';
                  return Marker(
              ...
              ..._sortedAssignments.where((a) => a.location != null).map((a) {
                final activeVisit = _todayVisits
                    .where((v) => v.locationId == a.locationId && v.isCheckedIn)
                    .firstOrNull;
                final visited = _todayVisits.any((v) =>
                    v.locationId == a.locationId && v.checkOutTime != null);
                return Marker(
        ...
        if (_todayJourney != null &&
            (_todayJourney!.isActive || _todayJourney!.isCompleted) &&
            _todayJourney!.routePoints.isNotEmpty)
          Positioned(
            top: 8,
            left: 12,
            right: 12,
            child: Container(
              ...
              child: Row(
                ...
                children: [
                  _buildJourneyStat(Icons.route,
                      _todayJourney!.distanceFormatted, 'Quãng đường'),
                  Container(width: 1, height: 30, color: Colors.grey[300]),
                  _buildJourneyStat(Icons.timer,
                      _todayJourney!.durationFormatted, 'Thời gian'),
                  Container(width: 1, height: 30, color: Colors.grey[300]),
                  _buildJourneyStat(
                      Icons.location_on,
                      '${_todayJourney!.checkedInCount}/${_todayJourney!.assignedCount}',
                      'Điểm'),
                ],
              ),
            ),
          ...
          builder: (ctx, scrollController) => Container(
            ...
            child: ListView(
              ...
              children: [
                ...
                if (_todayJourney != null &&
                    _todayJourney!.routePoints.any((p) => p.isDwell))
                  Padding(
                              ...
                              .map((p) => Padding(
                                    ...
                                    child: Row(children: [
                                      ...
                                      Expanded(
                                          child: Text(
                                        p.nearLocationName ??
                                            'Vị trí (${p.lat.toStringAsFixed(5)}, ${p.lng.toStringAsFixed(5)})',
                                        style: const TextStyle(fontSize: 12),
                                        overflow: TextOverflow.ellipsis,
                                      )),
                                      Text(
                ...
                if (_isSearching &&
                    ...
                    _locationSearchCtl.text.isNotEmpty)
                  Padding(
                    padding: const EdgeInsets.all(24),
                    child: Column(children: [
                      Icon(Icons.search_off, size: 36, color: Colors.grey[300]),
                      const SizedBox(height: 6),
                      Text('Không tìm thấy điểm bán',
                          style:
                  ...
                  Padding(
                    ...
                    child: Column(children: [
                      ...
                      const SizedBox(height: 6),
                      Text('Chưa đăng ký điểm bán nào',
                          style:
                              TextStyle(color: Colors.grey[500], fontSize: 13)),
                      const SizedBox(height: 8),
                      TextButton.icon(
                  ...
                  ...(_isSearching
                          ? _sortLocationsByDistance(_searchResults
                              .where((loc) => !_isBranchAttendanceLocation(loc))
                              .toList())
                          : _visibleRegisteredLocations)
  ...
  void _onSearchFieldLocations(String query) async {
    ...
    try {
      ...
      if (_locationSearchCtl.text.trim() == query.trim() &&
          resp['isSuccess'] == true &&
          resp['data'] != null) {
  ...
  void _fitAllMarkers() {
    ...
    for (final a in _sortedAssignments) {
      if (a.location != null) {
        points.add(LatLng(a.location!.latitude, a.location!.longitude));
      }
    }
  ...
  Widget _buildTodayProgressBar() {
    ...
    final completed =
        _todayVisits.where((v) => v.isCheckedOut || v.isReviewed).length;
  ...
  Widget _buildLocationCard(FieldLocationAssignment a) {
    final todayVisit =
        _todayVisits.where((v) => v.locationId == a.locationId).toList();
    final activeVisit = todayVisit.where((v) => v.isCheckedIn).firstOrNull;

    double? distance;
    if (_currentLat != null && a.location != null) {
      distance = _calculateDistance(_currentLat!, _currentLng!,
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
            color:
                accentColor.withValues(alpha: activeVisit != null ? 0.5 : 0.2)),
        boxShadow: [
      ...
      child: ListTile(
        ...
        subtitle: Row(children: [
          ...
          if (activeVisit?.checkInTime != null) ...[
            const SizedBox(width: 6),
            Text(
                '• ${DateFormat('HH:mm').format(activeVisit!.checkInTime!.toLocal())}',
        ...
        onTap: () {
          if (a.location != null) {
            _mapController.move(
                LatLng(a.location!.latitude, a.location!.longitude), 16);
            try {
              _sheetCtl.animateTo(0.12,
  ...
  Widget _buildFieldLocationCard(FieldLocation loc) {
    ...
    final todayVisit =
        _todayVisits.where((v) => v.locationId == loc.id).toList();
    final activeVisit = todayVisit.where((v) => v.isCheckedIn).firstOrNull;
    ...
    return Container(
      ...
      child: ListTile(
        ...
        trailing: activeVisit != null
            ? _actionButton('Check-out', Icons.logout, Colors.orange,
                () => _doCheckOut(activeVisit))
            : _actionButton('Check-in', Icons.login, const Color(0xFF6366F1),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: const [
                        DropdownMenuItem(
                            value: 'retail', child: Text('Bán lẻ')),
                        DropdownMenuItem(
                            value: 'wholesale', child: Text('Bán sỉ')),
                        DropdownMenuItem(
                            value: 'pharmacy', child: Text('Nhà thuốc')),
                        DropdownMenuItem(
                            value: 'restaurant', child: Text('Quán ăn')),
                        DropdownMenuItem(
                            value: 'supermarket', child: Text('Siêu thị')),
                        ...
                        ...photos.asMap().entries.map((entry) => Stack(
                              children: [
                                ClipRRect(
                                  borderRadius: BorderRadius.circular(8),
                                  child: Image.memory(
                                    base64Decode(entry.value.contains(',')
                            ...
                            onTap: () async {
                              try {
                                ...
                                if (picked != null) {
                                  ...
                                  final b64 =
                                      base64Encode(_compressImage(bytes));
                        ...
                        onPressed: saving
                            ...
                            : () async {
                                ...
                                if (lat == null || lng == null) {
                                  NotificationOverlayManager().showWarning(
                                      title: 'Lỗi',
                                      message: 'Chưa xác định được vị trí GPS');
                                  return;
                                }
                                setSheetState(() => saving = true);
                                final result =
                                    await _apiService.registerFieldLocation({
                                ...
                                if (result['isSuccess'] == true) {
                                  ...
                                  NotificationOverlayManager().showSuccess(
                                      title: 'Thành công',
                                      message:
                                          'Đã đăng ký điểm bán thành công!');
                                  _loadMyData();
                                } else {
                                  NotificationOverlayManager().showWarning(
                                      title: 'Lỗi',
                                      message:
                                          result['message'] ?? 'Lỗi đăng ký');
                                }
                              },
                        icon: saving
                            ? const SizedBox(
  ...
  Widget _buildHistoryTab() {
    ...
    final checkedOut =
        filtered.where((v) => v.isCheckedOut || v.isReviewed).length;
    final totalMinutes = filtered
        .where((v) => v.timeSpentMinutes != null)
        .fold<int>(0, (s, v) => s + v.timeSpentMinutes!);
    final uniqueEmployees =
        _isManager ? filtered.map((v) => v.employeeId).toSet().length : 0;
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 6),
                      Text(
                          '${DateFormat('dd/MM').format(_historyFrom)} - ${DateFormat('dd/MM').format(_historyTo)}',
                          style: const TextStyle(fontSize: 12)),
                    ]),
                  ),
                ),
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              _buildHistoryStat(
                  Icons.timer, _formatMinutes(totalMinutes), 'Thời gian'),
              if (_isManager)
                _buildHistoryStat(Icons.people, '$uniqueEmployees', 'NV'),
            ],
          ),
        ),
        const Divider(height: 1),
              ...
              : filtered.isEmpty
                  ...
                  : RefreshIndicator(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (ctx, i) =>
                            _buildHistoryVisitCard(filtered[i]),
                      ),
                    ),
        ),
      ],
  ...
  Widget _buildQuickDateBtn(String label, int days) {
    ...
    final isActive = _historyFrom.day == from.day &&
        _historyFrom.month == from.month &&
        _historyFrom.year == from.year &&
        _historyTo.day == now.day &&
        _historyTo.month == now.month &&
        _historyTo.year == now.year;
    return GestureDetector(
      onTap: () {
        setState(() {
          _historyFrom = from;
  ...
  List<Map<String, String>> _getUniqueEmployees() {
    ...
    for (final v in _historyVisits) {
      if (v.employeeId != null && seen.add(v.employeeId!)) {
        result.add(
            {'id': v.employeeId!, 'name': v.employeeName ?? v.employeeId!});
      }
    }
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(
            ...
            children: [
              ...
              _miniStat(Icons.store, '${j.checkedInCount}/${j.assignedCount}',
                  'Điểm'),
  ...
  Widget _buildHistoryVisitCard(VisitReport v) {
    ...
    final meetingSummary =
        (v.reportData?['meetingSummary'] ?? '').toString().trim();
    final customerStatus =
        (v.reportData?['customerStatus'] ?? '').toString().trim();
        ...
        child: Padding(
          ...
          child:
            ...
            Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
              CircleAvatar(
                radius: 18,
                backgroundColor: v.isCheckedOut || v.isReviewed
                    ? Colors.green.shade50
                    : Colors.orange.shade50,
                child: Icon(Icons.store,
                    size: 18,
                    color: v.isCheckedOut || v.isReviewed
                        ? Colors.green
                        : Colors.orange),
              ),
              const SizedBox(width: 10),
              Expanded(
                  child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                    Text(v.locationName ?? 'Không tên',
                        style: const TextStyle(
                            fontWeight: FontWeight.w600, fontSize: 14)),
                    if (v.locationAddress != null &&
                        v.locationAddress!.isNotEmpty)
                      Padding(
            ...
            Row(children: [
              ...
              if (v.checkOutTime != null) ...[
                Text(' → ',
                    style: TextStyle(fontSize: 11, color: Colors.grey[500])),
                Text(DateFormat('HH:mm').format(v.checkOutTime!.toLocal()),
                    style: TextStyle(fontSize: 11, color: Colors.grey[600])),
              ],
            ...
            if (v.checkInDistance != null) ...[
              ...
              Row(children: [
                ...
                const SizedBox(width: 4),
                Text('Khoảng cách: ${v.checkInDistance!.toStringAsFixed(0)}m',
    ...
    return Row(children: [
      ...
      Expanded(
        child: _buildVisitActionBtn(
          ...
          enabled: hasMap,
          onTap: () =>
              _openLocationMap(v.locationName ?? 'Điểm bán', lat!, lng!),
        ),
      ),
    ]);
  }
    ...
    return InkWell(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(6),
          border:
              Border.all(color: color.withValues(alpha: enabled ? 0.3 : 0.15)),
        ),
  ...
  void _showLocationHistory(VisitReport current) {
    final visits =
        _historyVisits.where((x) => x.locationId == current.locationId).toList()
          ..sort((a, b) {
                    ...
                    : ListView.separated(
                        ...
                        itemBuilder: (_, i) {
                          ...
                          return ListTile(
                            ...
                            leading: CircleAvatar(
                              radius: 16,
                              backgroundColor: x.isCheckedOut || x.isReviewed
                                  ? Colors.green.shade50
                                  : Colors.orange.shade50,
                              child: Icon(
  ...
  Widget _buildStatusChip(String status) {
    ...
    switch (status) {
      ...
      case 'in_progress':
        color = Colors.orange;
        label = status == 'in_progress' ? 'Đang đi' : 'Đang ở';
        break;
      case 'checked_out':
        color = Colors.blue;
        label = 'Đã xong';
  ...
  Color _getDeptColor(int deptIndex) =>
      _deptColors[deptIndex % _deptColors.length];
        ...
        children: [
          ...
          Padding(
            ...
            child: Row(children: [
                ...
                child: InkWell(
                  ...
                  child: Container(
                    ...
                    child: Row(children: [
                      ...
                      const SizedBox(width: 8),
                      Text(
                          '${DateFormat('dd/MM').format(_reportFrom)} - ${DateFormat('dd/MM/yyyy').format(_reportTo)}',
                          style: const TextStyle(fontSize: 13)),
                    ]),
                  ),
                ),
              ...
              IconButton(
                onPressed: () =>
                    setState(() => _showManagerMap = !_showManagerMap),
          ...
          // Map or List
          Expanded(
            child: _showManagerMap
                ? _buildEmployeeMapView()
                : _buildManagerListView(),
          ),
        ],
      ),
    );
  ...
  Widget _buildEmployeeMapView() {
    ...
    final withLocation = _employeeLocations
        .where((e) =>
            (e['latitude'] as num?)?.toDouble() != null &&
            (e['latitude'] as num?)?.toDouble() != 0)
        .length;
        ...
        Container(
          ...
          child: ListView(
            ...
            children: deptGroups.entries.map((entry) {
              final deptIdx =
                  (entry.value.first['departmentColorIndex'] as num?)
                          ?.toInt() ??
                      0;
              final color = _getDeptColor(deptIdx);
              return Padding(
    ...
    Navigator.of(context).push(
      MaterialPageRoute(
        ...
        builder: (_) => _FullscreenEmployeeMap(
          employeeLocations:
              List<Map<String, dynamic>>.from(_employeeLocations),
  ...
  Widget _buildEmployeeLocationTile(int index) {
    ...
    final source = emp['locationSource'] ?? '';
    final lastUpdate = emp['lastUpdateTime'] != null
        ? DateTime.tryParse(emp['lastUpdateTime'])
        : null;
    ...
    switch (source) {
      ...
      case 'checkin':
        ...
        break;
      case 'punch':
        sourceLabel = 'Chấm công';
        sourceIcon = Icons.fingerprint;
        break;
      case 'live':
        sourceLabel = 'Trực tuyến';
        sourceIcon = Icons.gps_fixed;
        break;
      default:
        sourceLabel = 'Chưa có vị trí';
        sourceIcon = Icons.location_off;
    }
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(10),
        side:
            isSelected ? BorderSide(color: color, width: 1.5) : BorderSide.none,
      ),
      child: InkWell(
        borderRadius: BorderRadius.circular(10),
        onTap: () {
          ...
          if (!isSelected && hasLocation) {
            ...
            final lng = (emp['longitude'] as num).toDouble();
            try {
              _managerMapController.move(LatLng(lat, lng), 16);
            } catch (_) {}
          }
        },
        child: Padding(
                ...
                child: Column(
                    ...
                    if (checkins.isNotEmpty)
                      Container(
                        ...
                        decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(8)),
                        child: Text('${checkins.length} check-in',
                            style: const TextStyle(
                                fontSize: 10,
                                color: Colors.green,
                                fontWeight: FontWeight.w600)),
                  ...
                  Row(children: [
                    ...
                    Text(sourceLabel,
                        style: TextStyle(
                            fontSize: 10,
                            color:
                                hasLocation ? Colors.green : Colors.grey[500])),
                    if (lastUpdate != null) ...[
                      const SizedBox(width: 6),
                  ...
                  if (isSelected && checkins.isNotEmpty) ...[
                    ...
                    ...checkins.map((v) {
                      ...
                      final checkIn = v['checkInTime'] != null
                          ? DateTime.tryParse(v['checkInTime'])
                          : null;
                      final checkOut = v['checkOutTime'] != null
                          ? DateTime.tryParse(v['checkOutTime'])
                          : null;
                        ...
                        child: Row(children: [
                          ...
                          Expanded(
                              child: Text(locName,
                                  ...
                                  overflow: TextOverflow.ellipsis)),
                          if (checkIn != null)
                            Text(DateFormat('HH:mm').format(checkIn.toLocal()),
                                style: TextStyle(
                    ...
                    Row(children: [
                      Expanded(
                        child: OutlinedButton.icon(
                          onPressed: _isLoadingJourney
                              ? null
                              : () =>
                                  _loadEmployeeJourney(emp['employeeId'], name),
  ...
  Future<void> _loadEmployeeJourney(
    ...
    try {
      ...
      if (result['isSuccess'] == true && result['data'] != null) {
        ...
        final points =
            journey.routePoints.where((p) => p.lat != 0 && p.lng != 0).toList();
        if (points.isNotEmpty) {
          if (points.length == 1) {
            try {
              _managerMapController.move(
                  LatLng(points.first.lat, points.first.lng), 15);
            } catch (_) {}
          } else {
  ...
  List<Widget> _buildJourneyOverlayLayers() {
    ...
    if (j == null) return [];
    final points =
        j.routePoints.where((p) => p.lat != 0 && p.lng != 0).toList();
    if (points.isEmpty) return [];
  ...
  Widget _buildManagerListView() {
    ...
    final weeklyWithNextAction =
        weekly.where((v) => _extractNextAction(v).isNotEmpty).length;

    return ListView(
      padding: const EdgeInsets.all(12),
      children: [
        // Weekly KPI for manager
        const Text('KPI 7 ngày gần nhất',
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 6),
            _buildStatCard('Có báo cáo', '$weeklyWithStructured',
                Icons.summarize, const Color(0xFF9333EA)),
            const SizedBox(width: 6),
            _buildStatCard('Có follow-up', '$weeklyWithNextAction',
                Icons.next_plan, const Color(0xFFF59E0B)),
        ...
        if (_managerJourneys.isNotEmpty) ...[
          Text('Hành trình nhân viên (${_managerJourneys.length})',
              style:
        ...
        // Assignments
        Text('Giao điểm (${_allAssignments.length})',
        ...
        Wrap(
          ...
          children: _managerCustomerStatusOptions.map((status) {
            ...
            return ChoiceChip(
              ...
              selected: selected,
              onSelected: (_) =>
                  setState(() => _managerCustomerStatusFilter = status),
            );
          }).toList(),
        ),
        const SizedBox(height: 8),
        ...
        if (searchFiltered.isEmpty)
          Center(
            child: Padding(
              ...
              child: Text('Không có báo cáo theo bộ lọc đã chọn',
  ...
  Widget _buildStatsRow() {
    ...
    final checkedOut =
        _reports.where((r) => r.isCheckedOut || r.isReviewed).length;
    final totalMinutes = _reports
        .where((r) => r.timeSpentMinutes != null)
        .fold<int>(0, (sum, r) => sum + r.timeSpentMinutes!);
    final journeyKm =
        _managerJourneys.fold<double>(0, (sum, j) => sum + j.totalDistanceKm);
    ...
    return Row(
      children: [
        ...
        const SizedBox(width: 6),
        _buildStatCard(
            'Xong', '$checkedOut', Icons.check_circle, const Color(0xFF22C55E)),
        const SizedBox(width: 6),
        _buildStatCard('Giờ', '${(totalMinutes / 60).toStringAsFixed(1)}h',
                  ...
                  child: Column(
                    ...
                    Text(j.employeeName ?? '',
                        style: const TextStyle(
                            fontWeight: FontWeight.w600, fontSize: 13)),
                    Text(
                        '${DateFormat('dd/MM').format(j.journeyDate.toLocal())} • ${j.distanceFormatted} • ${j.checkedInCount}/${j.assignedCount} điểm',
                        style:
  ...
  void _showJourneyRouteDialog(JourneyTracking j) {
    ...
    if (points.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Chưa có dữ liệu tuyến đường')));
      return;
    }
              ...
              child: ListView(
                ...
                children: [
                  ...
                  Row(
                      children: [
                    _buildMiniStat(
                        'Quãng đường', j.distanceFormatted, Icons.straighten),
                    const SizedBox(width: 8),
                    _buildMiniStat(
                        'Thời gian', j.durationFormatted, Icons.timer),
                    const SizedBox(width: 8),
                    _buildMiniStat(
                        'Check-in',
                        '${j.checkedInCount}/${j.assignedCount}',
                        Icons.location_on),
                  ].map((w) => Expanded(child: w)).toList()),
                  if (dwellPoints.isNotEmpty) ...[
                    const SizedBox(height: 12),
                    const Text('Các điểm dừng',
                        style: TextStyle(
                            fontWeight: FontWeight.bold, fontSize: 13)),
                    const SizedBox(height: 6),
                    ...dwellPoints.map((p) => Padding(
                          ...
                          child: Row(children: [
                            ...
                            const SizedBox(width: 8),
                            Text(DateFormat('HH:mm').format(p.time.toLocal()),
                                style: const TextStyle(
                                    fontSize: 12, fontWeight: FontWeight.w500)),
                            const SizedBox(width: 8),
                            Text('${p.dwellMinutes} phút',
                                style: const TextStyle(
                                    fontSize: 12,
                                    color: Colors.deepOrange,
                                    fontWeight: FontWeight.w600)),
                  ...
                  const SizedBox(height: 12),
                  Text('Lộ trình (${points.length} điểm)',
                      style: const TextStyle(
                          fontWeight: FontWeight.bold, fontSize: 13)),
                  const SizedBox(height: 6),
                  ...List.generate(math.min(points.length, 50), (i) {
                    ...
                    return Padding(
                      ...
                      child: Row(children: [
                        ...
                        const SizedBox(width: 6),
                        Text(DateFormat('HH:mm:ss').format(p.time.toLocal()),
                            style: TextStyle(
                        ...
                        if (p.isDwell) ...[
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                                color: Colors.orange.shade50,
                                borderRadius: BorderRadius.circular(4)),
                            child: Text('${p.dwellMinutes}p',
                                style: const TextStyle(
                                    fontSize: 9, color: Colors.deepOrange)),
                          ),
                        ],
                        if (p.speed != null) ...[
                          const SizedBox(width: 6),
                          Text('${p.speed!.toStringAsFixed(1)} km/h',
                              style: TextStyle(
                                  fontSize: 10, color: Colors.grey[500])),
                        ],
                      ]),
                  ...
                  if (points.length > 50)
                    Text('... và ${points.length - 50} điểm nữa',
                        style:
                            TextStyle(fontSize: 11, color: Colors.grey[500])),
                ],
              ),
  ...
  Widget _buildManagerAssignmentCard(FieldLocationAssignment a) {
    return Card(
      ...
      child: ListTile(
        ...
        subtitle: Text('${a.location?.name ?? ""} • ${a.dayOfWeekLabel}',
            style: const TextStyle(fontSize: 11)),
        trailing: Provider.of<PermissionProvider>(context, listen: false)
                .canDelete('FieldCheckIn')
            ? IconButton(
                ...
                onPressed: () async {
                  ...
                  if (confirm == true) {
                    ...
                    if (mounted && resp['isSuccess'] == true) {
                      NotificationOverlayManager()
                          .showSuccess(title: 'Xoá', message: 'Đã xoá');
                      _loadManagerData();
                    }
                  }
                },
  ...
  Widget _buildManagerReportCard(VisitReport visit) {
    ...
    final meetingSummary =
        (reportData['meetingSummary'] ?? '').toString().trim();
    final customerStatus =
        (reportData['customerStatus'] ?? '').toString().trim();
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                visit.status == 'checked_out'
                    ? TextButton(
                        ...
                        child:
                            const Text('Duyệt', style: TextStyle(fontSize: 11)),
                      )
                    : _buildStatusChip(visit.status),
              ],
            ),
            if (meetingSummary.isNotEmpty ||
                customerStatus.isNotEmpty ||
                nextAction.isNotEmpty ||
                (visit.reportNote?.isNotEmpty ?? false) ||
                visit.photos.isNotEmpty) ...[
              const SizedBox(height: 8),
              if (meetingSummary.isNotEmpty)
                _managerInfoLine(Icons.people_outline, 'Tình hình gặp khách',
                    meetingSummary),
              if (customerStatus.isNotEmpty)
                _managerInfoLine(
                    Icons.support_agent, 'Kết quả chăm sóc', customerStatus),
              if (nextAction.isNotEmpty)
                _managerInfoLine(
                    Icons.event_note, 'Hành động tiếp theo', nextAction),
              if (visit.reportNote != null && visit.reportNote!.isNotEmpty)
                _managerInfoLine(Icons.notes, 'Ghi chú', visit.reportNote!),
              if (visit.photos.isNotEmpty)
                _managerInfoLine(Icons.photo_library, 'Ảnh chăm sóc',
                    '${visit.photos.length} ảnh'),
            ],
          ],
        ),
      ),
        ...
        builder: (ctx, setDialogState) {
          void doAssign() async {
            ...
            if (mounted) {
              if (resp['isSuccess'] == true) {
                NotificationOverlayManager()
                    .showSuccess(title: 'Giao điểm', message: 'Đã giao');
                _loadManagerData();
              } else {
                NotificationOverlayManager().showWarning(
                    title: 'Lỗi', message: resp['message'] ?? 'Lỗi');
              }
            }
          }
          ...
          return AlertDialog(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                DropdownButtonFormField<String>(
                  ...
                  items: _employees.map((e) {
                    final name =
                        e['fullName'] ?? e['employeeName'] ?? e['name'] ?? '';
                    final id =
                        (e['applicationUserId'] ?? e['id'] ?? '').toString();
                ...
                DropdownButtonFormField<String>(
                  decoration: const InputDecoration(
                      labelText: 'Điểm bán', border: OutlineInputBorder()),
                  isExpanded: true,
                  items: _locations
            ...
            actions: [
              ...
              FilledButton(
                onPressed:
                    selectedEmployeeId != null && selectedLocationId != null
                        ? doAssign
                        : null,
  ...
  Widget build(BuildContext context) {
    ...
    for (final emp in widget.employeeLocations) {
      ...
      markers.add(Marker(
        ...
        child: GestureDetector(
          onTap: () => setState(
              () => _selectedId = isSel ? null : emp['employeeId'] as String?),
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 6, vertical: 3),
              decoration: BoxDecoration(
                ...
                child: Row(children: [
                  CircleAvatar(
                    ...
                    backgroundColor: _deptColor(
                            (selected['departmentColorIndex'] as num?)
                                    ?.toInt() ??
                                0)
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Row(children: [
                          Icon(
                            selected['locationSource'] == 'journey'
                                ? Icons.directions_walk
                                : selected['locationSource'] == 'live'
                                    ? Icons.gps_fixed
                                    : selected['locationSource'] == 'punch'
                                        ? Icons.fingerprint
                          ...
                          Text(
                            _sourceLabel(
                                selected['locationSource']?.toString()),
                            style: const TextStyle(
                                fontSize: 11,
                                color: Colors.green,
                                fontWeight: FontWeight.w500),
                          ...
                          if (selected['lastUpdateTime'] != null) ...[
                            ...
                            Text(
                              DateFormat('HH:mm').format(DateTime.parse(
                                      selected['lastUpdateTime'].toString())
                                  .toLocal()),
                              style: TextStyle(
                                  fontSize: 11, color: Colors.grey[600]),
                            ),
