...
class ExcelReportBuilder {
  ...
  /// A4 ngang, co vừa 1 trang ngang. Gói `excel` không ghi pageSetup.
  static List<int> fitSheetToA4Landscape(List<int> xlsxBytes) {
    try {
      ...
      for (var i = 0; i < archive.files.length; i++) {
        ...
        final patched = _injectA4Landscape(xml);
  ...
  static String _injectA4Landscape(String xml) {
    if (xml.contains('paperSize="9"')) return xml;
    var out = xml;
    ...
    out = out.replaceAllMapped(
      ...
      (m) {
        ...
        return '<sheetView$attrs${m[2]}>';
      },
    );
    const margins =
        '<pageMargins left="0.4" right="0.4" top="0.45" bottom="0.45" header="0.25" footer="0.25"/>';
    const setup =
