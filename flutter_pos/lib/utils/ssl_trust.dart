...
const String _kLetsEncryptYr2Pem = '''
-----BEGIN CERTIFICATE-----
...
class _TrustedRootsHttpOverrides extends HttpOverrides {
  ...
  HttpClient createHttpClient(SecurityContext? context) {
    ...
    // Android 6 đôi khi vẫn fail sau khi add root — chỉ nới cho host SBOX.
    client.badCertificateCallback = (cert, host, port) {
      if (!_allowHost(host)) return false;
      debugPrint('⚠️ SSL badCertificate allowlisted host=$host');
      return true;
    };
    return client;
  }
...
Future<void> installPosSslTrust() async {
  ...
  try {
    ...
    var added = 0;
    for (final pem in [_kIsrgRootX1Pem, _kIsrgRootYrPem, _kLetsEncryptYr2Pem]) {
      try {
        ctx.setTrustedCertificatesBytes(utf8.encode(pem.trim()));
        added++;
      } catch (e) {
        debugPrint('SSL trust embed skip: $e');
      }
    }
    HttpOverrides.global = _TrustedRootsHttpOverrides(ctx);
    debugPrint('✅ POS SSL trust: embedded LE/ISRG roots added=$added');
  } catch (e) {
