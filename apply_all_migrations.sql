...
-- Deprecated: work-day boundary is AppSettings.day_end_time only.
...
-- =========================================================
-- Travel attendance notification category + payslip travel fields
-- =========================================================
INSERT INTO "NotificationCategories" ("Id", "Code", "DisplayName", "Description", "Icon", "DisplayOrder", "IsSystem", "DefaultEnabled", "CreatedAt", "UpdatedAt")
SELECT 'a0000001-0000-0000-0000-000000000012'::uuid,
       'travel_attendance',
       'Chấm đi đường',
       'Chấm đi đường, duyệt giờ đi đường',
       'directions_car',
       2,
       true,
       true,
       ...
       NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM "NotificationCategories" WHERE "Code" = 'travel_attendance'
);

ALTER TABLE "Payslips" ADD COLUMN IF NOT EXISTS "TravelHours" numeric NULL;
ALTER TABLE "Payslips" ADD COLUMN IF NOT EXISTS "TravelSalary" numeric NULL;
...
SELECT '11111111-1111-1111-1111-111111111085'::uuid,
       'BusinessTripExpense', 'Công tác phí', 'Ứng công tác, hoạch toán chi phí đi công tác', 17, NOW()
WHERE NOT EXISTS (SELECT 1 FROM "Permissions" WHERE "Module" = 'BusinessTripExpense');

-- AuditableEntity: EF maps LastModified / LastModifiedBy
...
-- Per-employee travel salary mode on salary profile
