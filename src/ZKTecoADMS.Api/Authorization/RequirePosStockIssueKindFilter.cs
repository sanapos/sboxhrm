...
public sealed class RequirePosStockIssueKindFilter(
    ...
    IModulePermissionService permissionService) : IAsyncAuthorizationFilter
{
    public async Task OnAuthorizationAsync(AuthorizationFilterContext context)
    {
        ...
        var kind = (context.RouteData.Values["kind"] ?? "").ToString().Trim().ToLowerInvariant();
        var module = kind is "damage" or "huy" or "xuathuy"
            ? "PosDamageIssues"
            : "PosInternalUseIssues";

        var allowed = await permissionService.HasPermissionAsync(
            userId, role, storeId, module, action, context.HttpContext.RequestAborted);
        if (allowed)
            return;
    ...
    private static bool TryGetUserContext(
        ClaimsPrincipal user, out Guid userId, out string role, out Guid? storeId)
    {
        userId = Guid.Empty;
