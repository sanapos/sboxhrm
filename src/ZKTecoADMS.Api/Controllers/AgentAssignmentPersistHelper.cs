...
{
    public static async Task<(bool Ok, string? Error)> AssignStoreToAgentAsync(
    ...
    public static async Task<(bool Ok, string? Error)> RemoveStoreFromAgentAsync(
        ZKTecoDbContext db, Guid storeId, Guid agentId, string updatedBy)
    {
        var now = DateTime.UtcNow;
        var updated = await db.Stores
        ...
        var already = await db.Stores.AsNoTracking()
            .AnyAsync(s => s.Id == storeId && s.AgentId == null);
        return already
            ? (true, null)
