    ...
    : IQueryHandler<GetPendingLeavesQuery, AppResponse<PagedResult<LeaveDto>>>
{
    public async Task<AppResponse<PagedResult<LeaveDto>>> Handle(GetPendingLeavesQuery request, CancellationToken cancellationToken)
    {
        var pagedResult = await repository.GetPagedResultWithIncludesAsync(
            ...
            filter: l  => l.StoreId == request.StoreId && l.Status == LeaveStatus.Pending &&
                         (request.IsManager
                             ? (request.SubordinateUserIds == null
                                || request.SubordinateUserIds.Contains(l.EmployeeUserId)
                                || l.ApprovalRecords.Any(r => r.AssignedUserId == request.UserId && r.Status == ApprovalStatus.Pending))
                             : l.EmployeeUserId == request.UserId),
            includes: q => q.Include(l => l.EmployeeUser).Include(l => l.ApprovalRecords),
